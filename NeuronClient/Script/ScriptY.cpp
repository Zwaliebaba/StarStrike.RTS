							...
							CODE_BLOCK		**ppsCBlock)	// The generated code block
{
	...
	ASSERT_TEXT(psFSymbol != NULL, "ais_CodeFunction: Invalid function symbol pointer");
	ASSERT_TEXT(PTRVALID(psPBlock, sizeof(PARAM_BLOCK)),
		"scriptCodeFunction: Invalid param block pointer");
	ASSERT_TEXT((psPBlock->size == 0) || PTRVALID(psPBlock->pCode, psPBlock->size),
		"scriptCodeFunction: Invalid parameter code pointer");
	ASSERT_TEXT(ppsCBlock != NULL,
...
CODE_ERROR scriptCodeObjAssignment(OBJVAR_BLOCK	*psVariable,// The variable to assign to
								 ...
								 CODE_BLOCK		**ppsBlock)	// Generated code
{
	ASSERT_TEXT(PTRVALID(psVariable, sizeof(OBJVAR_BLOCK)),
		"scriptCodeObjAssignment: Invalid object variable block pointer");
	ASSERT_TEXT(PTRVALID(psVariable->pCode, psVariable->size),
		"scriptCodeObjAssignment: Invalid object variable code pointer");
	ASSERT_TEXT(psVariable->psObjVar != NULL,
...
//	UBYTE		*pElement;
	...
	ASSERT_TEXT(PTRVALID(psVariable, sizeof(ARRAY_BLOCK)),
		"scriptCodeObjAssignment: Invalid object variable block pointer");
	ASSERT_TEXT(PTRVALID(psVariable->pCode, psVariable->size),
		"scriptCodeObjAssignment: Invalid object variable code pointer");
	ASSERT_TEXT(psVariable->psArrayVar != NULL,
		"scriptCodeObjAssignment: Invalid object variable symbol pointer");
	ASSERT_TEXT(PTRVALID(psValue, sizeof(CODE_BLOCK)),
		"scriptCodeObjAssignment: Invalid value code block pointer");
	ASSERT_TEXT(PTRVALID(psValue->pCode, psValue->size),
					...
					CODE_BLOCK **ppsBlock)		// The final conditional code
{
	...
	ASSERT_TEXT(PTRVALID(psCondBlock, sizeof(CODE_BLOCK)),
		"scriptCodeConditional: Invalid conditional code block pointer");
	ASSERT_TEXT(PTRVALID(psCondBlock->pCode, psCondBlock->size),
...
{
	ASSERT_TEXT(PTRVALID(psParam, sizeof(CODE_BLOCK)),
		"scriptCodeParameter: Invalid parameter code block pointer");
	ASSERT_TEXT(PTRVALID(psParam->pCode, psParam->size),
...
{
	ASSERT_TEXT(PTRVALID(psObjCode, sizeof(CODE_BLOCK)),
		"scriptCodeObjectVariable: Invalid object code block pointer");
	ASSERT_TEXT(PTRVALID(psObjCode->pCode, psObjCode->size),
		"scriptCodeObjectVariable: Invalid object code pointer");
	ASSERT_TEXT(psVar != NULL,
...
{
	ASSERT_TEXT(PTRVALID(psArrayCode, sizeof(CODE_BLOCK)),
		"scriptCodeObjectVariable: Invalid object code block pointer");
	ASSERT_TEXT(PTRVALID(psArrayCode->pCode, psArrayCode->size),
		"scriptCodeObjectVariable: Invalid object code pointer");
	ASSERT_TEXT(psVar != NULL,
		"scriptCodeObjectVariable: Invalid variable symbol pointer");
	ASSERT_TEXT(ppsBlock != NULL,
...
#ifdef DEBUG
	ASSERT_TEXT(FALSE, "script parse error:\n{} at line {}\nToken: {}, Text: '{}'\n",
...
void scriptSetTypeTab(TYPE_SYMBOL *psTypeTab)
{
	...
	{
		ASSERT_TEXT(psTypeTab[i].typeID == type,
...
void scriptSetCallbackTab(CALLBACK_SYMBOL *psCallTab)
{
	...
	{
		ASSERT_TEXT(psCallTab[i].type == type,
