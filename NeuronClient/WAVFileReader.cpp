...
namespace
{
  ...
  // .WAV files
  //--------------------------------------------------------------------------------------
  constexpr uint32_t FOURCC_RIFF_TAG = FIXED_MSG("FFIR");
  constexpr uint32_t FOURCC_FORMAT_TAG = FIXED_MSG(" tmf");
  constexpr uint32_t FOURCC_DATA_TAG = FIXED_MSG("atad");
  constexpr uint32_t FOURCC_WAVE_FILE_TAG = FIXED_MSG("EVAW");
  constexpr uint32_t FOURCC_XWMA_FILE_TAG = FIXED_MSG("AMWX");
  constexpr uint32_t FOURCC_DLS_SAMPLE = FIXED_MSG("pmsw");
  constexpr uint32_t FOURCC_MIDI_SAMPLE = FIXED_MSG("lpms");
  constexpr uint32_t FOURCC_XWMA_DPDS = FIXED_MSG("sdpd");
  constexpr uint32_t FOURCC_XMA_SEEK = FIXED_MSG("kees");
  ...
  switch (wf->wFormatTag)
  {
    ...
    default:
    {
      ...
      switch (wfx->wFormatTag)
      {
        ...
        {
          if ((fmtChunk->size < sizeof(WAVEFORMATEXTENSIBLE)) || (wfx->cbSize < (sizeof(WAVEFORMATEXTENSIBLE) - sizeof(
            WAVEFORMATEX))))
            return E_FAIL;
                                                           ...
                                                           const uint8_t** startAudio, uint32_t* audioBytes)
{
  ...
  *audioBytes = 0;
  ...
  bool dpds, seek;
  HRESULT hr = WaveFindFormatAndData(wavData.data(), wavData.size(), wfx, startAudio, audioBytes, dpds, seek);
  if (FAILED(hr))
    return hr;
...
_Use_decl_annotations_ HRESULT Audio::LoadWAVAudioInMemoryEx(_In_ const byte_buffer_t& _wavData, WAVData& result)
{
  ...
  // Need at least enough data to have a valid minimal WAV file
  if (_wavData.size() < (sizeof(RIFFChunk) * 2 + sizeof(DWORD) + sizeof(WAVEFORMAT)))
    return E_FAIL;

  bool dpds, seek;
  HRESULT hr = WaveFindFormatAndData(_wavData.data(), _wavData.size(), &result.m_wfx, &result.startAudio, &result.audioBytes,
                                     dpds, seek);
  if (FAILED(hr))
    return hr;

  hr = WaveFindLoopInfo(_wavData.data(), _wavData.size(), &result.loopStart, &result.loopLength);
  if (FAILED(hr))
    return hr;

  if (dpds)
  {
    hr = WaveFindTable(_wavData.data(), _wavData.size(), FOURCC_XWMA_DPDS, &result.seek, &result.seekCount);
    if (FAILED(hr))
      return hr;
  }
  else if (seek)
  {
    hr = WaveFindTable(_wavData.data(), _wavData.size(), FOURCC_XMA_SEEK, &result.seek, &result.seekCount);
    if (FAILED(hr))
      return hr;
  }
