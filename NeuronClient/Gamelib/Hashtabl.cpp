...
{
	ASSERT_TEXT( PTRVALID(psTable, sizeof(HASHTABLE)),
...
hashTable_Clear( HASHTABLE *psTable )
{
	...
	for ( i=0; i<psTable->udwTableSize; i++ )
	{
		...
		while ( psNode != NULL )
		{
			...
			ASSERT_TEXT( PTRVALID(psNode->psElement, psTable->udwElementSize),
			...
			/* return node to heap */
			ASSERT_TEXT( PTRVALID(psNode, sizeof(HASHNODE)),
							...
							int iKey1, int iKey2 )
{
	...
	ASSERT_TEXT( PTRVALID(psElement, psTable->udwElementSize),
