...
int iV_CreateFontIndirect(IMAGEFILE* ImageFile, UWORD* AsciiTable, int SpaceSize)
{
  ...
  IVIS_FONT* Font = &iVFonts[NumFonts];
  ...
  {
    UWORD Index = AsciiTable[c];
    int Above = iV_GetImageYOffset(Font->FontFile, Index);
    int Below = Above + iV_GetImageHeight(Font->FontFile, Index);
    int Height = abs(Above) + abs(Below);
...
UDWORD pie_DrawFormattedText(const char* String, UDWORD x, UDWORD y, UDWORD Width, UDWORD Justify, BOOL DrawBack)
{
  ...
  while (si < Len)
  {
    ...
    if (FFlags & FTEXTF_SKIP_LEADING_SPACES) { while ((si < strlen(String)) && (String[si] == ' ')) { si++; } }
    ...
    while ((si < strlen(String)) && (WWidth <= Width) && (!NewLine))
    {
      int osi = si;
    ...
    TWidth = iV_GetTextWidth((const char*)FString);
...
{
  uint8* bp = psRendSurface->buffer + x + psRendSurface->scantable[y];

  int lineSkip = psRendSurface->width - w;
  for (int i = 0; i < h; i++)
  {
    for (int j = 0; j < w; j++)
    {
...
void pie_RenderCharToSurface(UDWORD* lpSurface, SDWORD pitch, IMAGEFILE* ImageFile, UWORD ID, int x, int y, UWORD colour)
{
  ...
  UDWORD Modulus = ImageFile->TexturePages[Image->TPageID].width;
  iBitmap* bmp = ImageFile->TexturePages[Image->TPageID].bmp;
  ...
  UWORD* bp = (UWORD*)lpSurface + x + y * pitch;
...
void pie_TextRender(IMAGEFILE* ImageFile, UWORD ID, int x, int y)
{
  ...
  else
  {
    ...
    else
    {
      ...
      UDWORD Red = psPalette[TextColourIndex].r;
      UDWORD Green = psPalette[TextColourIndex].g;
      UDWORD Blue = psPalette[TextColourIndex].b;
...
void TextRender270(IMAGEFILE* ImageFile, UWORD ImageID, int x, int y)
{
  ...
  IMAGEDEF* Image = &ImageFile->ImageDefs[ImageID];
  int Modulus = ImageFile->TexturePages[Image->TPageID].width;
  ...
  uint8* srcbp = bp = psRendSurface->buffer + x + psRendSurface->scantable[y + h - 1];
  UDWORD width = pie_GetVideoBufferWidth();
  for (int j = 0; j < h; j++)
  {
    bp = srcbp;
    for (int i = 0; i < w; i++)
    {
      if (*Bmp++)
      {
        UBYTE present = *bp; // What colour is there at the moment?
