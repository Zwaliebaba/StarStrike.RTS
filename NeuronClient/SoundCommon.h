...
namespace Neuron::Audio
{
  ...
  void CreateADPCM(_Out_writes_bytes_(wfxSize) WAVEFORMATEX* wfx, size_t wfxSize, int sampleRate, int channels,
                   int samplesPerBlock);
  ...
  class SoundEffectInstanceBase
  {
    public:
      ...
      ~SoundEffectInstanceBase() { DEBUG_ASSERT(!voice); }
      ...
      bool Play() // Returns true if STOPPED -> PLAYING
      {
        if (voice)
        {
          if (state == PAUSED)
          {
            check_hresult(voice->Start(0));
            state = PLAYING;
          }
          else if (state != PLAYING)
          {
            if (mVolume != 1.f)
              check_hresult(voice->SetVolume(mVolume));

            if (mPitch != 1.f)
              check_hresult(voice->SetFrequencyRatio(mPitch));
      ...
      {
        DEBUG_ASSERT(volume >= -XAUDIO2_MAX_VOLUME_LEVEL && volume <= XAUDIO2_MAX_VOLUME_LEVEL);
      ...
      void SetPitch(float pitch)
      {
        ...
        DEBUG_ASSERT(pitch >= XAUDIO2_MIN_FREQ_RATIO && pitch <= XAUDIO2_DEFAULT_FREQ_RATIO);
