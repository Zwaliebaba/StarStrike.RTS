...
void strresReleaseIDStrings(STR_RES *psRes)
{
	...
	ASSERT_TEXT(PTRVALID(psRes, sizeof(STR_RES)),
...
BOOL strresLoadFixedID(STR_RES *psRes, STR_ID *psID, UDWORD numID)
{
	...
	for (i=0; i<numID; i++)
	{
		ASSERT_TEXT(psID->id == psRes->nextID,
...
BOOL strresStoreString(STR_RES *psRes, char *pID, char *pString)
{
	...
	if (!psID)
	{
		...
		psID->pIDStr = (char *)MALLOC(sizeof(STRING) * (stringLen(pID) + 1));
		if (!psID->pIDStr)
		{
			DBERROR(("strresStoreString: Out of memory"));
			FREE(psID);
	...
	pNew = (char *)MALLOC(sizeof(STRING) * (stringLen(pString) + 1));
	if (!pNew)
	{
		DBERROR(("strresStoreString: Out of memory"));
		return FALSE;
...
char *strresGetString(STR_RES *psRes, UDWORD id)
{
	...
	if (!psBlock)
	{
		ASSERT_TEXT(FALSE, "strresGetString: String !found");
		// Return the default string
		return psRes->psStrings->apStrings[0];
	}

	ASSERT_TEXT(psBlock->apStrings[id - psBlock->idStart] != NULL,
