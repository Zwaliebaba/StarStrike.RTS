...
void frameSetCursorFromRes(WORD resID)
{
  ...
  if (resID != currentCursorResID)
  {
    ...
    for (i = 0; (i < nextCursor && hNewCursor == nullptr); i++)
    {
...
static BOOL WinInitGlide(HINSTANCE hInstance, char* name, int width, int height, BOOL maximize)
{
  ...
  cls.hbrBackground = static_cast<HBRUSH>(GetStockObject(NULL_BRUSH)); //GetStockObject(BLACK_BRUSH);
...
#ifdef DEBUG
  hWndMain = CreateWindowEx(WS_EX_APPWINDOW, "Framework", name, WS_POPUP, 0, 0, width, height, nullptr, nullptr, cls.hInstance, nullptr);
#else
  hWndMain = CreateWindowEx(WS_EX_APPWINDOW | WS_EX_TOPMOST, "Framework", name, WS_POPUP, 0, 0, width, height, NULL, NULL, cls.hInstance,
                            NULL);
#endif
  //	width; height;
                       ...
                       UDWORD height) // The window height
{
  ...
  hInternalCursor = CreateCursor(hInstance, DEF_CURSOR_X, DEF_CURSOR_Y, DEF_CURSOR_WIDTH,DEF_CURSOR_HEIGHT, aCursorMask, aCursorData);
...
BOOL loadFile(char* pFileName, UBYTE** ppFileData, UDWORD* pFileSize) { return (loadFile2(pFileName, ppFileData, pFileSize,TRUE)); }
...
BOOL loadFileToBuffer(char* pFileName, UBYTE* pFileBuffer, UDWORD bufferSize, UDWORD* pSize)
{
  ...
  res = loadFileFromWDG(pFileName, &pFileBuffer, &FileSize,WDG_USESUPPLIED);
