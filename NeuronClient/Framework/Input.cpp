...
void keyScanToString(KEY_CODE code, char* ascii, UDWORD maxStringSize)
{
  ...
  ASSERT_TEXT((code >= 0) && (code <= KEY_MAXSCAN), "Invalid key code: {}", static_cast<int>(code));
...
void inputProcessMessages(UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_SYSKEYDOWN:
    ...
    if ((aKeyState[code] == KEY_UP) || (aKeyState[code] == KEY_RELEASED) || (aKeyState[code] == KEY_PRESSRELEASE))
...
case WM_MOUSEWHEEL:	// not defined in non-NT.....bugger.
  ...
  case WM_MOUSEMOVE:
    ...
    break;
  case WM_LBUTTONDOWN:
    if ((aMouseState[MOUSE_LMB] == KEY_UP) || (aMouseState[MOUSE_LMB] == KEY_RELEASED) || (aMouseState[MOUSE_LMB] == KEY_PRESSRELEASE))
    {
  ...
  case WM_LBUTTONUP:
    ...
    else if (aMouseState[MOUSE_LMB] == KEY_DOWN || aMouseState[MOUSE_LMB] == KEY_DRAG)
  ...
  case WM_LBUTTONDBLCLK:
    ...
    break;
  case WM_RBUTTONDOWN:
    if ((aMouseState[MOUSE_RMB] == KEY_UP) || (aMouseState[MOUSE_RMB] == KEY_RELEASED) || (aMouseState[MOUSE_RMB] == KEY_PRESSRELEASE))
    {
  ...
  case WM_RBUTTONUP:
    ...
    else if (aMouseState[MOUSE_RMB] == KEY_DOWN || aMouseState[MOUSE_RMB] == KEY_DRAG)
  ...
  case WM_RBUTTONDBLCLK:
    ...
    break;
  case WM_MBUTTONDOWN:
    if ((aMouseState[MOUSE_MMB] == KEY_UP) || (aMouseState[MOUSE_MMB] == KEY_RELEASED) || (aMouseState[MOUSE_MMB] == KEY_PRESSRELEASE))
    {
  ...
  case WM_MBUTTONUP:
    ...
    else if (aMouseState[MOUSE_MMB] == KEY_DOWN || aMouseState[MOUSE_MMB] == KEY_DRAG)
  ...
  case WM_KILLFOCUS:
    ...
    for (i = 0; i < KEY_MAXSCAN; i++)
    {
      if ((aKeyState[i] == KEY_PRESSED) || (aKeyState[i] == KEY_DOWN))
        aKeyState[i] = KEY_RELEASED;
    }
    for (i = 0; i < 3; i++)
    {
      if ((aMouseState[i] == KEY_PRESSED) || (aMouseState[i] == KEY_DOWN) || (aMouseState[i] == KEY_DRAG))
...
void inputNewFrame(void)
{
  ...
  for (i = 0; i < KEY_MAXSCAN; i++)
  {
    ...
    else if ((aKeyState[i] == KEY_RELEASED) || (aKeyState[i] == KEY_PRESSRELEASE))
      aKeyState[i] = KEY_UP;
  }
  ...
  for (i = 0; i < 3; i++)
  {
    ...
    else if ((aMouseState[i] == KEY_RELEASED) || (aMouseState[i] == KEY_DOUBLECLICK) || (aMouseState[i] == KEY_PRESSRELEASE))
      aMouseState[i] = KEY_UP;
  }
}
...
{
  ASSERT_TEXT((code >= 0) && (code < KEY_MAXSCAN), "Invalid key code: {}", (int)code);
...
SDWORD mouseX(void) { return mouseXPos; }

/* Return the Y coordinate of the mouse */
SDWORD mouseY(void) { return mouseYPos; }

/* The next three are mutually exclusive */

/* True if mouse wheel moved forward in last frame */
BOOL mouseWheelForward(void) { return (bMouseWheelForward); }

/* True if mouse wheel moved backwards in last frame */
BOOL mouseWheelBackwards(void) { return (bMouseWheelBackwards); }

/* True if mouse wheel remained still in last frame */
BOOL mouseWheelStatic(void) { return (bMouseWheelStatic); }

/* This returns true if the mouse key is currently depressed */
BOOL mouseDown(MOUSE_KEY_CODE code)
{
  ASSERT_TEXT((code >= 0) && (code < 3), "Invalid mouse key code: {}", (int)code);
