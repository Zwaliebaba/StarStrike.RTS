...
/// Represents a single sector in the 4×4 world grid.
...
public:
    ...
    /// World-space min corner of this sector.
    [[nodiscard]] Vec3i minBound() const noexcept;

    /// World-space max corner of this sector (exclusive).
    [[nodiscard]] Vec3i maxBound() const noexcept;

    /// Is the given world position inside this sector?
    [[nodiscard]] bool isInBounds(const Vec3i& worldPos) const noexcept;

    /// Convert a world position inside this sector to its owning ChunkID.
    [[nodiscard]] ChunkID worldPosToChunkID(const Vec3i& worldPos) const;
...
public:
    ...
    /// Find which sector contains the given world position (or nullptr).
    [[nodiscard]] const Sector* findSectorForWorldPos(const Vec3i& worldPos) const;
