...
galaxy_seed docked_planet;

galaxy_seed hyperspace_planet;

planet_data current_planet_data;
...
struct commander saved_cmdr = {"JAMESON",/* Name 			*/
                               0,/* Mission Number 	*/
                               0x14, 0xAD,/* Ship X,Y			*/
...
struct ship_data *ship_list[NO_OF_SHIPS + 1] = {nullptr, &missile_data, &coriolis_data, &esccaps_data, &alloy_data, &cargo_data, &boulder_data, &asteroid_data, &rock_data, &orbit_data, &transp_data, &cobra3a_data, &pythona_data, &boa_data, &anacnda_data, &hermit_data, &viper_data, &sidewnd_data, &mamba_data, &krait_data, &adder_data, &gecko_data, &cobra1_data, &worm_data, &cobra3b_data, &asp2_data, &pythonb_data, &ferdlce_data, &moray_data, &thargoid_data, &thargon_data, &constrct_data, &cougar_data, &dodec_data};
