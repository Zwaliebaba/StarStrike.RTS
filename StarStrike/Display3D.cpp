/* 
	Display3D.c - draws the 3D terrain view. Both the 3D && pseudo-3D components:-
	textured tiles.
...
/* Vectors that hold the player && camera directions && positions */
iView player, camera;
...
/* Points for flipping the texture around if the tile is flipped || rotated */
...
/* Records the present X && Y values for the current mouse tile (in tiles */
...
/* Mouse x && y - saves reading them every time we want to use them */
extern SDWORD mX, mY;
...
char buildInfo[255];
UDWORD GetCameraDistance(void) { return distance; }
...
void draw3DScene(void)
{
  ...
  */
  ...
  else
  {
    ...
    if (!gamePaused())
    {
      iV_DrawText((const char*)getLevelName(),RET_X + 134, 420 + E_H);
...
void CalcBSPCameraPos(iVector* NewBSPCamera)
{
  ...
  Camera.pitch = static_cast<SWORD>(player.r.x);
  Camera.yaw = static_cast<SWORD>(player.r.y);
...
void drawTiles(iView* camera, iView* player)
{
  ...
  /* ---------------------------------------------------------------- */
  /* Rotate && project all the tiles within the grid                 */
  /* ---------------------------------------------------------------- */
  ...
  */
  ...
  for (i = 0; i < static_cast<SDWORD>(visibleYTiles) + 1; i++)
  {
    ...
    for (j = 0; j < static_cast<SDWORD>(visibleXTiles) + 1; j++)
    {
      ...
      if ((playerXTile + j < 0) || (playerZTile + i < 0) || (playerXTile + j > static_cast<SDWORD>(mapWidth - 1)) || (playerZTile + i >
        static_cast<SDWORD>(mapHeight - 1)))
      {
        ...
        if ((playerXTile + j < -1) || (playerZTile + i < -1) || (playerXTile + j > static_cast<SDWORD>(mapWidth - 1)) || (playerZTile + i >
          static_cast<SDWORD>(mapHeight - 1)))
      ...
      else
      {
        ...
        /* Is it in the centre && therefore worth averaging height over? */
  ...
  //	doBuildingLights();
  /* ---------------------------------------------------------------- */
  /* Draw all the tiles || add them to bucket sort                     */
  /* ---------------------------------------------------------------- */
  tilesRejected = 0;
  for (i = 0; i < static_cast<SDWORD>(visibleYTiles); i++)
  {
    for (j = 0; j < static_cast<SDWORD>(visibleXTiles); j++)
    {
      ...
      if (tileScreenInfo[i][j].drawInfo == TRUE)
      {
        ...
        if (static_cast<UDWORD>(i) > mapHeight || static_cast<UDWORD>(j) > mapWidth)
          ASSERT_TEXT(FALSE, "Weirdy tile coords");
...
BOOL init3DView(void)
{
  ...
  /* Make sure && change these to comply with map.c */
  imdRot.x = -35;
  /* Maximum map size */
  terrainMaxX = 128;
  terrainMaxY = 128;
...
/* John's routine - deals with flipping around the vertex ordering for source textures
   when flips && rotations are being done */
void flipsAndRots(int texture)
{
...
BOOL clipDroid(DROID* psDroid)
{
  if (psDroid->x >= static_cast<UDWORD>(player.p.x) && psDroid->x < static_cast<UDWORD>(player.p.x) + (visibleXTiles * TILE_UNITS) &&
    psDroid->y >= static_cast<UDWORD>(player.p.z) && psDroid->y < static_cast<UDWORD>(player.p.z) + (visibleYTiles * TILE_UNITS))
    return (TRUE);
  return (FALSE);
}

/* Clips anything - !necessarily a droid */
BOOL clipXY(SDWORD x, SDWORD y)
{
  if (x > player.p.x && x < static_cast<SDWORD>(player.p.x + (visibleXTiles * TILE_UNITS)) && y > player.p.z && y < static_cast<SDWORD>(
...
void calcFlagPosScreenCoords(SDWORD* pX, SDWORD* pY, SDWORD* pR)
{
  ...
  /* Pop matrices && get the screen coordinates for last point*/
...
/* Renders the bullets && their effects in 3D */
void display3DProjectiles(void)
{
  PROJ_OBJECT* psObj;
      ...
      //			if(GFX_VISIBLE(psObj))
      {
        ...
        if (gameTime != psObj->born)
        {
          ...
          */
          ...
          if (psObj->psWStats->weaponSubClass == WSC_ROCKET || psObj->psWStats->weaponSubClass == WSC_MISSILE || psObj->psWStats->
            weaponSubClass == WSC_COMMAND || psObj->psWStats->weaponSubClass == WSC_SLOWMISSILE || psObj->psWStats->weaponSubClass ==
            WSC_SLOWROCKET || psObj->psWStats->weaponSubClass == WSC_ENERGY || psObj->psWStats->weaponSubClass == WSC_EMP)
...
void renderProjectile(PROJ_OBJECT* psCurr)
{
  ...
  /* Reject flame || command since they have interim drawn fx */
  if (psStats->weaponSubClass == WSC_FLAME || psStats->weaponSubClass == WSC_COMMAND || // OR psStats->weaponSubClass == WSC_ENERGY)
    psStats->weaponSubClass == WSC_ELECTRONIC || psStats->weaponSubClass == WSC_EMP || (bMultiPlayer && psStats->weaponSubClass ==
      WSC_LAS_SAT))
  ...
  if (clipXY(psCurr->x, psCurr->y))
  {
    ...
    brightness = lightDoFogAndIllumination(pie_MAX_BRIGHT_LEVEL, getCentreX() - psCurr->x, getCentreZ() - psCurr->y, &specular);
    if (psStats->weaponSubClass == WSC_ROCKET || psStats->weaponSubClass == WSC_MISSILE || psStats->weaponSubClass == WSC_SLOWROCKET ||
...
void renderAnimComponent(COMPONENT_OBJECT* psObj)
{
  ...
  ASSERT_TEXT(PTRVALID(psParentObj, sizeof(SIMPLE_OBJECT)), "renderAnimComponent: invalid parent object pointer");
...
/*	Renders ONE terrain tile && any droids, structures, features that are on it. Sorts the objects though, so's there
	only drawn once && drawn at the appropriate time so tles aren't drawn over them */
void drawTexturedTile(UDWORD i, UDWORD j)
{
  ...
  if ((realX < 0) || (realY < 0) || (realX > mapWidth - 2) || (realY > mapHeight - 2))
  {
    psTile = &edgeTile;
    bEdgeTile = TRUE;
  ...
  if (!TILE_DRAW(psTile))
  {
    /* Bomb out if we're !supposed to draw this tile! */
    return;
  }
  ...
  //	if ( TEST_TILE_VISIBLE(selectedPlayer, psTile) OR godMode)
  {
    ...
    if (TRI_FLIPPED(psTile))
    {
      ...
      p[0].x = tileScreenInfo[i + 0][j + 0].sx;
      p[0].y = tileScreenInfo[i + 0][j + 0].sy;
      p[0].z = tileScreenInfo[i + 0][j + 0].sz;
      p[1].x = tileScreenInfo[i + 0][j + 1].sx;
      p[1].y = tileScreenInfo[i + 0][j + 1].sy;
      p[1].z = tileScreenInfo[i + 0][j + 1].sz;
      p[2].x = tileScreenInfo[i + 1][j + 0].sx;
      p[2].y = tileScreenInfo[i + 1][j + 0].sy;
      p[2].z = tileScreenInfo[i + 1][j + 0].sz;
    ...
    else
    {
      ...
      p[1].y = tileScreenInfo[i + 0][j + 1].sy;
      p[1].z = tileScreenInfo[i + 0][j + 1].sz;
      p[2].x = tileScreenInfo[i + 1][j + 1].sx;
      p[2].y = tileScreenInfo[i + 1][j + 1].sy;
      p[2].z = tileScreenInfo[i + 1][j + 1].sz;
    ...
    if (TRI_FLIPPED(psTile))
    {
      ...
      p[0].x = tileScreenInfo[i + 0][j + 1].sx;
      p[0].y = tileScreenInfo[i + 0][j + 1].sy;
      p[0].z = tileScreenInfo[i + 0][j + 1].sz;
      p[1].x = tileScreenInfo[i + 1][j + 1].sx;
      p[1].y = tileScreenInfo[i + 1][j + 1].sy;
      p[1].z = tileScreenInfo[i + 1][j + 1].sz;
      p[2].x = tileScreenInfo[i + 1][j + 0].sx;
      p[2].y = tileScreenInfo[i + 1][j + 0].sy;
      p[2].z = tileScreenInfo[i + 1][j + 0].sz;
...
void displayStaticObjects(void)
{
  ...
  for (clan = 0; clan < MAX_PLAYERS; clan++)
  {
    ...
    for (psStructure = apsStructLists[clan]; psStructure != nullptr; psStructure = psStructure->psNext)
    {
      test++;
...
void displayDelivPoints(void)
{
  ...
  for (psDelivPoint = apsFlagPosLists[selectedPlayer]; psDelivPoint != nullptr; psDelivPoint = psDelivPoint->psNext)
  {
...
void displayFeatures(void)
{
  ...
  for (psFeature = apsFeatureLists[clan]; psFeature != nullptr; psFeature = psFeature->psNext)
  {
...
/* Sets the player's position && view angle - defaults player rotations as well */
void setViewPos(UDWORD x, UDWORD y, BOOL Pan)
{
  //BOOL	changed = FALSE;
  SDWORD midX, midY;
...
void setPlayerPos(SDWORD x, SDWORD y)
{
  ...
  ASSERT_TEXT((x > 0) && (x < (SDWORD)(mapWidth*TILE_UNITS)) && (y > 0) && (y < (SDWORD)(mapHeight*TILE_UNITS)),
...
void renderProximityMsg(PROXIMITY_DISPLAY* psProxDisp)
{
  ...
  else if (psProxDisp->type == POS_PROXOBJ)
  {
    ...
    ASSERT_TEXT(FALSE, "Buggered proximity message type");
  //	centreX = ( player.p.x + ((visibleXTiles/2)<<TILE_SHIFT) );
  //	centreZ = ( player.p.z + ((visibleYTiles/2)<<TILE_SHIFT) );
  ...
  if (pViewProximity)
  {
    ...
    case PROX_ARTEFACT:
      ...
      break;
    default: ASSERT_TEXT(FALSE, "Buggered proximity message type");
      break;
    }
  }
  else
  {
    ...
    ASSERT_TEXT(((BASE_OBJECT *)psProxDisp->psMessage->pViewData)->type == OBJ_FEATURE, "renderProximityMsg: invalid feature");
...
void renderStructure(STRUCTURE* psStructure)
{
  ...
  // -------------------------------------------------------------------------------
  /* Power stations && factories have pulsing lights  */
  if (psStructure->sDisplay.imd->numFrames > 0)
  {
    ...
    So its got clan colour trim && !really an anim - these HACKS just keep
    coming back to haunt us hey? - AB 02/09/99*/
    if (bMultiPlayer && psStructure->pStructureType->type == REF_BLASTDOOR) { animFrame = getPlayerColour(psStructure->player); }
    else
    {
  ...
  if (psStructure->visible[selectedPlayer] || godMode || demoGetStatus())
  {
    ...
    buildings in other words are !made up of components - much quicker! */
    ...
    if ((gameTime2 - psStructure->timeLastHit < ELEC_DAMAGE_DURATION) && (psStructure->lastHitWeapon == WSC_ELECTRONIC))
    ...
    if (imd != nullptr && bHitByElectronic)
    {
    ...
    if ((psStructure->status == SS_BEING_BUILT) || (psStructure->status == SS_BEING_DEMOLISHED) || (psStructure->status == SS_BEING_BUILT &&
      ...
      if (psStructure->sDisplay.imd->nconnectors == 1)
      {
        ...
        if (weaponImd != nullptr)
        {
          ...
          if (psStructure->pStructureType->type == REF_REPAIR_FACILITY)
          {
            ...
            if (weaponImd->nconnectors && psRepairFac->psObj != nullptr && psRepairFac->psObj->type == OBJ_DROID && ((DROID*)psRepairFac->
          ...
          else if (weaponImd->nconnectors && psStructure->visible[selectedPlayer] > (UBYTE_MAX / 2))
          {
...
void renderDefensiveStructure(STRUCTURE* psStructure)
{
  ...
  if (psStructure->visible[selectedPlayer] || godMode || demoGetStatus())
  {
    ...
    if (godMode || demoGetStatus())
      ...
      buildingBrightness = avGetObjLightLevel((BASE_OBJECT*)psStructure, buildingBrightness);
    brightness = lightDoFogAndIllumination(static_cast<UBYTE>(buildingBrightness), getCentreX() - structX, getCentreZ() - structY,
                                           &specular);
    ...
    else if (psStructure->status == SS_BUILT)
    {
      ...
      if (psStructure->sDisplay.imd->nconnectors == 1)
      {
        ...
        if (weaponImd != nullptr)
        {
          ...
          //we have a droid weapon so do we draw a muzzle flash
          if (weaponImd->nconnectors && psStructure->visible[selectedPlayer] > (UBYTE_MAX / 2))
          {
            /* Now we need to move to the end fo the barrel */
            pie_TRANSLATE(weaponImd->connectors[0].x, weaponImd->connectors[0].z, weaponImd->connectors[0].y);
            //and draw the muzzle flash
...
void renderDeliveryPoint(FLAG_POSITION* psPosition)
{
  ...
  ASSERT_TEXT(psPosition->factoryType < NUM_FLAG_TYPES && psPosition->factoryInc < MAX_FACTORY, "Invalid assembly point");
...
BOOL renderWallSection(STRUCTURE* psStructure)
{
  ...
  if (psStructure->visible[selectedPlayer] || godMode || demoGetStatus())
  {
    ...
    fit tightly to the ground && to neighbours. 
    */
...
void renderShadow(DROID* psDroid, iIMDShape* psShadowIMD)
{
  ...
  brightness = lightDoFogAndIllumination(pie_MAX_BRIGHT_LEVEL, getCentreX() - psDroid->x, getCentreZ() - psDroid->y, &specular);
...
void drawWeaponReloadBar(BASE_OBJECT* psObj, WEAPON* psWeap)
{
  ...
  if (psStats->numRounds > 1)
    bSalvo = TRUE;
  if ((bSalvo && (psStats->reloadTime > GAME_TICKS_PER_SEC)) || (psStats->firePause > GAME_TICKS_PER_SEC) || ((psObj->type == OBJ_DROID) &&
    vtolDroid((DROID*)psObj)))
  {
    ...
    case OBJ_STRUCTURE:
      ...
      scrR = scale * 20;
      break;
    default: ASSERT_TEXT(FALSE, "drawWeaponReloadBars: invalid object type");
      damLevel = 100;
      break;
    }
...
void drawStructureSelections(void)
{
  ...
  for (psStruct = apsStructLists[selectedPlayer]; psStruct; psStruct = psStruct->psNext)
  {
    ...
    {
      /* If it's selected */
      if ((psStruct->selected) || (bMouseOverOwnStructure && (psStruct == (STRUCTURE*)psClickedOn) && (((STRUCTURE*)psClickedOn)->status ==
          SS_BUILT))
...
void showDroidSelection(DROID* psDroid)
{
  /* First, let's see if we need to - Get out if it's !selected && no appropriate to highlight */
...
void drawDeliveryPointSelection(void)
{
  ...
  for (psDelivPoint = apsFlagPosLists[selectedPlayer]; psDelivPoint; psDelivPoint = psDelivPoint->psNext)
  {
    if (psDelivPoint->selected && psDelivPoint->frameNumber == currentGameFrame)
    {
      ...
      if (pie_Hardware())
      {
        /* Three DFX clips properly right now - !sure if software does */
        if ((scrX + scrR) > 0 && (scrY + scrR) > 0 && (scrX - scrR) < DISP_WIDTH && (scrY - scrR) < DISP_HEIGHT)
          iV_Box(scrX - scrR, scrY - scrR, scrX + scrR, scrY + scrR, 110);
      }
      else
      {
        if ((scrX - scrR > 0) && (scrX + scrR < DISP_WIDTH) && (scrY - scrR > 0) && (scrY + scrR < DISP_HEIGHT))
          iV_Box(scrX - scrR, scrY - scrR, scrX + scrR, scrY + scrR, 110);
      }
    }
  }
...
void drawDroidSelections(void)
{
  ...
  case BAR_DOT:
    ...
    bTinyBars = TRUE;
    break;
  default: ASSERT_TEXT(FALSE, "Invalid energy bar display value");
    break;
  }
  ...
  {
    bBeingTracked = FALSE;
    /* If it's selected && on screen || it's the one the mouse is over ||*/
    // ABSOLUTELY MAD LOGICAL EXPRESSION!!! :-)
    if ((eitherSelected(psDroid) && psDroid->sDisplay.frameNumber == currentGameFrame) || (bMouseOverOwnDroid && (psDroid == (DROID*)
      psClickedOn)) || (droidUnderRepair(psDroid) && psDroid->sDisplay.frameNumber == currentGameFrame))
    {
      ...
      {
        if (damage > REPAIRLEV_HIGH)
          powerCol = defaultColours.green;
        else if (damage > REPAIRLEV_LOW)
          powerCol = defaultColours.yellow;
        else
          powerCol = defaultColours.red;
      }
      //show resistance values if CTRL/SHIFT depressed(now done in reload bar)
      //            if (ctrlShiftDown())
      //          {
...
/*	Get the onscreen corrdinates of a droid - so we can draw a bounding box - this need to be severely
	speeded up && the accuracy increased to allow variable size bouding boxes */
void calcScreenCoords(DROID* psDroid)
{
  //BOOL	setMouse = FALSE;
  SDWORD centX, centY, centZ;
  ...
  /* Pop matrices && get the screen corrdinates */
  pie_ROTATE_PROJECT(centX, centY, centZ, cX, cY);
    ...
    for (j = 0; j < visibleYTiles; j++)
    {
      ...
      if (tileZ <= nearestZ)
      {
        ...
        if (inQuad(&pt, &quad))
        {
          ...
          if ((tile3dX > 0) && (tile3dY > 0) && (tile3dX <= mapWidth - 1) && (tile3dY <= mapHeight - 1))
...
/* Flattens an imd to the landscape && handles 4 different rotations */
iIMDShape* flattenImd(iIMDShape* imd, UDWORD structX, UDWORD structY, UDWORD direction)
{
  UDWORD i;
  ...
  ASSERT_TEXT(imd->npoints < MAX_FLATTEN_POINTS, "flattenImd: too many points in the PIE to flatten it");
  ...
  /* Little hack below 'cos sometimes they're !exactly 90 degree alligned. */
  direction /= 90;
  direction *= 90;
...
// -------------------------------------------------------------------------------------
/* New improved (&& much faster) tile drawer */
// -------------------------------------------------------------------------------------
void drawTerrainTile(UDWORD i, UDWORD j) //hardware only
{
  ...
  /* Let's just get out now if we're !supposed to draw it */
  ...
  */
  ...
  /* Check for rotations && flips - this sets up the coordinates for texturing */
  flipsAndRots(tileNumber & ~TILE_NUMMASK);
  ...
  if (TRI_FLIPPED(psTile))
  {
    if (pie_GetRenderEngine() == ENGINE_GLIDE)
    {
      pie_DrawFastTriangle((PIEVERTEX*)&tileScreenInfo[i + 0][j + 0], (PIEVERTEX*)&tileScreenInfo[i + 0][j + 1],
                           (PIEVERTEX*)&tileScreenInfo[i + 1][j + 0], &texturePage, 0, 0);
    }
  ...
  else
  {
    ...
    {
      pie_DrawFastTriangle((PIEVERTEX*)&tileScreenInfo[i + 0][j + 0], (PIEVERTEX*)&tileScreenInfo[i + 0][j + 1],
                           (PIEVERTEX*)&tileScreenInfo[i + 1][j + 1], &texturePage, 0, 0);
    }
    else if (pie_GetRenderEngine() == ENGINE_D3D)
    {
      memcpy(&aVrts[0], &tileScreenInfo[i + 0][j + 0], sizeof(PIEVERTEX));
  ...
  if (TRI_FLIPPED(psTile))
  {
    if (pie_GetRenderEngine() == ENGINE_GLIDE)
    {
      pie_DrawFastTriangle((PIEVERTEX*)&tileScreenInfo[i + 0][j + 1], (PIEVERTEX*)&tileScreenInfo[i + 1][j + 1],
                           (PIEVERTEX*)&tileScreenInfo[i + 1][j + 0], &texturePage, 0, 0);
    }
    else if (pie_GetRenderEngine() == ENGINE_D3D)
    {
  ...
  else
  {
    if (pie_GetRenderEngine() == ENGINE_GLIDE)
    {
      pie_DrawFastTriangle((PIEVERTEX*)&tileScreenInfo[i + 0][j + 0], (PIEVERTEX*)&tileScreenInfo[i + 1][j + 1],
                           (PIEVERTEX*)&tileScreenInfo[i + 1][j + 0], &texturePage, 0, 0);
    }
    else if (pie_GetRenderEngine() == ENGINE_D3D)
    {
...
void drawTerrainWEdgeTile(UDWORD i, UDWORD j)
{
  ...
  if (TRI_FLIPPED(psTile))
  {
    ...
    if (pie_GetRenderEngine() == ENGINE_GLIDE) { pie_DrawFastTriangle(&aVrts[0], &aVrts[1], &aVrts[2], &texturePage, 0,pie_ADDITIVE); }
    else if (pie_GetRenderEngine() == ENGINE_D3D)
      pie_DrawPoly(3, aVrts, tileTexInfo[tileNumber & TILE_NUMMASK].texPage, nullptr);
  }
  else
...
void drawTerrainWaterTile(UDWORD i, UDWORD j) //hardware only
{
  ...
  if (TERRAIN_TYPE(psTile) == TER_WATER)
  {
    ...
    if (pie_GetRenderEngine() == ENGINE_GLIDE)
    {
      pie_DrawFastTriangle(&aVrts[0], &aVrts[1], &aVrts[2], &texturePage,WATER_TRANS_MODE,WATER_ALPHA_LEVEL);
    }
    else if (pie_GetRenderEngine() == ENGINE_D3D)
...
void trackHeight(SDWORD desiredHeight)
{
  ...
  /* Now do auto pitch as well, but only if we're !using mouselook && !tracking */
  if (!getWarCamStatus() && !getRotActive())
  {
    ...
    if (abs(static_cast<int>(pitch - desPitch)) < 2) // near enough
    {
      /*NOP*/
    }
...
void processSensorTarget(void)
{
  ...
  if (bSensorTargetting)
  {
    if ((gameTime2 - lastTargetAssignation) < TARGET_TO_SENSOR_TIME)
    {
      ...
      {
        x = /*mouseX();*/static_cast<SWORD>(psSensorObj->sDisplay.screenX);
        y = static_cast<SWORD>(psSensorObj->sDisplay.screenY);
        if (!gamePaused())
        ...
        offset = static_cast<SWORD>(12 + ((TARGET_TO_SENSOR_TIME) - (gameTime2 - lastTargetAssignation)) / 2);
...
void processDestinationTarget(void)
{
  ...
  if (bDestTargetting)
  {
    ...
    {
      x = static_cast<SWORD>(destTargetX);
      y = static_cast<SWORD>(destTargetY);

      offset = static_cast<SWORD>(((DEST_TARGET_TIME) - (gameTime2 - lastDestAssignation)) / 2);
...
void testEffect2(UDWORD player)
{
  ...
  for (psStructure = apsStructLists[player]; psStructure; psStructure = psStructure->psNext)
  {
    if (psStructure->status == SS_BUILT)
    {
      ...
      else if (psStructure->pStructureType->type == REF_REARM_PAD && psStructure->visible[selectedPlayer])
      {
...
void testEffect(void)
{
  ...
  /* Hardware only effect, && then only if you've got additive! */
  if ((pie_Hardware() && war_GetAdditive()))
  {
...
UDWORD getDroidRankGraphic(DROID* psDroid)
{
  ...
  case 8:
    gfxId = IMAGE_LEV_7;
    break;
  default: ASSERT_TEXT(FALSE, "Weird droid level in drawDroidRank");
    break;
  }
  //#else
  /*
...
//			gfxId = IMAGE_GN_0;	// Unexperienced 
  ...
  case 8:
    ...
    break;
  default:
    ASSERT_TEXT(FALSE, "Weird droid level in drawDroidRank");
  break;
}
*/
  //#endif
...
/*	DOES : Assumes matrix context set && that z-buffer write is force enabled (Always). 
...
static void doConstructionLines(void)
{
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    for (psDroid = apsDroidLists[i]; psDroid; psDroid = psDroid->psNext)
    {
      if (clipXY(psDroid->x, psDroid->y))
      {
        if ((psDroid->visible[selectedPlayer] == UBYTE_MAX) && (psDroid->sMove.Status != MOVESHUFFLE))
        {
          ...
          else if ((psDroid->action == DACTION_DEMOLISH) || (psDroid->action == DACTION_REPAIR) || (psDroid->action == DACTION_CLEARWRECK)
...
static void addConstructionLine(DROID* psDroid, STRUCTURE* psStructure)
{
  ...
  if (((pts[0].sx > 0 && pts[0].sx < DISP_WIDTH) && (pts[0].sy > 0 && pts[0].sy < DISP_HEIGHT)) || ((pts[1].sx > 0 && pts[1].sx <
    DISP_WIDTH) && (pts[1].sy > 0 && pts[1].sy < DISP_HEIGHT)) || ((pts[2].sx > 0 && pts[2].sx < DISP_WIDTH) && (pts[2].sy > 0 && pts[2].sy
    < DISP_HEIGHT)))
