...
BOOL addToForce(DROID_TEMPLATE* templ)
{
  ...
  // add droid.
  auto pF = static_cast<FORCE_MEMBER*>(MALLOC(sizeof(FORCE_MEMBER))); // create a slot in the force.
  if (!pF)
    return FALSE;
...
{
  UDWORD i;
  FORCE_MEMBER* pF2 = nullptr;
  DROID_TEMPLATE* psCurr;

  FORCE_MEMBER* pF = Force.pMembers;
  for (i = 0; i < number; i++)
    pF = pF->psNext; // goto that force element;
  ...
  UDWORD templateid = pF->pTempl->ref;
  FREE(pF);
...
VOID useTheForce(BOOL bAddTempl) //Luke
{
  ...
  UDWORD x1 = x; // now we have a coord, place droids
  UDWORD y1 = y;
  ...
  while (Force.pMembers) // for each force member
  {
    ...
    DROID_TEMPLATE* pTempl = Force.pMembers->pTempl;
    ...
    */
    ...
    //		{
    DROID* pDr = buildDroid(pTempl, x << TILE_SHIFT, y << TILE_SHIFT, selectedPlayer, FALSE);
...
{
  STRING fileName[255] = "";
  FORCE_MEMBER* pMember;
  UDWORD count = 0;

  strcpy(fileName, name);

  FILE* pFileHandle = fopen(fileName, "wb"); // open the file
  if (!pFileHandle)
  {
  ...
  count = 0;
  for (DROID_TEMPLATE* pT = pfForce->pForceTemplates; pT; pT = pT->psNext)
  ...
  count = 0;
  for (FORCE_MEMBER* pCount = pfForce->pMembers; pCount; pCount = pCount->psNext)
    count++; // count droids
  if (fwrite(&count, sizeof(UDWORD), 1, pFileHandle) != 1)
  {
    DBERROR(("Write failed for %s", fileName));
...
BOOL loadForce(char* name)
{
  ...
  FILE* pFileHandle = fopen(fileName, "rb"); // check file exists
  ...
  Force.pForceTemplates = nullptr;
  Force.pMembers = nullptr;
...
PLAYERSTATS getMultiStats(UDWORD player, BOOL bLocal)
{
  ...
  DPID playerDPID = player2dpid[player];
...
BOOL loadMultiStats(char* sPlayerName, PLAYERSTATS* playerStats)
{
  ...
  CreateDirectory(fileName, nullptr); // make dir, no problem if fails!
  ...
  {
    UDWORD tmpSize = static_cast<UDWORD>(size);
    loadFile(fileName, &pFileData, &tmpSize);
  }
  auto codedst = ((SAVEDPLAYERSTATS*)pFileData);
...
VOID updateMultiStatsGames(void)
{
  PLAYERSTATS st = getMultiStats(selectedPlayer,TRUE);
  st.played++;
...
VOID updateMultiStatsKills(BASE_OBJECT* psKilled, UDWORD player)
{
  ...
  {
    PLAYERSTATS st = getMultiStats(player,TRUE);
