...
BOOL bridgeValid(UDWORD startX, UDWORD startY, UDWORD endX, UDWORD endY)
{
  ...
  BOOL xBridge = ((startX == endX) ? TRUE : FALSE);
  BOOL yBridge = ((startY == endY) ? TRUE : FALSE);
  ...
  UDWORD bridgeLength = (xBridge ? abs(static_cast<int>(startY - endY)) : abs(static_cast<int>(startX - endX)));
  ...
  UDWORD startHeight = mapTile(startX, startY)->height;
  UDWORD endHeight = mapTile(endX, endY)->height;

  /*	
    Don't whinge about this piece of code please! It's nice && short
    && is called very infrequently. Could be made slightly faster. 
  */
  for (UDWORD i = (xBridge ? (min(startY, endY)) : (min(startX, endX))); i < (xBridge
                                                                                ? (max(startY, endY))
                                                                                : (max(startX, endX))); i++)
  {
    /* Get the height of a bridge section */
    UDWORD sectionHeight = mapTile((xBridge ? startX : startY), i)->height;
...
void getBridgeInfo(UDWORD startX, UDWORD startY, UDWORD endX, UDWORD endY, BRIDGE_INFO* info)
{
  ...
  UDWORD startHeight = map_TileHeight(startX, startY);
  UDWORD endHeight = map_TileHeight(endX, endY);

  /* Find out which is higher */
  BOOL startHigher = (startHeight >= endHeight ? TRUE : FALSE);
  ...
  */
  ...
  if (xBridge)
  {
    ...
    info->bridgeLength = abs(static_cast<int>(startY - endY));
  }
  /* We've got a bridge of constant Y */
  else if (yBridge)
  {
    info->bConstantX = FALSE;
    info->bridgeLength = abs(static_cast<int>(startX - endX));
  }
  else
  ...
  if (bridgeValid(startX, startY, endX, endY))
  {
    ...
    if (bridge.bConstantX)
    {
      for (i = min(bridge.startY, bridge.endY); i < (max(bridge.startY, bridge.endY) + 1); i++)
      {
        ...
        addEffect(&dv, EFFECT_SMOKE, SMOKE_TYPE_DRIFTING,FALSE, nullptr, 0);
  ...
  else
  {
    ...
    if (bridge.bConstantX)
    {
      for (i = min(bridge.startY, bridge.endY); i < (max(bridge.startY, bridge.endY) + 1); i++)
      {
        ...
        dv.z = ((i * 128) + 64);
        dv.y = bridge.bridgeHeight;
        addEffect(&dv, EFFECT_EXPLOSION, EXPLOSION_TYPE_SMALL,FALSE, nullptr, 0);
      }
    }
    else
    {
