...
BOOL scrEnableComponent(void)
{
  ...
  case ST_BRAIN:
    ...
    break;
  default: ASSERT((FALSE, "scrEnableComponent: unknown type"));
    return FALSE;
  }
...
BOOL scrAddDroidToMissionList(void)
{
  ...
  if (!stackPushResult(ST_DROID, (SDWORD)psDroid)) { return FALSE; }
...
BOOL scrAddReticuleButton(void)
{
  ...
  case IDRET_CANCEL:
    ...
    break;
  default: ASSERT((FALSE, "scrAddReticuleButton: Invalid reticule Button ID"));
    return FALSE;
  }
  return TRUE;
}
...
BOOL scrRemoveMessage(void)
{
  ...
  {
    ASSERT((FALSE, "scrRemoveMessage:cannot find message - %s", psViewData->pName));
    return FALSE;
  }
...
BOOL scrBuildDroid(void)
{
  ...
  {
    ASSERT((FALSE, "scrBuildUnit: invalid template - %s for factory - %s", &psTemplate->aName, psFactory->pStructureType->pName));
    return FALSE;
  }
...
BOOL scrSetAssemblyPoint(void)
{
  ...
  {
    ASSERT((FALSE, "scrSetAssemblyPoint: structure is !a factory"));
    return FALSE;
  }
...
BOOL scrDestroyFeature(void)
{
  ...
  if (psFeature == NULL) { ASSERT((PTRVALID(psFeature, sizeof(FEATURE)), "scrDestroyFeature: Invalid feature pointer")); }
...
BOOL scrGetFeature(void)
{
  ...
  {
    if (!stackPushResult(ST_FEATURE, (SDWORD)NULL)) { return FALSE; }
    return TRUE;
  }
  ...
  // none found
  if (!stackPushResult(ST_FEATURE, (UDWORD)NULL)) { return FALSE; }
}
...
BOOL scrAddFeature(void)
{
  ...
  if (psStat != NULL)
  {
    ...
    for (psFeat = apsFeatureLists[0]; psFeat; psFeat = psFeat->psNext)
    {
      ...
      if ((iTestX == iMapX) && (iTestY == iMapY))
      {
        ...
        else { ASSERT((FALSE, "scrAddFeature: building feature on tile already occupied\n")); }
      }
    }
  ...
  if (!stackPushResult(ST_FEATURE, (UDWORD)psFeat)) { return FALSE; }
...
BOOL scrAddStructure(void)
{
  ...
  if (psStat != NULL)
  {
    ...
    if (TILE_HAS_STRUCTURE(mapTile(iMapX,iMapY))) { ASSERT((FALSE, "scrAddStructure: tile already occupied by structure\n")); }
...
for ( iW=iMapX; iW<=iMapX+(SDWORD)psStat->baseWidth; iW+=iWidth )
{
  ...
  if (!stackPushResult(ST_STRUCTURE, (UDWORD)psStruct)) { return FALSE; }
...
BOOL scrDestroyStructure(void)
{
  ...
  if (psStruct == NULL) { ASSERT((PTRVALID(psStruct, sizeof(STRUCTURE)), "scrDestroyStructure: Invalid structure pointer")); }
...
BOOL scrEnumStruct(void)
{
  ...
  {
    if (!stackPushResult(ST_STRUCTURE, (SDWORD)NULL)) { return FALSE; }
    return TRUE;
  }
  ...
  // push NULL, none found;
  if (!stackPushResult(ST_STRUCTURE, (UDWORD)NULL)) { return FALSE; }
  return TRUE;
}
...
BOOL scrGetTemplate(void)
{
  ...
  for (psTemplate = apsDroidTemplates[player]; psTemplate != NULL; psTemplate = psTemplate->psNext)
  {
    ...
    case ST_WEAPON:
      ...
      break;
    default: ASSERT((FALSE, "scrGetTemplate: unknown type"));
      return FALSE;
    }
  ...
  if (!stackPushResult(ST_TEMPLATE, (UDWORD)psTemplate)) { return FALSE; }
...
BOOL scrGetDroid(void)
{
  ...
  for (psDroid = apsDroidLists[player]; psDroid != NULL; psDroid = psDroid->psNext)
  {
    ...
    case ST_WEAPON:
      ...
      break;
    default: ASSERT((FALSE, "scrGetUnit: unknown type"));
      return FALSE;
    }
  ...
  if (!stackPushResult(ST_DROID, (UDWORD)psDroid)) { return FALSE; }
...
BOOL scrSetScrollParams(void)
{
  ...
  if (minY < 0) { ASSERT((FALSE, "Minimum scroll y value is less than zero - ", minY)); }
  if (maxX > (SDWORD)mapWidth) { ASSERT((FALSE, "Maximum scroll x value is greater than mapWidth - ", maxX)); }
  if (maxX < (SDWORD)(visibleXTiles + 1)) { ASSERT((FALSE, "Maximum scroll x has to be bigger than Visible Width(22) - ", maxX)); }
  if (maxY > (SDWORD)mapHeight) { ASSERT((FALSE, "Maximum scroll y value is greater than mapWidth - ", maxY)); }
  if (maxY < (SDWORD)(visibleYTiles + 1)) { ASSERT((FALSE, "Maximum scroll y has to be bigger than Visible Height(22) - ", maxY)); }
...
BOOL scrSetDefaultSensor(void)
{
  ...
  {
    ASSERT((FALSE, "scrSetDefaultSensor: This sensor is !a default one - %s", getStatName(&asSensorStats[sensorInc]) ));
    return FALSE;
  }
...
BOOL scrSetDefaultECM(void)
{
  ...
  {
    ASSERT((FALSE, "scrSetDefaultECM: This ecm is !a default one - %s", getStatName(&asECMStats[ecmInc])));
    return FALSE;
  }
...
BOOL scrSetDefaultRepair(void)
{
  ...
  {
    ASSERT((FALSE, "scrSetDefaultRepair: This repair is !a default one - %s", getStatName(&asRepairStats[repairInc])));
    return FALSE;
  }
...
BOOL scrSetStructureLimits(void)
{
  ...
  if (limit > LOTS_OF)
  {
    ASSERT((FALSE, "scrSetStructureLimits: limit is too high - %d - must be less than %d", limit, LOTS_OF));
    return FALSE;
  }
...
BOOL scrStructureBuiltInRange(void)
{
  ...
  if (!stackPushResult(ST_STRUCTURE, (UDWORD)psCurr)) { return FALSE; }
...
BOOL scrDestroyStructuresInArea(void)
{
  ...
  if (player >= MAX_PLAYERS) { ASSERT((FALSE,"Player number too high in scrDestroyStructuresInArea")); }
    ...
    if (bVisible ? psDroid->visible[playerLooking] : TRUE)
    {
      ...
      if (dX >= x1 && dX <= x2 && dY >= y1 && dY <= y2)
      {
        ...
        case SIZE_SUPER_HEAVY:
          ...
          break;
        default: ASSERT((FALSE, "Weird droid size in threat assessment"));
          break;
        }
      }
    }
...
BOOL scrForceDamage(void)
{
  ...
  case OBJ_FEATURE:
    ...
    break;
  default: ASSERT((FALSE,"Unsupported base object type in scrForceDamage"));
    return (FALSE);
    break;
  }
...
BOOL scrDestroyUnitsInArea(void)
{
  ...
  if (player >= MAX_PLAYERS) { ASSERT((FALSE,"Invalid player number in scrKillDroidsInArea")); }
...
BOOL scrGetGameStatus(void)
{
  ...
  default: ASSERT((FALSE,"ScrGetGameStatus. Invalid STATUS_ variable"));
    break;
  }
...
BOOL scrTakeOverSingleDroid(void)
{
  ...
  if (!stackPushResult(ST_DROID, (SDWORD)psNewDroid)) { return FALSE; }
  return TRUE;
}
...
BOOL scrTakeOverSingleStructure(void)
{
  ...
  if (!stackPushResult(ST_STRUCTURE, (SDWORD)psNewStruct)) { return FALSE; }
  return TRUE;
}
...
BOOL scrIsVtol(void)
{
  ...
  if (psDroid == NULL) { ASSERT((FALSE,"scrIsVtol: null droid passed in.")); }
