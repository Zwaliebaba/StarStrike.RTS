...
void resetRadarRedraw(void) { RadarRedraw = TRUE; }
...
{
  ASSERT_TEXT(ZoomLevel <= MAX_RADARZOOM,"SetRadarZoom: Max radar zoom exceeded");
...
UDWORD GetRadarZoom(void) { return RadarZoom; }
...
void CalcRadarPosition(UDWORD mX, UDWORD mY, UDWORD* PosX, UDWORD* PosY)
{
  ...
  if (sPosX < scrollMinX) { sPosX = scrollMinX; }
  else if (sPosX > scrollMaxX) { sPosX = scrollMaxX; }
  if (sPosY < scrollMinY) { sPosY = scrollMinY; }
...
static void CalcRadarScroll(UWORD boxSizeH, UWORD boxSizeV)
{
  ...
  if (BorderX > 16) { BorderX = 16; }
  else if (BorderX < 0) { BorderX = 0; }

  if (BorderY > 16) { BorderY = 16; }
  else if (BorderY < 0) { BorderY = 0; }
  ...
  viewX += visibleXTiles;
  if (viewX > (RadVisWidth / boxSizeH) - BorderX) { RadarScrollX += viewX - (RadVisWidth / boxSizeH) + BorderX; }

  if (viewY < BorderY) { RadarScrollY += viewY - BorderY; }

  viewY += visibleYTiles;
  if (viewY > (RadVisHeight / boxSizeV) - BorderY) { RadarScrollY += viewY - (RadVisHeight / boxSizeV) + BorderY; }

  if (RadarScrollX < 0) { RadarScrollX = 0; }
  else if (RadarScrollX > RadarMapWidth - (RadVisWidth / boxSizeH)) { RadarScrollX = RadarMapWidth - (RadVisWidth / boxSizeH); }

  if (RadarScrollY < 0) { RadarScrollY = 0; }
  else if (RadarScrollY > RadarMapHeight - (RadVisHeight / boxSizeV)) { RadarScrollY = RadarMapHeight - (RadVisHeight / boxSizeV); }

  if ((PrevRadarOffsetX != RadarOffsetX) || (PrevRadarOffsetY != RadarOffsetY) || (PrevRadarScrollX != RadarScrollX) || (PrevRadarScrollY !=
    RadarScrollY) || (PrevRadarMapOriginX != RadarMapOriginX) || (PrevRadarMapOriginY != RadarMapOriginY) || (PrevRadarMapWidth !=
    RadarMapWidth) || (PrevRadarMapHeight != RadarMapHeight)) { RadarRedraw = TRUE; }
}
...
void drawRadar(void)
{
...
#ifdef TESTRADAR
  godMode = TRUE; scrollMinX = 0; scrollMinY = 0; scrollMaxX = scrollMinX + 191; scrollMaxY = scrollMinY + 127;
#endif
  ...
  if (RadarRedraw)
  {
    if ((RadVisWidth != RadarWidth) || (RadVisHeight != RadarHeight)) { ClearRadar(radarBuffer,RADWIDTH, boxSizeH, boxSizeV); }
  }
  ...
  {
    iV_UniTransBoxFill(RADTLX,RADTLY, RADTLX + RADWIDTH,RADTLY + RADHEIGHT, (FILLRED << 16) | (FILLGREEN << 8) | FILLBLUE, FILLTRANS);
  }
  else { iV_TransBoxFill(RADTLX,RADTLY, RADTLX + RADWIDTH,RADTLY + RADHEIGHT); }
...
void downloadAtStartOfFrame(void) { pie_DownLoadRadar(radarBuffer,RADAR_3DFX_TPAGEID); }
...
static void UpdateRadar(UWORD boxSizeH, UWORD boxSizeV)
{
  ...
  if (!gamePaused()) { sweep += boxSizeV; }

  if (sweep >= (UDWORD)RadarHeight) { sweep = 0; }

  if (!gamePaused()) { if (sweepStrobeIndex++ >= BOX_PULSE_SIZE) { sweepStrobeIndex = 0; } }
}
...
static void DrawRadarTiles(UBYTE* screen, UDWORD Modulus, UWORD boxSizeH, UWORD boxSizeV)
{
  ...
  ASSERT_TEXT( (SizeV!=0) && (SizeV!=0) ,"Zero pixel size" );
  ...
  if (RadarRedraw) { EndY = VisHeight; }
  else
  {
    if ((SweepPos < 0) || (SweepPos >= VisHeight)) { return; }
  ...
  if (SizeH == 1)
  {
    for (i = 0; i < EndY; i += SizeH)
    {
      ...
      {
#ifdef CHECKBUFFER
        ASSERT_TEXT( ((UDWORD)WScr) >= radarBuffer , "WScr Onderflow"); ASSERT(
      ...
      for (j = 0; j < VisWidth; j += SizeH)
      {
        ...
        if (TEST_TILE_VISIBLE(selectedPlayer, WTile) || godMode)
        {
          ...
          for (c = 0; c < SizeV; c++)
          {
            ...
            {
#ifdef CHECKBUFFER
              ASSERT_TEXT( ((UDWORD)WPtr) >= (UDWORD)radarBuffer , "WPtr Onderflow"); ASSERT(
...
static void DrawRadarObjects(UBYTE* screen, UDWORD Modulus, UWORD boxSizeH, UWORD boxSizeV)
{
  ...
  if ((SweepPos < 0) || (SweepPos >= RadVisHeight)) { return; }
  ...
  for (psProxDisp = apsProxDisp[selectedPlayer]; psProxDisp != NULL; psProxDisp = psProxDisp->psNext)
  {
    ...
    {
      psViewProx = (VIEW_PROXIMITY*)((VIEWDATA*)psProxDisp->psMessage->pViewData)->pData;
      x = (psViewProx->x / TILE_UNITS) - RadarScrollX;
      y = (psViewProx->y / TILE_UNITS) - RadarScrollY;
    }
    else if (psProxDisp->type == POS_PROXOBJ)
    {
      x = (((BASE_OBJECT*)psProxDisp->psMessage->pViewData)->x / TILE_UNITS) - RadarScrollX;
      y = (((BASE_OBJECT*)psProxDisp->psMessage->pViewData)->y / TILE_UNITS) - RadarScrollY;
    }
    x -= RadarMapOriginX;
    y -= RadarMapOriginY;
    x *= boxSizeH;
...
{
  if ((x >= RADTLX - 1) && (x < RADTLX + RADWIDTH + 1) && (y >= RADTLY - 1) && (y < RADTLY + RADHEIGHT + 1)) { return TRUE; }
...
#define SAMPLES 8
  ...
  for (i = 0; i < TILE_HEIGHT; i += TILE_HEIGHT / SAMPLES)
  {
    for (j = 0; j < TILE_WIDTH; j += TILE_WIDTH / SAMPLES)
    {
      penNumber = tileBitmap[i * TILE_WIDTH + j];
      tRed += psPalette[penNumber].r;
      tGreen += psPalette[penNumber].g;
      tBlue += psPalette[penNumber].b;
    }
  }
