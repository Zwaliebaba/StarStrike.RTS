...
void atmosUpdateSystem(void)
{
  ...
  if (!gamePaused() && weather != WT_NONE)
  {
    ...
    for (i = 0; i < numberToAdd; i++)
    {
      ...
      if (pos.x > 0 && pos.z > 0 && pos.x < static_cast<SDWORD>((mapWidth - 1) * TILE_UNITS) && pos.z < static_cast<SDWORD>((mapHeight - 1)
        * TILE_UNITS))
      {
...
void processParticle(ATPART* psPart)
{
  ...
  if (!gamePaused())
  {
    ...
    if (psPart->position.x < 0 || psPart->position.z < 0 || psPart->position.x > ((mapWidth - 1) * TILE_UNITS) || psPart->position.z > ((
...
void atmosAddParticle(iVector* pos, AP_TYPE type)
{
  ...
  /* Record it's type */
  asAtmosParts[freeParticle].type = static_cast<UBYTE>(type);
...
// -----------------------------------------------------------------------------
WT_CLASS atmosGetWeatherType(void) { return (weather); }
