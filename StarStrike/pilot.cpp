...
void fly_to_vector(struct univ_object *ship, Vector vec)
{
  ...
  if ((fabs(dir) * 2) >= rat2) ship->rotx = (dir < 0) ? rat : -rat;
  ...
  if (abs(ship->rotz) < 16)
  {
    ...
    if ((fabs(dir) * 2) >= rat2)
    {
      ...
      if (ship->rotx < 0) ship->rotz = -ship->rotz;
    }
  }
  ...
  if (direction >= cnt2) { ship->acceleration = 3; }
}
...
void fly_to_docking_bay(struct univ_object *ship)
{
  ...
  {
    ship->rotz = 1;
    if (((vec.x >= 0) && (vec.y >= 0)) || ((vec.x < 0) && (vec.y < 0))) ship->rotz = -ship->rotz;
    ...
    if (fabs(vec.y) > 0.002436) ship->rotx = (vec.y < 0) ? -1 : 1;
...
void auto_pilot_ship(struct univ_object *ship)
{
  ...
  if ((ship->flags & FLG_FLY_TO_PLANET) || ((ship_count[SHIP_CORIOLIS] == 0) && (ship_count[SHIP_DODEC] == 0)))
  {
    fly_to_planet(ship);
    return;
  }
...
{
  if (auto_pilot || witchspace || hyper_ready) return;

  auto_pilot = 1;
}

void disengage_auto_pilot(void) { if (auto_pilot) auto_pilot = 0; }