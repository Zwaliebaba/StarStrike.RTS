...
extern galaxy_seed hyperspace_planet;
...
static auto digrams = "ABOUSEITILETSTONLONUTHNOALLEXEGEZACEBISOUSESARMAINDIREA?ERATENBERALAVETIEDORQUANTEISRION";
...
void waggle_galaxy(galaxy_seed *glx_ptr)
{
  ...
  unsigned int x = glx_ptr->a + glx_ptr->c;
  unsigned int y = glx_ptr->b + glx_ptr->d;
...
struct galaxy_seed find_planet(int cx, int cy) { return _find_planet(cx, cy, cmdr.galaxy); }

int _find_planet_number(galaxy_seed planet, galaxy_seed glx)
{
  for (int i = 0; i < 256; i++)
  {
    if ((planet.a == glx.a) && (planet.b == glx.b) && (planet.c == glx.c) && (planet.d == glx.d) && (planet.e == glx.e) && (planet.f == glx.f)) return i;
...
int find_planet_number(galaxy_seed planet) { return _find_planet_number(planet, cmdr.galaxy); }
...
void name_planet(char *gname, galaxy_seed glx)
{
  ...
  if ((glx.a & 0x40) == 0) size = 3;
  else size = 4;

  for (int i = 0; i < size; i++)
  {
    unsigned int x = glx.f & 0x1F;
    if (x != 0)
    {
      x += 12;
      x *= 2;
      *gp++ = digrams[x];
      if (digrams[x + 1] != '?') *gp++ = digrams[x + 1];
    }
...
void describe_inhabitants(char *str, struct galaxy_seed planet)
{
  ...
  if (planet.e < 128) strcat(str, "Human Colonial");
  else
  {
    int inhab = (planet.f / 4) & 7;
    if (inhab < 3) strcat(str, inhabitant_desc1[inhab]);

    inhab = planet.f / 32;
    if (inhab < 6) strcat(str, inhabitant_desc2[inhab]);

    inhab = (planet.d ^ planet.b) & 7;
    if (inhab < 6) strcat(str, inhabitant_desc3[inhab]);
  ...
  while (*source != '\0')
  {
    if (*source == '<')
    {
      ...
      *ptr = '\0';
      ...
      if (hoopy_casinos) option = gen_msx_rnd_number();
      else
      {
    ...
    if (*source == '%')
    {
      ...
      switch (*source)
      {
        case 'H':
          ...
          capitalise_name(str);
          for (ptr = str; *ptr != '\0';) *desc_ptr++ = *ptr++;
          break;
        ...
        case 'R':
          ...
          for (i = 0; i <= len; i++)
          {
            ...
            if (i == 0) *desc_ptr++ = digrams[x];
            else *desc_ptr++ = tolower(digrams[x]);
...
char *describe_planet(struct galaxy_seed planet)
{
  ...
  if (cmdr.mission == 1)
  {
    char* mission_text = mission_planet_desc(planet);
    if (mission_text != nullptr) return mission_text;
  }
...
void generate_planet_data(struct planet_data *pl, struct galaxy_seed planet_seed)
{
  ...
  if (pl->government < 2) pl->economy = pl->economy | 2;
