...
BOOL objectInRange(BASE_OBJECT* psList, SDWORD x, SDWORD y, SDWORD range)
{
  ...
  for (psCurr = psList; psCurr; psCurr = psCurr->psNext)
  {
    ...
    if ((psCurr->type == OBJ_STRUCTURE) && (((STRUCTURE*)psCurr)->status != SS_BUILT)) { continue; }

    // skip flying vtols
    if ((psCurr->type == OBJ_DROID) && vtolDroid((DROID*)psCurr) && ((DROID*)psCurr)->sMove.Status != MOVEINACTIVE) { continue; }

    xdiff = (SDWORD)psCurr->x - x;
    ydiff = (SDWORD)psCurr->y - y;
    if (xdiff * xdiff + ydiff * ydiff < rangeSq) { return TRUE; }
  }
...
BOOL scrObjectInRange(void)
{
  ...
  if (!stackPopParams(4, VAL_INT, &player, VAL_INT, &x, VAL_INT, &y, VAL_INT, &range)) { return FALSE; }

  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrObjectInRange: invalid player number");
    return FALSE;
  }

  found = objectInRange((BASE_OBJECT*)apsDroidLists[player], x, y, range) || objectInRange(
    (BASE_OBJECT*)apsStructLists[player], x, y, range);

  if (!stackPushResult(VAL_BOOL, found)) { return FALSE; }
...
BOOL scrDroidInRange(void)
{
  ...
  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrUnitInRange: invalid player number");
    return FALSE;
  }
...
BOOL scrStructInRange(void)
{
  ...
  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrStructInRange: invalid player number");
    return FALSE;
  }
...
BOOL scrPlayerPower(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &player)) { return FALSE; }
  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrPlayerPower: invalid player number");
    return FALSE;
  }
  if (!stackPushResult(VAL_INT, asPower[player]->currentPower)) { return FALSE; }
  return TRUE;
}
...
static BOOL objectInArea(BASE_OBJECT* psList, SDWORD x1, SDWORD y1, SDWORD x2, SDWORD y2)
{
  ...
  for (psCurr = psList; psCurr; psCurr = psCurr->psNext)
  {
    ...
    ox = (SDWORD)psCurr->x;
    oy = (SDWORD)psCurr->y;
    if (ox >= x1 && ox <= x2 && oy >= y1 && oy <= y2) { return TRUE; }
  }
...
BOOL scrObjectInArea(void)
{
  ...
  if (!stackPopParams(5, VAL_INT, &player, VAL_INT, &x1, VAL_INT, &y1, VAL_INT, &x2, VAL_INT, &y2)) { return FALSE; }

  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrObjectInArea: invalid player number");
    return FALSE;
  }

  found = objectInArea((BASE_OBJECT*)apsDroidLists[player], x1, y1, x2, y2) || objectInArea(
    (BASE_OBJECT*)apsStructLists[player], x1, y1, x2, y2);
...
BOOL scrDroidInArea(void)
{
  ...
  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrUnitInArea: invalid player number");
    return FALSE;
  }
...
BOOL scrStructInArea(void)
{
  ...
  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrStructInArea: invalid player number");
    return FALSE;
  }
...
BOOL scrSeenStructInArea(void)
{
  ...
  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSeenStructInArea: invalid player number");
    return FALSE;
  }
  ...
  for (psCurr = apsStructLists[enemy]; psCurr; psCurr = psCurr->psNext)
  {
    ...
    // possible skip walls
    if (walls && (psCurr->pStructureType->type != REF_WALL || psCurr->pStructureType->type != REF_WALLCORNER)) { continue; }
    ...
    if (ox >= x1 && ox <= x2 && oy >= y1 && oy <= y2)
    {
      ...
      if (psCurr->visible[player]) { found = TRUE; }
    }
  }
...
BOOL scrStructButNoWallsInArea(void)
{
  ...
  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrStructButNoWallsInArea: invalid player number");
    return FALSE;
  }

  for (psStruct = apsStructLists[player]; psStruct; psStruct = psStruct->psNext)
  {
    if ((psStruct->pStructureType->type != REF_WALL) && (psStruct->pStructureType->type != REF_WALLCORNER) && (psStruct->status ==
      SS_BUILT))
    {
...
static SDWORD numObjectsInArea(BASE_OBJECT* psList, SDWORD x1, SDWORD y1, SDWORD x2, SDWORD y2)
{
  ...
  for (psCurr = psList; psCurr; psCurr = psCurr->psNext)
  {
    ...
    ox = (SDWORD)psCurr->x;
    oy = (SDWORD)psCurr->y;
    if (ox >= x1 && ox <= x2 && oy >= y1 && oy <= y2) { count += 1; }
  }
...
BOOL scrNumObjectsInArea(void)
{
  ...
  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrNumObjectsInArea: invalid player number");
    return FALSE;
  }

  count = numObjectsInArea((BASE_OBJECT*)apsDroidLists[player], x1, y1, x2, y2) + numObjectsInArea(
    (BASE_OBJECT*)apsStructLists[player], x1, y1, x2, y2);

  if (!stackPushResult(VAL_INT, count)) { return FALSE; }
...
BOOL scrNumDroidsInArea(void)
{
  ...
  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrNumUnitInArea: invalid player number");
    return FALSE;
  }
...
BOOL scrNumStructsInArea(void)
{
  ...
  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrNumStructsInArea: invalid player number");
    return FALSE;
  }
...
BOOL scrNumStructsButNotWallsInArea(void)
{
  ...
  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrNumStructsButNotWallsInArea: invalid player number");
    return FALSE;
  }
...
BOOL scrNumStructsByTypeInArea(void)
{
  ...
  if (!stackPopParams(6, VAL_INT, &player, VAL_INT, &type, VAL_INT, &x1, VAL_INT, &y1, VAL_INT, &x2, VAL_INT, &y2)) { return FALSE; }

  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrNumStructsByTypeInArea: invalid player number");
    return FALSE;
  }
  ...
  for (psStruct = apsStructLists[player]; psStruct; psStruct = psStruct->psNext)
  {
    if ((psStruct->pStructureType->type == (UDWORD)type) && (psStruct->status == SS_BUILT))
    {
      ox = (SDWORD)psStruct->x;
      oy = (SDWORD)psStruct->y;
      if ((ox >= x1) && (ox <= x2) && (oy >= y1) && (oy <= y2)) { count += 1; }
...
BOOL scrDroidHasSeen(void)
{
  ...
  if (!stackPopParams(2, ST_BASEOBJECT, &psObj, VAL_INT, &player)) { return FALSE; }

  if (psObj == NULL)
  {
    ASSERT_TEXT(FALSE, "scrUnitHasSeen: NULL object");
    return FALSE;
  }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrUnitHasSeen:player number is too high");
    return FALSE;
  }

  // See if any droid has seen this object
  seen = FALSE;
  if (psObj->visible[player]) { seen = TRUE; }

  if (!stackPushResult(VAL_BOOL, (UDWORD)seen)) { return FALSE; }
...
BOOL scrDroidInRangeOfPosition(void)
{
  ...
  if (!stackPopParams(5, VAL_INT, &range, VAL_INT, &player, VAL_INT, &iX, VAL_INT, &iY, VAL_INT, &iZ)) { return FALSE; }

  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrUnitInRangeOfPosition: invalid player number");
    return FALSE;
  }
  ...
  if (!stackPushResult(VAL_BOOL, (UDWORD)found)) { return FALSE; }
...
BOOL scrEnableComponent(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &player)) { return FALSE; }
  if (!stackPop(&sVal)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrEnableComponent:player number is too high");
    return FALSE;
  }
  ...
  case ST_BRAIN:
    ...
    break;
  default: ASSERT_TEXT(FALSE, "scrEnableComponent: unknown type");
    return FALSE;
  }
...
BOOL scrMakeComponentAvailable(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrMakeComponentAvailable:player number is too high");
    return FALSE;
  }
...
BOOL scrAddDroidToMissionList(void)
{
  ...
  if (!stackPopParams(2, ST_TEMPLATE, &psTemplate, VAL_INT, &player)) { return FALSE; }

  /*	if ((UBYTE)player == selectedPlayer )
    {
      ASSERT_TEXT( FALSE, "scrAddDroidToMissionList: can't add own player to list" );
      return FALSE;
    }*/

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrAddUnitToMissionList:player number is too high");
    return FALSE;
  }

  ASSERT_TEXT(PTRVALID(psTemplate, sizeof(DROID_TEMPLATE)), "scrAddUnitToMissionList: Invalid template pointer");
  ...
  if (!stackPushResult((INTERP_TYPE)ST_DROID, (SDWORD)psDroid)) { return FALSE; }
...
BOOL scrAddDroid(void)
{
  ...
  if (!stackPopParams(4, ST_TEMPLATE, &psTemplate, VAL_INT, &x, VAL_INT, &y, VAL_INT, &player)) { return FALSE; }
  /*	if (!stackPop(&sVal))
    {
      return FALSE;
    }
    ...
    {
      ASSERT_TEXT(FALSE, "scrAddDroid: type mismatch for object");
      return FALSE;
    }
  ...
  */
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrAddUnit:player number is too high");
    return FALSE;
  }

  ASSERT_TEXT(PTRVALID(psTemplate, sizeof(DROID_TEMPLATE)), "scrAddUnit: Invalid template pointer");
...
BOOL scrAddDroidToTransporter(void)
{
  ...
  if (!stackPopParams(2, ST_DROID, &psTransporter, ST_DROID, &psDroid)) { return FALSE; }

  if (psTransporter == NULL || psDroid == NULL)
  {
    //ignore!
    ASSERT_TEXT(FALSE, "scrAddUnitToTransporter: null unit passed");
    return TRUE;
  }

  ASSERT_TEXT(PTRVALID(psTransporter, sizeof(DROID)), "scrAddUnitToTransporter: invalid transporter pointer");
  ASSERT_TEXT(PTRVALID(psDroid, sizeof(DROID)), "scrAddUnitToTransporter: invalid unit pointer");
  ASSERT_TEXT(psTransporter->droidType == DROID_TRANSPORTER, "scrAddUnitToTransporter: invalid transporter type");

  /* check for space */
  if (checkTransporterSpace(psTransporter, psDroid))
  {
    if (droidRemove(psDroid, mission.apsDroidLists)) { grpJoin(psTransporter->psGroup, psDroid); }
  }
...
BOOL scrBuildingDestroyed(void)
{
  ...
  if (!stackPopParams(2, ST_STRUCTUREID, &structureID, VAL_INT, &player)) { return FALSE; }
  /*	if (!stackPop(&sVal))
    {
      return FALSE;
    }
    ...
    {
      ASSERT_TEXT(FALSE, "scrBuildingDestroyed: type mismatch for object");
      return FALSE;
    }
  ...
  */
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrBuildingDestroyed:player number is too high");
    return FALSE;
  }

  destroyed = TRUE;
  //look thru the players list to see if the structure still exists
  for (psCurr = apsStructLists[player]; psCurr != NULL; psCurr = psCurr->psNext) { if (psCurr->id == structureID) { destroyed = FALSE; } }

  if (!stackPushResult(VAL_BOOL, (UDWORD)destroyed)) { return FALSE; }
...
BOOL scrEnableStructure(void)
{
  ...
  if (!stackPopParams(2, ST_STRUCTURESTAT, &index, VAL_INT, &player)) { return FALSE; }
  /*	if (!stackPop(&sVal))
    {
      return FALSE;
    }
    ...
    {
      ASSERT_TEXT(FALSE, "scrEnableStructure: type mismatch for object");
      return FALSE;
    }*/
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrEnableStructure:player number is too high");
    return FALSE;
  }

  if (index < (SDWORD)0 || index > (SDWORD)numStructureStats)
  {
    ASSERT_TEXT(FALSE, "scrEnableStructure:invalid structure stat");
    return FALSE;
  }
...
BOOL scrIsStructureAvailable(void)
{
  ...
  if (!stackPopParams(2, ST_STRUCTURESTAT, &index, VAL_INT, &player)) { return FALSE; }
  if (apStructTypeLists[player][index] == AVAILABLE) { result = TRUE; }
  else { result = FALSE; }

  if (!stackPushResult(VAL_BOOL, result)) { return FALSE; }
  return TRUE;
}
...
BOOL scrSelectDroidByID(void)
{
  ...
  if (!stackPopParams(2, ST_DROIDID, &droidID, VAL_INT, &player)) { return FALSE; }
  /*	if (!stackPop(&sVal))
    {
      return FALSE;
    }
    ...
    {
      ASSERT_TEXT(FALSE, "scrSelectDroidByID: type mismatch for object");
      return FALSE;
    }
  */
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSelectUnitByID:player number is too high");
    return FALSE;
  }

  selected = FALSE;
  if (selectDroidByID(droidID, player)) { selected = TRUE; }

  //store the reult cos might need to check the droid exists before doing anything else
  if (!stackPushResult(VAL_BOOL, (UDWORD)selected)) { return FALSE; }
  return TRUE;
}
...
BOOL scrNumMB(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &val)) { return FALSE; }
...
BOOL scrApproxRoot(void)
{
  ...
  if (!stackPopParams(2, VAL_INT, &val1, VAL_INT, &val2)) { return FALSE; }

  if (val1 < val2) { val1 = val2 + (val1 >> 1); }
  else { val1 = val1 + (val2 >> 1); }

  if (!stackPushResult(VAL_INT, val1)) { return FALSE; }
  return TRUE;
}
...
BOOL scrAddReticuleButton(void)
{
  ...
  case IDRET_CANCEL:
    ...
    break;
  default: ASSERT_TEXT(FALSE, "scrAddReticuleButton: Invalid reticule Button ID");
    return FALSE;
  }
  return TRUE;
}
...
BOOL scrRemoveReticuleButton(void)
{
  ...
  if (!stackPopParams(2, VAL_INT, &val, VAL_BOOL, &bReset)) { return FALSE; }
  if (bInTutorial)
  {
...
BOOL scrAddMessage(void)
{
  ...
  if (!stackPopParams(4, ST_INTMESSAGE, &psViewData, VAL_INT, &msgType, VAL_INT, &player, VAL_BOOL, &playImmediate)) { return FALSE; }
  /*
    if (!stackPop(&sVal))
    {
      return FALSE;
    ...
    {
      ASSERT_TEXT(FALSE, "scrAddMessage: type mismatch for object");
      return FALSE;
    }
  */
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrAddMessage:player number is too high");
    return FALSE;
  }
  ...
  if (psMessage)
  {
    ...
    if (msgType == MSG_PROXIMITY)
    {
      ...
      if (((VIEW_PROXIMITY*)psViewData->pData)->z < height) { ((VIEW_PROXIMITY*)psViewData->pData)->z = height; }
    }
...
BOOL scrRemoveMessage(void)
{
  ...
  if (!stackPopParams(3, ST_INTMESSAGE, &psViewData, VAL_INT, &msgType, VAL_INT, &player)) { return FALSE; }
  ...
  {
    ASSERT_TEXT(FALSE, "scrRemoveMessage:cannot find message - %s", psViewData->pName);
    return FALSE;
  }
...
/*BOOL scrAddTutorialMessage(void)
{
	...
	if (player >= MAX_PLAYERS)
	{
		ASSERT_TEXT(FALSE, "scrAddTutorialMessage:player number is too high");
		return FALSE;
	}
...
BOOL scrBuildDroid(void)
{
  ...
  if (!stackPopParams(4, ST_TEMPLATE, &psTemplate, ST_STRUCTURE, &psFactory, VAL_INT, &player, VAL_INT, &productionRun)) { return FALSE; }

  if (psFactory == NULL)
  {
    ASSERT_TEXT(FALSE, "scrBuildUnit: NULL factory object");
    return FALSE;
  }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrBuildUnit:player number is too high");
    return FALSE;
  }

  if (productionRun > UBYTE_MAX)
  {
    ASSERT_TEXT(FALSE, "scrBuildUnit: production run too high");
    return FALSE;
  }

  ASSERT_TEXT(PTRVALID(psFactory, sizeof(STRUCTURE)), "scrBuildUnit: Invalid structure pointer");
  ASSERT_TEXT((psFactory->pStructureType->type == REF_FACTORY ||
      psFactory->pStructureType->type == REF_CYBORG_FACTORY || psFactory->pStructureType->type == REF_VTOL_FACTORY),
    "scrBuildUnit: structure is !a factory");
  ASSERT_TEXT(PTRVALID(psTemplate, sizeof(DROID_TEMPLATE)), "scrBuildUnit: Invalid template pointer");

  //check building the right sort of droid for the factory
  if (!validTemplateForFactory(psTemplate, psFactory))
  {
    ASSERT_TEXT(FALSE, "scrBuildUnit: invalid template - %s for factory - %s", psTemplate->aName, psFactory->pStructureType->pName);
    return FALSE;
  }
...
BOOL scrSetAssemblyPoint(void)
{
  ...
  if (!stackPopParams(3, ST_STRUCTURE, &psBuilding, VAL_INT, &x, VAL_INT, &y)) { return FALSE; }

  if (psBuilding == NULL)
  {
    ASSERT_TEXT(FALSE, "scrSetAssemblyPoint: NULL structure");
    return FALSE;
  }

  if (psBuilding->pStructureType->type != REF_FACTORY && psBuilding->pStructureType->type != REF_CYBORG_FACTORY && psBuilding->
    pStructureType->type != REF_VTOL_FACTORY)
  {
    ASSERT_TEXT(FALSE, "scrSetAssemblyPoint: structure is !a factory");
    return FALSE;
  }

  setAssemblyPoint(((FACTORY*)psBuilding->pFunctionality)->psAssemblyPoint, x, y, psBuilding->player, TRUE);
...
BOOL scrStructureIdle(void)
{
  ...
  if (!stackPopParams(1, ST_STRUCTURE, &psBuilding)) { return FALSE; }
  //	DBPRINTF(("scrStructureIdle called\n"));

  if (psBuilding == NULL)
  {
    ASSERT_TEXT(FALSE, "scrStructureIdle: NULL structure");
    return FALSE;
  }

  //test for idle
  idle = FALSE;
  if (structureIdle(psBuilding)) { idle = TRUE; }

  //	DBPRINTF(("structure %p is %d\n",psBuilding,idle));

  if (!stackPushResult(VAL_BOOL, (UDWORD)idle)) { return FALSE; }
...
BOOL scrAttackLocation(void)
{
  ...
  if (!stackPopParams(3, VAL_INT, &x, VAL_INT, &y, VAL_INT, &player)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrAttackLocation:player number is too high");
    return FALSE;
  }
...
BOOL scrDestroyFeature(void)
{
  ...
  if (!stackPopParams(1, ST_FEATURE, &psFeature)) { return FALSE; }

  if (psFeature == NULL) { ASSERT_TEXT(PTRVALID(psFeature, sizeof(FEATURE)), "scrDestroyFeature: Invalid feature pointer"); }
...
BOOL scrInitGetFeature(void)
{
  ...
  if (!stackPopParams(3, ST_FEATURESTAT, &iFeat, VAL_INT, &player, VAL_INT, &bucket)) { return FALSE; }
...
BOOL scrGetFeature(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &bucket)) { return FALSE; }

  count = 0;
  // go to the correct start point in the feature list.
  for (psFeat = apsFeatureLists[0]; psFeat && count < getFeatureCount[bucket]; count++) { psFeat = psFeat->psNext; }

  if (psFeat == NULL) // no more to find.
  {
    if (!stackPushResult((INTERP_TYPE)ST_FEATURE, (SDWORD)NULL)) { return FALSE; }
    return TRUE;
  }
  ...
  while (psFeat)
  {
    if ((psFeat->psStats->subType == psFeatureStatToFind[bucket]->subType) && (psFeat->visible[playerToEnum[bucket]] != 0) && !
  ...
  // none found
  if (!stackPushResult((INTERP_TYPE)ST_FEATURE, (UDWORD)NULL)) { return FALSE; }
}
...
BOOL scrGetFeature(void)
{
	...
	while(psCurrEnumFeature[bucket])
	{
		...
		{
			if (!stackPushResult((INTERP_TYPE)ST_FEATURE,(UDWORD) psCurrEnumFeature[bucket]))			//	push result
			{
				return FALSE;
			}
...
BOOL scrAddFeature(void)
{
  ...
  if (!stackPopParams(3, ST_FEATURESTAT, &iFeat, VAL_INT, &iX, VAL_INT, &iY)) { return FALSE; }

  psStat = (FEATURE_STATS*)(asFeatureStats + iFeat);

  ASSERT_TEXT(PTRVALID(psStat, sizeof(FEATURE_STATS)), "scrAddFeature: Invalid feature pointer");
  ...
  if (psStat != NULL)
  {
    ...
    /* check for wrecked feature already on-tile && remove */
    for (psFeat = apsFeatureLists[0]; psFeat; psFeat = psFeat->psNext)
    {
      ...
      if ((iTestX == iMapX) && (iTestY == iMapY))
      {
        ...
        else { ASSERT_TEXT(FALSE, "scrAddFeature: building feature on tile already occupied\n"); }
      }
    }
  ...
  if (!stackPushResult((INTERP_TYPE)ST_FEATURE, (UDWORD)psFeat)) { return FALSE; }
...
BOOL scrAddStructure(void)
{
  ...
  if (!stackPopParams(4, ST_STRUCTURESTAT, &iStruct, VAL_INT, &iPlayer, VAL_INT, &iX, VAL_INT, &iY)) { return FALSE; }

  psStat = (STRUCTURE_STATS*)(asStructureStats + iStruct);

  ASSERT_TEXT(PTRVALID(psStat, sizeof(STRUCTURE_STATS)), "scrAddStructure: Invalid feature pointer");
  ...
  if (psStat != NULL)
  {
    ...
    if (TILE_HAS_STRUCTURE(mapTile(iMapX,iMapY))) { ASSERT_TEXT(FALSE, "scrAddStructure: tile already occupied by structure\n"); }
    ...
    if (psStruct != NULL)
    {
      ...
      && you end up in an infinite loop) we don't need to do this here 
...
for ( iW=iMapX; iW<=iMapX+(SDWORD)psStat->baseWidth; iW+=iWidth )
{
  ...
  if (!stackPushResult((INTERP_TYPE)ST_STRUCTURE, (UDWORD)psStruct)) { return FALSE; }
...
BOOL scrDestroyStructure(void)
{
  ...
  if (!stackPopParams(1, ST_STRUCTURE, &psStruct)) { return FALSE; }

  if (psStruct == NULL) { ASSERT_TEXT(PTRVALID(psStruct, sizeof(STRUCTURE)), "scrDestroyStructure: Invalid structure pointer"); }
...
BOOL scrInitEnumStruct(void)
{
  ...
  if (!stackPopParams(4, VAL_BOOL, &any, ST_STRUCTURESTAT, &iStat, VAL_INT, &player, VAL_INT, &targetplayer)) { return FALSE; }

  if (any == 1) { structfindany = TRUE; }
  else { structfindany = FALSE; }
...
BOOL scrEnumStruct(void)
{
  ...
  count = 0;
  for (psStruct = apsStructLists[playerToEnumStruct]; psStruct && count < enumStructCount; count++) { psStruct = psStruct->psNext; }

  if (psStruct == NULL) // no more to find.
  {
    if (!stackPushResult((INTERP_TYPE)ST_STRUCTURE, (SDWORD)NULL)) { return FALSE; }
    return TRUE;
  }
  ...
  // push NULL, none found;
  if (!stackPushResult((INTERP_TYPE)ST_STRUCTURE, (UDWORD)NULL)) { return FALSE; }
  return TRUE;
}

// -----------------------------------------------------------------------------------------
/*looks to see if a structure (specified by type) exists && is being built*/
BOOL scrStructureBeingBuilt(void)
{
  ...
  if (!stackPopParams(2, ST_STRUCTURESTAT, &structInc, VAL_INT, &player)) { return FALSE; }
  ...
  /*	if (!stackPop(&sVal))
    ...
    if (sVal.type != ST_STRUCTURESTAT)
    {
      ASSERT_TEXT(FALSE, "scrStructureBeingBuilt: type mismatch for object");
      return FALSE;
    }
  ...
  */
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrStructureBeingBuilt:player number is too high");
    return FALSE;
  }

  psStats = (STRUCTURE_STATS*)(asStructureStats + structInc);
  beingBuilt = FALSE;
  if (checkStructureStatus(psStats, player, SS_BEING_BUILT)) { beingBuilt = TRUE; }

  if (!stackPushResult(VAL_BOOL, (UDWORD)beingBuilt)) { return FALSE; }
...
BOOL scrStructureComplete(void)
{
  ...
  BOOL result;
  if (!stackPopParams(1, ST_STRUCTURE, &psStruct)) { return FALSE; }
  if (psStruct->status == SS_BUILT) { result = TRUE; }
  else { result = FALSE; }
  if (!stackPushResult(VAL_BOOL, result)) { return FALSE; }
...
BOOL scrStructureBuilt(void)
{
  ...
  /*	if (!stackPop(&sVal))
    ...
    if (sVal.type != ST_STRUCTURESTAT)
    {
      ASSERT_TEXT(FALSE, "scrStructureBuilt: type mismatch for object");
      return FALSE;
    }
    psStats = (STRUCTURE_STATS *)(asStructureStats + sVal.v.ival);
  */
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrStructureBuilt:player number is too high");
    return FALSE;
  }
  ...
  built = FALSE;
  if (checkStructureStatus(psStats, player, SS_BUILT)) { built = TRUE; }
  if (!stackPushResult(VAL_BOOL, (UDWORD)built)) { return FALSE; }
  return TRUE;
}

// -----------------------------------------------------------------------------------------
/*centre the view on an object - can be droid/structure || feature */
BOOL scrCentreView(void)
{
  BASE_OBJECT* psObj;
  //	INTERP_VAL	sVal;

  if (!stackPopParams(1, ST_BASEOBJECT, &psObj)) { return FALSE; }

  if (psObj == NULL)
  {
    ASSERT_TEXT(FALSE, "scrCentreView: NULL object");
    return FALSE;
  }
...
BOOL scrCentreViewPos(void)
{
  ...
  if (!stackPopParams(2, VAL_INT, &x, VAL_INT, &y)) { return FALSE; }

  if ((x < 0) || (x >= (SDWORD)mapWidth * TILE_UNITS) || (y < 0) || (y >= (SDWORD)mapHeight * TILE_UNITS))
  {
    ASSERT_TEXT(FALSE, "scrCenterViewPos: coords off map");
    return FALSE;
  }
...
BOOL scrGetStructure(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrGetStructure:player number is too high");
    return FALSE;
  }
...
BOOL scrGetTemplate(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrGetTemplate:player number is too high");
    return FALSE;
  }
  ...
  found = FALSE;
  for (psTemplate = apsDroidTemplates[player]; psTemplate != NULL; psTemplate = psTemplate->psNext)
  {
    switch (sVal.type)
    {
    case ST_BODY:
      if (psTemplate->asParts[COMP_BODY] == sVal.v.ival) { found = TRUE; }
      break;
    case ST_PROPULSION:
      if (psTemplate->asParts[COMP_PROPULSION] == sVal.v.ival) { found = TRUE; }
      break;
    case ST_ECM:
      if (psTemplate->asParts[COMP_ECM] == sVal.v.ival) { found = TRUE; }
      break;
    case ST_SENSOR:
      if (psTemplate->asParts[COMP_SENSOR] == sVal.v.ival) { found = TRUE; }
      break;
    case ST_CONSTRUCT:
      if (psTemplate->asParts[COMP_CONSTRUCT] == sVal.v.ival) { found = TRUE; }
      break;
    case ST_REPAIR:
      if (psTemplate->asParts[COMP_REPAIRUNIT] == sVal.v.ival) { found = TRUE; }
      break;
    case ST_WEAPON:
      ...
      break;
    default: ASSERT_TEXT(FALSE, "scrGetTemplate: unknown type");
      return FALSE;
    }
  ...
  //make sure pass NULL back if not got one
  if (!found) { psTemplate = NULL; }

  if (!stackPushResult((INTERP_TYPE)ST_TEMPLATE, (UDWORD)psTemplate)) { return FALSE; }
...
BOOL scrGetDroid(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrGetUnit:player number is too high");
    return FALSE;
  }
  ...
  for (psDroid = apsDroidLists[player]; psDroid != NULL; psDroid = psDroid->psNext)
  {
    ...
    {
    case ST_BODY:
      if (psDroid->asBits[COMP_BODY].nStat == (UDWORD)sVal.v.ival) { found = TRUE; }
      break;
    case ST_PROPULSION:
      if (psDroid->asBits[COMP_PROPULSION].nStat == (UDWORD)sVal.v.ival) { found = TRUE; }
      break;
    case ST_ECM:
      if (psDroid->asBits[COMP_ECM].nStat == (UDWORD)sVal.v.ival) { found = TRUE; }
      break;
    case ST_SENSOR:
      if (psDroid->asBits[COMP_SENSOR].nStat == (UDWORD)sVal.v.ival) { found = TRUE; }
      break;
    case ST_CONSTRUCT:
      if (psDroid->asBits[COMP_CONSTRUCT].nStat == (UDWORD)sVal.v.ival) { found = TRUE; }
      break;
    case ST_REPAIR:
      if (psDroid->asBits[COMP_REPAIRUNIT].nStat == (UDWORD)sVal.v.ival) { found = TRUE; }
      break;
    case ST_WEAPON:
      for (i = 0; i < DROID_MAXWEAPS; i++)
      {
      ...
      break;
    default: ASSERT_TEXT(FALSE, "scrGetUnit: unknown type");
      return FALSE;
    }
  ...
  //make sure pass NULL back if not got one
  if (!found) { psDroid = NULL; }

  if (!stackPushResult((INTERP_TYPE)ST_DROID, (UDWORD)psDroid)) { return FALSE; }
...
BOOL scrSetScrollParams(void)
{
  ...
  if (!stackPopParams(4, VAL_INT, &minX, VAL_INT, &minY, VAL_INT, &maxX, VAL_INT, &maxY)) { return FALSE; }

  //check the values entered are valid
  if (minX < 0)
  {
    ASSERT_TEXT(FALSE, "Minimum scroll x value is less than zero - ", minX);
    return FALSE;
  }
  if (minY < 0) { ASSERT_TEXT(FALSE, "Minimum scroll y value is less than zero - ", minY); }
  if (maxX > (SDWORD)mapWidth) { ASSERT_TEXT(FALSE, "Maximum scroll x value is greater than mapWidth - ", maxX); }
  if (maxX < (SDWORD)(visibleXTiles + 1)) { ASSERT_TEXT(FALSE, "Maximum scroll x has to be bigger than Visible Width(22) - ", maxX); }
  if (maxY > (SDWORD)mapHeight) { ASSERT_TEXT(FALSE, "Maximum scroll y value is greater than mapWidth - ", maxY); }
  if (maxY < (SDWORD)(visibleYTiles + 1)) { ASSERT_TEXT(FALSE, "Maximum scroll y has to be bigger than Visible Height(22) - ", maxY); }
  ...
  initLighting(prevMinX < scrollMinX ? prevMinX : scrollMinX, prevMinY < scrollMinY ? prevMinY : scrollMinY,
               prevMaxX < scrollMaxX ? prevMaxX : scrollMaxX, prevMaxY < scrollMaxY ? prevMaxY : scrollMaxY);
...
BOOL scrSetScrollMinX(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &minX)) { return FALSE; }
  ...
  //when the scroll limits change midgame - need to redo the lighting
  //initLighting(scrollMinX, scrollMinY, scrollMaxX, scrollMaxY);
  initLighting(prevMinX < scrollMinX ? prevMinX : scrollMinX, scrollMinY, scrollMaxX, scrollMaxY);
...
BOOL scrSetScrollMinY(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &minY)) { return FALSE; }

  //check the value entered are valid
  if (minY < 0)
  {
    ASSERT_TEXT(FALSE, "Minimum scroll y value is less than zero - ", minY);
    return FALSE;
  }
  ...
  //when the scroll limits change midgame - need to redo the lighting
  //initLighting(scrollMinX, scrollMinY, scrollMaxX, scrollMaxY);
  initLighting(scrollMinX, prevMinY < scrollMinY ? prevMinY : scrollMinY, scrollMaxX, scrollMaxY);
...
BOOL scrSetScrollMaxX(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &maxX)) { return FALSE; }

  //check the value entered are valid
  if (maxX > (SDWORD)mapWidth)
  {
    ASSERT_TEXT(FALSE, "Maximum scroll x value is greater than mapWidth - ", maxX);
    return FALSE;
  }
  ...
  //when the scroll limits change midgame - need to redo the lighting
  //initLighting(scrollMinX, scrollMinY, scrollMaxX, scrollMaxY);
  initLighting(scrollMinX, scrollMinY, prevMaxX < scrollMaxX ? prevMaxX : scrollMaxX, scrollMaxY);
...
BOOL scrSetScrollMaxY(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &maxY)) { return FALSE; }

  //check the value entered are valid
  if (maxY > (SDWORD)mapHeight)
  {
    ASSERT_TEXT(FALSE, "Maximum scroll y value is greater than mapWidth - ", maxY);
    return FALSE;
  }
  ...
  //when the scroll limits change midgame - need to redo the lighting
  //initLighting(scrollMinX, scrollMinY, scrollMaxX, scrollMaxY);
  initLighting(scrollMinX, scrollMinY, scrollMaxX, prevMaxY < scrollMaxY ? prevMaxY : scrollMaxY);
...
BOOL scrSetDefaultSensor(void)
{
  ...
  if (!stackPopParams(2, ST_SENSOR, &sensorInc, VAL_INT, &player)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSetDefaultSensor:player number is too high");
    return FALSE;
  }
  ...
  {
    ASSERT_TEXT(FALSE, "scrSetDefaultSensor: Sensor Inc is too high - %d", sensorInc);
    return FALSE;
  }
  ...
  {
    ASSERT_TEXT(FALSE, "scrSetDefaultSensor: This sensor is !a default one - %s", getStatName(&asSensorStats[sensorInc]) );
    return FALSE;
  }
...
BOOL scrSetDefaultECM(void)
{
  ...
  if (!stackPopParams(2, ST_ECM, &ecmInc, VAL_INT, &player)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSetDefaultECM:player number is too high");
    return FALSE;
  }
  ...
  {
    ASSERT_TEXT(FALSE, "scrSetDefaultECM: ECM Inc is too high - %d", ecmInc);
    return FALSE;
  }
  ...
  {
    ASSERT_TEXT(FALSE, "scrSetDefaultECM: This ecm is !a default one - %s", getStatName(&asECMStats[ecmInc]));
    return FALSE;
  }
...
BOOL scrSetDefaultRepair(void)
{
  ...
  if (!stackPopParams(2, ST_REPAIR, &repairInc, VAL_INT, &player)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSetDefaultRepair:player number is too high");
    return FALSE;
  }
  ...
  {
    ASSERT_TEXT(FALSE, "scrSetDefaultRepair: Repair Inc is too high - %d", repairInc);
    return FALSE;
  }
  ...
  {
    ASSERT_TEXT(FALSE, "scrSetDefaultRepair: This repair is !a default one - %s", getStatName(&asRepairStats[repairInc]));
    return FALSE;
  }
...
BOOL scrSetStructureLimits(void)
{
  ...
  if (!stackPopParams(3, ST_STRUCTURESTAT, &structInc, VAL_INT, &limit, VAL_INT, &player)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSetStructureLimits:player number is too high");
    return FALSE;
  }

  if (structInc > numStructureStats)
  {
    ASSERT_TEXT(FALSE, "scrSetStructureLimits: Structure stat is too high - %d", structInc);
    return FALSE;
  }

  if (limit < 0)
  {
    ASSERT_TEXT(FALSE, "scrSetStructureLimits: limit is less than zero - %d", limit);
    return FALSE;
  }

  if (limit > LOTS_OF)
  {
    ASSERT_TEXT(FALSE, "scrSetStructureLimits: limit is too high - %d - must be less than %d", limit, LOTS_OF);
    return FALSE;
  }
...
BOOL scrPlaySound(void)
{
  ...
  if (!stackPopParams(2, ST_SOUND, &soundID, VAL_INT, &player)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrPlaySound:player number is too high");
    return FALSE;
  }
  ...
  if (player == (SDWORD)selectedPlayer)
  {
    ...
    if (bInTutorial) { audio_QueueTrack(ID_SOUND_OF_SILENCE); }
  }
  return TRUE;
}
...
BOOL scrPlaySoundPos(void)
{
  ...
  if (!stackPopParams(5, ST_SOUND, &soundID, VAL_INT, &player, VAL_INT, &iX, VAL_INT, &iY, VAL_INT, &iZ)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrPlaySoundPos:player number is too high");
    return FALSE;
  }

  if (player == (SDWORD)selectedPlayer) { audio_QueueTrackPos(soundID, iX, iY, iZ); }
  return TRUE;
}
...
BOOL scrShowConsoleText(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrAddConsoleText:player number is too high");
    return FALSE;
  }
...
BOOL scrAnyDroidsLeft(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrAnyUnitsLeft:player number is too high");
    return FALSE;
  }

  //check the players list for any droid
  droidsLeft = TRUE;
  if (apsDroidLists[player] == NULL) { droidsLeft = FALSE; }

  if (!stackPushResult(VAL_BOOL, (UDWORD)droidsLeft)) { return FALSE; }
...
BOOL scrGameOverMessage(void)
{
  ...
  if (!stackPopParams(4, ST_INTMESSAGE, &psViewData, VAL_INT, &msgType, VAL_INT, &player, VAL_BOOL, &gameOver)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrGameOverMessage:player number is too high");
    return FALSE;
  }
  ...
  ASSERT_TEXT(msgType != MSG_PROXIMITY, "scrGameOverMessage: Bad message type (MSG_PROXIMITY)");
...
BOOL scrGameOver(void)
{
  ...
  if (!stackPopParams(1, VAL_BOOL, &gameOver)) { return FALSE; }
...
BOOL scrAnyFactoriesLeft(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrAnyFactorysLeft:player number is too high");
    return FALSE;
  }
  ...
  if (!stackPushResult(VAL_BOOL, (UDWORD)result)) { return FALSE; }
...
BOOL scrAnyStructButWallsLeft(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrAnyStructuresButWallsLeft:player number is too high");
    return FALSE;
  }

  //check the players list for any structures
  structuresLeft = TRUE;
  if (apsStructLists[player] == NULL) { structuresLeft = FALSE; }
  else
  {
    structuresLeft = FALSE;
    for (psCurr = apsStructLists[player]; psCurr != NULL; psCurr = psCurr->psNext)
    {
      if (psCurr->pStructureType->type != REF_WALL && psCurr->pStructureType->type != REF_WALLCORNER)
      {
        structuresLeft = TRUE;
        break;
      }
  ...
  if (!stackPushResult(VAL_BOOL, (UDWORD)structuresLeft)) { return FALSE; }
...
BOOL scrSetRetreatPoint(void)
{
  ...
  if (!stackPopParams(3, VAL_INT, &player, VAL_INT, &x, VAL_INT, &y)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSetRetreatPoint: player out of range");
    return FALSE;
  }
  if (x < 0 || x >= (SDWORD)mapWidth * TILE_UNITS || y < 0 || y >= (SDWORD)mapHeight * TILE_UNITS)
  {
    ASSERT_TEXT(FALSE, "scrSetRetreatPoint: coords off map");
    return FALSE;
  }
...
BOOL scrSetRetreatForce(void)
{
  ...
  if (!stackPopParams(2, VAL_INT, &player, VAL_INT, &level)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSetRetreatForce: player out of range");
    return FALSE;
  }

  if (level > 100 || level < 0)
  {
    ASSERT_TEXT(FALSE, "scrSetRetreatForce: level out of range");
    return FALSE;
  }

  // count up the current number of droids
  numDroids = 0;
  for (psCurr = apsDroidLists[player]; psCurr; psCurr = psCurr->psNext) { numDroids += 1; }
...
BOOL scrSetRetreatLeadership(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSetRetreatLeadership: player out of range");
    return FALSE;
  }

  if (level > 100 || level < 0)
  {
    ASSERT_TEXT(FALSE, "scrSetRetreatLeadership: level out of range");
    return FALSE;
  }
...
BOOL scrSetGroupRetreatPoint(void)
{
  ...
  if (!stackPopParams(3, ST_GROUP, &psGroup, VAL_INT, &x, VAL_INT, &y)) { return FALSE; }
...
BOOL scrSetGroupRetreatForce(void)
{
  ...
  if (!stackPopParams(2, ST_GROUP, &psGroup, VAL_INT, &level)) { return FALSE; }
  ...
  // count up the current number of droids
  numDroids = 0;
  for (psCurr = psGroup->psList; psCurr; psCurr = psCurr->psGrpNext) { numDroids += 1; }
...
BOOL scrSetRetreatHealth(void)
{
  ...
  if (!stackPopParams(2, VAL_INT, &player, VAL_INT, &health)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSetHealthForce: player out of range");
    return FALSE;
  }

  if (health > 100 || health < 0)
  {
    ASSERT_TEXT(FALSE, "scrSetHealthForce: health out of range");
    return FALSE;
  }
...
BOOL scrSetGroupRetreatHealth(void)
{
  ...
  if (!stackPopParams(2, ST_GROUP, &psGroup, VAL_INT, &health)) { return FALSE; }

  if (health > 100 || health < 0)
  {
    ASSERT_TEXT(FALSE, "scrSetGroupRetreatHealth: health out of range");
    return FALSE;
  }
...
BOOL scrStartMission(void)
{
  ...
  if (!stackPopParams(2, VAL_INT, &missionType, ST_LEVEL, &pGame)) { return FALSE; }

  //if (missionType > MISSION_NONE)
  if (missionType > LDS_NONE)
  {
    ASSERT_TEXT(FALSE, "Invalid Mission Type");
    return FALSE;
  }
  ...
  /*	if (!setUpMission(missionType))
    {
      ASSERT_TEXT(FALSE, "Unable to start mission - %s", pGame);
      return FALSE;
    }*/
...
BOOL scrSetSnow(void)
{
  ...
  if (!stackPopParams(1, VAL_BOOL, &bState)) { return FALSE; }

  if (bState) { atmosSetWeatherType(WT_SNOWING); }
...
BOOL scrSetRain(void)
{
  ...
  if (bState) { atmosSetWeatherType(WT_RAINING); }
  else { atmosSetWeatherType(WT_NONE); }
  return TRUE;
}
...
BOOL scrSetBackgroundFog(void)
{
  ...
  if (bState) //true, so go to false
  {
    ...
    if ((fogStatus == 0) && war_GetFog() && !(bMultiPlayer && game.fog)) { pie_EnableFog(TRUE); }
...
BOOL scrSetDepthFog(void)
{
  ...
  {
    //restart fog if it was off
    if ((fogStatus == 0) && war_GetFog()) { pie_EnableFog(TRUE); }
...
BOOL scrSetFogColour(void)
{
  ...
  if (!stackPopParams(3, VAL_INT, &red, VAL_INT, &green, VAL_INT, &blue)) { return FALSE; }
...
BOOL scrIsHumanPlayer(void)
{
  ...
  if (!stackPushResult(VAL_BOOL, isHumanPlayer(player))) { return FALSE; }
...
BOOL scrCreateAlliance(void)
{
  ...
  if (!stackPopParams(2, VAL_INT, &player1, VAL_INT, &player2)) { return FALSE; }

  if (player1 < 0 || player1 >= MAX_PLAYERS || player2 < 0 || player2 >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrCreateAlliance: player out of range");
    return FALSE;
  }

  if (bMultiPlayer) { if (game.alliance == NO_ALLIANCES || player1 >= game.maxPlayers || player2 >= game.maxPlayers) { return TRUE; } }
  ...
  /*
    ...
    if(bMultiPlayer) 
    {
      ...
      {
  #ifdef DEBUG
        CONPRINTF(ConsoleString,(ConsoleString,"%d && %d form an alliance.",player1,player2));
  #endif
...
BOOL scrBreakAlliance(void)
{
  ...
  if(bMultiPlayer)
    {
      ...
      {
        CONPRINTF(ConsoleString,(ConsoleString,"%d && %d break alliance.",player1,player2));
  ...
  */
  ...
  if (bMultiPlayer)
  {
    if (game.alliance == NO_ALLIANCES || player1 >= game.maxPlayers || player2 >= game.maxPlayers) { return TRUE; }
    breakAlliance(player1, player2,TRUE,TRUE);
  }
  else { breakAlliance(player1, player2,FALSE,TRUE); }
  /*
...
BOOL scrAllianceExists(void)
{
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    for (j = 0; j < MAX_PLAYERS; j++)
    {
      ...
      {
        if (!stackPushResult(VAL_BOOL, TRUE)) { return FALSE; }
        return TRUE;
      }
    }
  }

  if (!stackPushResult(VAL_BOOL, FALSE)) { return FALSE; }
...
BOOL scrAllianceExistsBetween(void)
{
  ...
  if (!stackPopParams(2, VAL_INT, &i, VAL_INT, &j)) { return FALSE; }
  if (alliances[i][j] == ALLIANCE_FORMED)
  {
    if (!stackPushResult(VAL_BOOL, TRUE)) { return FALSE; }
    return TRUE;
...
BOOL scrDominatingAlliance(void)
{
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    for (j = 0; j < MAX_PLAYERS; j++)
    {
      if (isHumanPlayer(j) && isHumanPlayer(i) && i != j && alliances[i][j] != ALLIANCE_FORMED)
      {
        if (!stackPushResult(VAL_BOOL, FALSE)) { return FALSE; }
        return TRUE;
      }
...
BOOL scrMyResponsibility(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &player)) { return FALSE; }
  if (myResponsibility(player)) { if (!stackPushResult(VAL_BOOL, TRUE)) { return FALSE; } }
  else { if (!stackPushResult(VAL_BOOL, FALSE)) { return FALSE; } }
...
BOOL scrStructureBuiltInRange(void)
{
  ...
  if (!stackPopParams(5, ST_STRUCTURESTAT, &index, VAL_INT, &x, VAL_INT, &y, VAL_INT, &range, VAL_INT, &player)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrStructureBuiltInRange:player number is too high");
    return FALSE;
  }

  if (x < (SDWORD)0 || (x >> TILE_SHIFT) > (SDWORD)mapWidth)
  {
    ASSERT_TEXT(FALSE, "scrStructureBuiltInRange : invalid X coord");
    return FALSE;
  }
  if (y < (SDWORD)0 || (y >> TILE_SHIFT) > (SDWORD)mapHeight)
  {
    ASSERT_TEXT(FALSE,"scrStructureBuiltInRange : invalid Y coord");
    return FALSE;
  }
  if (index < (SDWORD)0 || index > (SDWORD)numStructureStats)
  {
    ASSERT_TEXT(FALSE, "scrStructureBuiltInRange : Invalid structure stat");
    return FALSE;
  }
  if (range < (SDWORD)0)
  {
    ASSERT_TEXT(FALSE, "scrStructureBuiltInRange : Rnage is less than zero");
    return FALSE;
  }
  ...
  //make sure pass NULL back if not got one
  if (!found) { psCurr = NULL; }

  if (!stackPushResult((INTERP_TYPE)ST_STRUCTURE, (UDWORD)psCurr)) { return FALSE; }
...
BOOL scrRandom(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &range)) { return FALSE; }

  if (range == 0) { result = 0; }
  else if (range > 0) { result = rand() % range; }
  else { result = rand() % (-range); }

  if (!stackPushResult(VAL_INT, result)) { return FALSE; }
...
BOOL scrEnableResearch(void)
{
  ...
  if (!stackPopParams(2, ST_RESEARCH, &psResearch, VAL_INT, &player)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrEnableResearch:player number is too high");
    return FALSE;
  }

  if (!enableResearch(psResearch, player)) { return FALSE; }
  return TRUE;
}
...
BOOL scrCompleteResearch(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrCompleteResearch:player number is too high");
    return FALSE;
  }

  if (psResearch == NULL)
  {
    ASSERT_TEXT(FALSE, "scrCompleteResearch: no such research topic");
    return FALSE;
  }
  ...
  {
    ASSERT_TEXT(FALSE, "scrCompleteResearch: invalid research index");
    return FALSE;
  }

  researchResult(researchIndex, (UBYTE)player, FALSE);

  if (bMultiPlayer && (gameTime > 2)) { SendResearch((UBYTE)player, researchIndex); }
...
BOOL scrFlashOn(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &button)) { return FALSE; }
  ...
  if (widgGetFromID(psWScreen, button) != NULL) { widgSetButtonFlash(psWScreen, button); }
  return TRUE;
}
...
BOOL scrFlashOff(void)
{
  ...
  if (widgGetFromID(psWScreen, button) != NULL) { widgClearButtonFlash(psWScreen, button); }
  return TRUE;
}
...
BOOL scrSetPowerLevel(void)
{
  ...
  if (!stackPopParams(2, VAL_INT, &power, VAL_INT, &player)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSetPowerLevel:player number is too high");
    return FALSE;
  }
...
BOOL scrAddPower(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrAddPower:player number is too high");
    return FALSE;
  }
...
scrapped && replaced by setNoGoAreas, left in for compatibility*/
BOOL scrSetLandingZone(void)
{
  SDWORD x1, x2, y1, y2;

  if (!stackPopParams(4, VAL_INT, &x1, VAL_INT, &y1, VAL_INT, &x2, VAL_INT, &y2)) { return FALSE; }
  ...
  {
    ASSERT_TEXT(FALSE, "scrSetLandingZone: x1 is greater than max mapWidth");
    return FALSE;
  }
  //if (x2 > (SDWORD)mapWidth)
  if (x2 > (SDWORD)MAP_MAXWIDTH)
  {
    ASSERT_TEXT(FALSE, "scrSetLandingZone: x2 is greater than max mapWidth");
    return FALSE;
  }
  //if (y1 > (SDWORD)mapHeight)
  if (y1 > (SDWORD)MAP_MAXHEIGHT)
  {
    ASSERT_TEXT(FALSE, "scrSetLandingZone: y1 is greater than max mapHeight");
    return FALSE;
  }
  //if (y2 > (SDWORD)mapHeight)
  if (y2 > (SDWORD)MAP_MAXHEIGHT)
  {
    ASSERT_TEXT(FALSE, "scrSetLandingZone: y2 is greater than max mapHeight");
    return FALSE;
  }
  //check won't overflow!
  if (x1 > UBYTE_MAX || y1 > UBYTE_MAX || x2 > UBYTE_MAX || y2 > UBYTE_MAX)
  {
    ASSERT_TEXT(FALSE, "scrSetLandingZone: one coord is greater than %s", UBYTE_MAX);
    return FALSE;
  }
...
/*set the landing Zone position for the Limbo droids && adds the Limbo droids 
...
BOOL scrSetLimboLanding(void)
{
  ...
  if (x1 > (SDWORD)MAP_MAXWIDTH)
  {
    ASSERT_TEXT(FALSE, "scrSetLimboLanding: x1 is greater than max mapWidth");
    return FALSE;
  }
  //if (x2 > (SDWORD)mapWidth)
  if (x2 > (SDWORD)MAP_MAXWIDTH)
  {
    ASSERT_TEXT(FALSE, "scrSetLimboLanding: x2 is greater than max mapWidth");
    return FALSE;
  }
  //if (y1 > (SDWORD)mapHeight)
  if (y1 > (SDWORD)MAP_MAXHEIGHT)
  {
    ASSERT_TEXT(FALSE, "scrSetLimboLanding: y1 is greater than max mapHeight");
    return FALSE;
  }
  //if (y2 > (SDWORD)mapHeight)
  if (y2 > (SDWORD)MAP_MAXHEIGHT)
  {
    ASSERT_TEXT(FALSE, "scrSetLimboLanding: y2 is greater than max mapHeight");
    return FALSE;
  }
  //check won't overflow!
  if (x1 > UBYTE_MAX || y1 > UBYTE_MAX || x2 > UBYTE_MAX || y2 > UBYTE_MAX)
  {
    ASSERT_TEXT(FALSE, "scrSetLimboLanding: one coord is greater than %s", UBYTE_MAX);
    return FALSE;
  }
...
BOOL scrSetNoGoArea(void)
{
  ...
  if (!stackPopParams(5, VAL_INT, &x1, VAL_INT, &y1, VAL_INT, &x2, VAL_INT, &y2, VAL_INT, &area)) { return FALSE; }

  if (area == LIMBO_LANDING)
  {
    ASSERT_TEXT(FALSE, "scrSetNoGoArea: Cannot set the Limbo Landing area with this function");
    return FALSE;
  }
  ...
  if (x1 > (SDWORD)MAP_MAXWIDTH)
  {
    ASSERT_TEXT(FALSE, "scrSetNoGoArea: x1 is greater than max mapWidth");
    return FALSE;
  }
  //if (x2 > (SDWORD)mapWidth)
  if (x2 > (SDWORD)MAP_MAXWIDTH)
  {
    ASSERT_TEXT(FALSE, "scrSetNoGoArea: x2 is greater than max mapWidth");
    return FALSE;
  }
  //if (y1 > (SDWORD)mapHeight)
  if (y1 > (SDWORD)MAP_MAXHEIGHT)
  {
    ASSERT_TEXT(FALSE, "scrSetNoGoArea: y1 is greater than max mapHeight");
    return FALSE;
  }
  //if (y2 > (SDWORD)mapHeight)
  if (y2 > (SDWORD)MAP_MAXHEIGHT)
  {
    ASSERT_TEXT(FALSE, "scrSetNoGoArea: y2 is greater than max mapHeight");
    return FALSE;
  }
  //check won't overflow!
  if (x1 > UBYTE_MAX || y1 > UBYTE_MAX || x2 > UBYTE_MAX || y2 > UBYTE_MAX)
  {
    ASSERT_TEXT(FALSE, "scrSetNoGoArea: one coord is greater than %s", UBYTE_MAX);
    return FALSE;
  }

  if (area >= MAX_NOGO_AREAS)
  {
    ASSERT_TEXT(FALSE, "scrSetNoGoArea: max num of areas is %d", MAX_NOGO_AREAS);
    return FALSE;
  }
...
BOOL scrSetRadarZoom(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &level)) { return TRUE; }

  // MAX_RADARZOOM is different on PC and PSX
  if (level < 0 || level > 2)
  {
    ASSERT_TEXT(FALSE, "scrSetRadarZoom: zoom level out of range");
    return FALSE;
  }
...
BOOL scrSetMissionTime(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &time)) { return FALSE; }
  ...
  {
    ASSERT_TEXT(FALSE,"The mission timer cannot be set to more than 99!");
    time = -1;
  }
  //store the value
  mission.time = time;
...
BOOL scrMissionTimeRemaining(void)
{
  ...
  else { timeRemaining /= 100; }

  if (!stackPushResult(VAL_INT, timeRemaining)) { return (FALSE); }
  return (TRUE);
}
...
BOOL scrSetReinforcementTime(void)
{
  ...
  {
    ASSERT_TEXT(FALSE,"The transport timer cannot be set to more than 1 hour!");
    time = -1;
  }
  ...
  if (missionCanReEnforce()) { addTransporterTimerInterface(); }
  ...
  if (time < 0)
  {
    ...
    scripts set the time to -1 at the between stage if there are !going 
    to be reinforcements on the submap  */
    for (psDroid = apsDroidLists[selectedPlayer]; psDroid != NULL; psDroid = psDroid->psNext)
    {
      if (psDroid->droidType == DROID_TRANSPORTER) { break; }
    }
    //if not found a transporter, can remove the launch button
    if (psDroid == NULL) { intRemoveTransporterLaunch(); }
  }
...
BOOL scrSetAllStructureLimits(void)
{
  ...
  if (!stackPopParams(2, VAL_INT, &limit, VAL_INT, &player)) { return FALSE; }
...
BOOL scrDistanceTwoPts(void)
{
  ...
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Distance between two points - cannot get parameters");
    return (FALSE);
  }
  ...
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Distance between two points - cannot return result");
    return (FALSE);
  }
  return (TRUE);
}
...
BOOL scrLOSTwoBaseObjects(void)
{
  ...
  {
    ASSERT_TEXT(FALSE,"SCRIPT : scrLOSTwoBaseObjects - cannot get parameters");
    return (FALSE);
  }

  if (bWallsBlock) { retVal = visibleObjWallBlock(psSource, psDest); }
  else { retVal = visibleObject(psSource, psDest); }

  if (!stackPushResult(VAL_BOOL, retVal))
  {
    ASSERT_TEXT(FALSE,"SCRIPT : scrLOSTwoBaseObjects - cannot return result");
    return (FALSE);
  }
  return (TRUE);
}
...
BOOL scrDestroyStructuresInArea(void)
{
  ...
  {
    ASSERT_TEXT(FALSE,"SCRIPT : scrDestroyStructuresInArea - Cannot get parameters");
    return (FALSE);
  }

  if (player >= MAX_PLAYERS) { ASSERT_TEXT(FALSE,"Player number too high in scrDestroyStructuresInArea"); }
  ...
  for (psStructure = apsStructLists[player]; psStructure; psStructure = psNextS)
  {
    ...
    if (psStructure->pStructureType->type == typeRef)
    {
      ...
      {
        if (bVisible) { destroyStruct(psStructure); }
        else { removeStruct(psStructure, TRUE); }
      }
    }
  }
  ...
  if (bTakeFeatures)
  {
    for (psFeature = apsFeatureLists[0]; psFeature; psFeature = psNextF)
    {
...

      {
        if (sX >= x1 && sX <= x2 && sY >= y1 && sY <= y2)
        {
          if (bVisible) { destroyFeature(psFeature); }
...
BOOL scrThreatInArea(void)
{
  ...
  {
    ASSERT_TEXT(FALSE,"SCRIPT : scrThreatInArea - Cannot get parameters");
    return (FALSE);
  }

  if (playerLooking >= MAX_PLAYERS || playerTarget >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE,"Player number too high in scrThreatInArea");
    return (FALSE);
  }
  ...
  {
    if (psDroid->droidType != DROID_WEAPON && psDroid->droidType != DROID_PERSON && psDroid->droidType != DROID_CYBORG && psDroid->
      droidType != DROID_CYBORG_SUPER) { continue; }

    dX = psDroid->x;
    dY = psDroid->y;
    /* Do we care if the droid is visible || !*/
    if (bVisible ? psDroid->visible[playerLooking] : TRUE)
    {
      ...
      if (dX >= x1 && dX <= x2 && dY >= y1 && dY <= y2)
      {
        ...
        case SIZE_SUPER_HEAVY:
          ...
          break;
        default: ASSERT_TEXT(FALSE, "Weird droid size in threat assessment");
          break;
        }
      }
    }
  ...
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Cannot push result in scrThreatInArea");
    return (FALSE);
  }
  return (TRUE);
}
...
BOOL scrGetNearestGateway(void)
{
  ...
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Cannot get parameters for scrGetNearestGateway");
    return (FALSE);
  }

  if (x < 0 || x > (SDWORD)mapWidth || y < 0 || y > (SDWORD)mapHeight)
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Invalid coordinates in getNearestGateway");
    return (FALSE);
  }

  if (psGateways == NULL)
  {
    ASSERT_TEXT(FALSE,"SCRIPT : No gateways found in getNearestGatway");
    return (FALSE);
  }
  ...
  *rY = retY;
  ...
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Cannot return result for stackPushResult");
    return (FALSE);
  }
...
BOOL scrSetWaterTile(void)
{
  ...
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Cannot get parameter for scrSetWaterTile");
    return (FALSE);
  }

  if (tileNum > 96)
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Water tile number too high in scrSetWaterTile");
    return (FALSE);
  }
...
BOOL scrSetRubbleTile(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &tileNum))
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Cannot get parameter for scrSetRubbleTile");
    return (FALSE);
  }

  if (tileNum > 96)
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Rubble tile number too high in scrSetWaterTile");
    return (FALSE);
  }
...
BOOL scrSetCampaignNumber(void)
{
  ...
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Cannot get parameter for scrSetCampaignNumber");
    return (FALSE);
  }
...
// -----------------------------------------------------------------------------------------
BOOL scrGetUnitCount(void) { return TRUE; }
...
BOOL scrTestStructureModule(void)
{
  ...
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Cannot get parameters in scrTestStructureModule");
    return (FALSE);
  }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Player number too high in scrTestStructureModule");
    return (FALSE);
  }
  ...
  if (psStructure) { if (structHasModule(psStructure)) { bFound = TRUE; } }
  /* psStructure was NULL - so test the general case */
  else
  {
    // Search them all, but exit if we get one!!
    for (psStruct = apsStructLists[player], bFound = FALSE; psStruct && !bFound; psStruct = psStruct->psNext)
    {
      if (structHasModule(psStruct)) { bFound = TRUE; }
    }
  }
  ...
  {
    ASSERT_TEXT(FALSE,"SCRIPT : Cannot push result for scrTestStructureModule");
    return (FALSE);
  }
...
BOOL scrForceDamage(void)
{
  ...
  {
    ASSERT_TEXT(FALSE,"Cannot pop params for scrForceDamage");
    return (FALSE);
  }

  /* Got to be a percent, so must be less than || equal to 100 */
  if (damagePercent > 100)
  {
    ASSERT_TEXT(FALSE,"scrForceDamage : You're supposed to be passing in a PERCENTAGE VALUE, \
  ...
  case OBJ_FEATURE:
    ...
    break;
  default: ASSERT_TEXT(FALSE,"Unsupported base object type in scrForceDamage");
    return (FALSE);
    break;
  }
...
BOOL scrDestroyUnitsInArea(void)
{
  ...
  {
    ASSERT_TEXT(FALSE,"Cannot get params for scrDestroyUnitsInArea");
    return (FALSE);
  }

  if (player >= MAX_PLAYERS) { ASSERT_TEXT(FALSE,"Invalid player number in scrKillDroidsInArea"); }

  for (psDroid = apsDroidLists[player]; psDroid; psDroid = psNext)
  {
    psNext = psDroid->psNext; // get a copy cos pointer will be lost
    if ((psDroid->x > x1) && (psDroid->x < x2) && (psDroid->y > y1) && (psDroid->y < y2))
    {
...
BOOL scrRemoveDroid(void)
{
  ...
  {
    ASSERT_TEXT(FALSE,"Cannot get vars for scrRemoveDroid!");
    return (FALSE);
  }

  if (psDroid) { vanishDroid(psDroid); }
...
BOOL structHasModule(STRUCTURE* psStruct)
{
  ...
  if (psStruct->status != SS_BUILT) { return (FALSE); }
  ...
  if (psStruct == NULL)
  {
    ASSERT_TEXT(psStruct!=NULL,"structHasModule - Testing for a module from a NULL struct - huh!?");
    return (FALSE);
  }
  ...
  if (psStruct)
  {
    ...
    if (StructIsFactory(psStruct) || psStats->type == REF_POWER_GEN || psStats->type == REF_RESEARCH)
    {
      switch (psStats->type)
      {
      case REF_POWER_GEN:
        if (((POWER_GEN*)psStruct->pFunctionality)->capacity) { bFound = TRUE; }
        break;
      case REF_FACTORY:
      case REF_VTOL_FACTORY:
        if (((FACTORY*)psStruct->pFunctionality)->capacity) { bFound = TRUE; }
        break;
      case REF_RESEARCH:
        if (((RESEARCH_FACILITY*)psStruct->pFunctionality)->capacity) { bFound = TRUE; }
        break;
      default:
...
BOOL scrAddTemplate(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrAddTemplate:player number is too high");
    return FALSE;
  }

  ASSERT_TEXT(PTRVALID(psTemplate, sizeof(DROID_TEMPLATE)),"scrAddTemplate: Invalid template pointer");

  if (addTemplate(player, psTemplate)) { if (!stackPushResult(VAL_BOOL,TRUE)) { return FALSE; } }
  else { if (!stackPushResult(VAL_BOOL,FALSE)) { return FALSE; } }
  return TRUE;
}
...
BOOL scrPickStructLocation(void)
{
  ...
  if (!stackPopParams(4, ST_STRUCTURESTAT, &index, VAL_REF | VAL_INT, &pX, VAL_REF | VAL_INT, &pY, VAL_INT, &player)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrPickStructLocation:player number is too high");
    return FALSE;
  }

  // check for wacky coords.
  if (*pX < 0 || *pX > (SDWORD)(mapWidth << TILE_SHIFT) || *pY < 0 || *pY > (SDWORD)(mapHeight << TILE_SHIFT)) { goto failedstructloc; }
  ...
  for (incX = 1, incY = 1; incX < numIterations; incX++, incY++)
  {
    if (!found)
    {
      ...
      {
        if (validLocation((BASE_STATS*)psStat, x, y, player, FALSE) && structDoubleCheck((BASE_STATS*)psStat, x, y))
        {
          found = TRUE;
          break;
        }
...
BOOL scrSetTransporterExit(void)
{
  ...
  if (!stackPopParams(3, VAL_INT, &iPlayer, VAL_INT, &iExitTileX, VAL_INT, &iExitTileY)) { return FALSE; }
...
BOOL scrFlyTransporterIn(void)
{
  ...
  if (!stackPopParams(4, VAL_INT, &iPlayer, VAL_INT, &iEntryTileX, VAL_INT, &iEntryTileY, VAL_BOOL, &bTrackTransporter)) { return FALSE; }
 ...
 *        e.g. in the intelligence screen || desgin screen)
 *
 *  RETURNS:
 *
 */
BOOL scrGetGameStatus(void)
{
  ...
  if (!stackPopParams(1, VAL_INT, &GameChoice)) { return FALSE; }
  ...
  case STATUS_BattleMapViewEnabled:
    ...
    if (result == TRUE) { DBPRINTF(("battle map active")); }
    else { DBPRINTF(("battle map notactive")); }
  ...
  default: ASSERT_TEXT(FALSE,"ScrGetGameStatus. Invalid STATUS_ variable");
    break;
  }
...
BOOL scrGetPlayerColour(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrGetPlayerColour:player number is too high");
    return FALSE;
  }

  colour = (SDWORD)getPlayerColour(player);

  if (!stackPushResult(VAL_INT, colour)) { return FALSE; }
...
BOOL scrSetPlayerColour(void)
{
  ...
  if (!stackPopParams(2, VAL_INT, &colour, VAL_INT, &player)) { return FALSE; }

  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSetPlayerColour:player number is too high");
    return FALSE;
  }

  if (colour >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSetPlayerColour:colour number is too high");
    return FALSE;
  }
...
BOOL scrTakeOverDroidsInArea(void)
{
  ...
  {
    ASSERT_TEXT(FALSE, "scrTakeOverUnitsInArea:player number is too high");
    return FALSE;
  }

  if (x1 > (SDWORD)(MAP_MAXWIDTH << TILE_SHIFT))
  {
    ASSERT_TEXT(FALSE, "scrTakeOverUnitsInArea: x1 is greater than max mapWidth");
    return FALSE;
  }

  if (x2 > (SDWORD)(MAP_MAXWIDTH << TILE_SHIFT))
  {
    ASSERT_TEXT(FALSE, "scrTakeOverUnitsInArea: x2 is greater than max mapWidth");
    return FALSE;
  }

  if (y1 > (SDWORD)(MAP_MAXHEIGHT << TILE_SHIFT))
  {
    ASSERT_TEXT(FALSE, "scrTakeOverUnitsInArea: y1 is greater than max mapHeight");
    return FALSE;
  }

  if (y2 > (SDWORD)(MAP_MAXHEIGHT << TILE_SHIFT))
  {
    ASSERT_TEXT(FALSE, "scrTakeOverUnitsInArea: y2 is greater than max mapHeight");
    return FALSE;
  }
  ...
  for (psDroid = apsDroidLists[fromPlayer]; psDroid != NULL; psDroid = psNext)
  {
    ...
    {
      //give the droid away
      if (giftSingleDroid(psDroid, toPlayer)) { numChanged++; }
    }
  }

  if (!stackPushResult(VAL_INT, numChanged)) { return FALSE; }
...
BOOL scrTakeOverSingleDroid(void)
{
  ...
  if (!stackPopParams(2, ST_DROID, &psDroidToTake, VAL_INT, &playerToGain)) { return FALSE; }

  if (playerToGain >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrTakeOverSingleUnit:player number is too high");
    return FALSE;
  }

  if (psDroidToTake == NULL)
  {
    ASSERT_TEXT(FALSE, "scrTakeOverSingleUnit: Null unit");
    return FALSE;
  }

  ASSERT_TEXT(PTRVALID(psDroidToTake, sizeof(DROID)), "scrTakeOverSingleUnit: Invalid unit pointer");

  psNewDroid = giftSingleDroid(psDroidToTake, playerToGain);

  if (!stackPushResult((INTERP_TYPE)ST_DROID, (SDWORD)psNewDroid)) { return FALSE; }
  return TRUE;
}
...
BOOL scrTakeOverDroidsInAreaExp(void)
{
  ...
  for (psDroid = apsDroidLists[fromPlayer]; psDroid != NULL; psDroid = psNext)
  {
    psNext = psDroid->psNext;
    //check if within area specified
    if ((psDroid->droidType != DROID_CONSTRUCT) && (psDroid->droidType != DROID_REPAIR) && (psDroid->droidType != DROID_CYBORG_CONSTRUCT) &&
      (psDroid->droidType != DROID_CYBORG_REPAIR) &&
      //			((SDWORD)getDroidLevel(psDroid) <= level) AND
      ((SDWORD)psDroid->numKills <= level) && psDroid->x >= x1 && psDroid->x <= x2 && psDroid->y >= y1 && psDroid->y <= y2)
    {
      //give the droid away
      if (giftSingleDroid(psDroid, toPlayer)) { numChanged++; }
    }
...
BOOL scrTakeOverSingleStructure(void)
{
  ...
  if (!stackPopParams(2, ST_STRUCTURE, &psStructToTake, VAL_INT, &playerToGain)) { return FALSE; }

  if (playerToGain >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrTakeOverSingleStructure:player number is too high");
    return FALSE;
  }

  if (psStructToTake == NULL)
  {
    ASSERT_TEXT(FALSE, "scrTakeOverSingleStructure: Null structure");
    return FALSE;
  }

  ASSERT_TEXT(PTRVALID(psStructToTake, sizeof(STRUCTURE)), "scrTakeOverSingleStructure: Invalid structure pointer");

  structureInc = psStructToTake->pStructureType->ref - REF_STRUCTURE_START;
  if (playerToGain == (SDWORD)selectedPlayer && StructIsFactory(psStructToTake) && asStructLimits[playerToGain][structureInc].
  ...
  else
  {
    ...
    if (psNewStruct)
    {
      ...
      if (asStructLimits[playerToGain][structureInc].currentQuantity > asStructLimits[playerToGain][structureInc].limit)
      {
        asStructLimits[playerToGain][structureInc].limit = asStructLimits[playerToGain][structureInc].currentQuantity;
      }
      //for each structure taken - add graphical effect if the selectedPlayer
      if (playerToGain == (SDWORD)selectedPlayer) { assignSensorTarget((BASE_OBJECT*)psNewStruct); }
    }
  }

  if (!stackPushResult((INTERP_TYPE)ST_STRUCTURE, (SDWORD)psNewStruct)) { return FALSE; }
  return TRUE;
}
...
BOOL scrTakeOverStructsInArea(void)
{
  ...
  if (fromPlayer >= MAX_PLAYERS || toPlayer >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrTakeOverStructsInArea:player number is too high");
    return FALSE;
  }

  if (x1 > (SDWORD)(MAP_MAXWIDTH << TILE_SHIFT))
  {
    ASSERT_TEXT(FALSE, "scrTakeOverStructsInArea: x1 is greater than max mapWidth");
    return FALSE;
  }

  if (x2 > (SDWORD)(MAP_MAXWIDTH << TILE_SHIFT))
  {
    ASSERT_TEXT(FALSE, "scrTakeOverStructsInArea: x2 is greater than max mapWidth");
    return FALSE;
  }

  if (y1 > (SDWORD)(MAP_MAXHEIGHT << TILE_SHIFT))
  {
    ASSERT_TEXT(FALSE, "scrTakeOverStructsInArea: y1 is greater than max mapHeight");
    return FALSE;
  }

  if (y2 > (SDWORD)(MAP_MAXHEIGHT << TILE_SHIFT))
  {
    ASSERT_TEXT(FALSE, "scrTakeOverStructsInArea: y2 is greater than max mapHeight");
    return FALSE;
  }
  ...
  for (psStruct = apsStructLists[fromPlayer]; psStruct != NULL; psStruct = psNext)
  {
    ...
    //check if within area specified
    if (psStruct->x >= x1 && psStruct->x <= x2 && psStruct->y >= y1 && psStruct->y <= y2)
    {
      ...
      if (toPlayer == (SDWORD)selectedPlayer && StructIsFactory(psStruct) && asStructLimits[toPlayer][structureInc].currentQuantity >=
        MAX_FACTORY) { DBPRINTF(("scrTakeOverStructsInArea - factory ignored for selectedPlayer\n")); }
      else
      {
        ...
        if (psNewStruct)
        {
          ...
          if (asStructLimits[toPlayer][structureInc].currentQuantity > asStructLimits[toPlayer][structureInc].limit)
          {
            asStructLimits[toPlayer][structureInc].limit = asStructLimits[toPlayer][structureInc].currentQuantity;
          }
          //for each structure taken - add graphical effect if the selectedPlayer
          if (toPlayer == (SDWORD)selectedPlayer) { assignSensorTarget((BASE_OBJECT*)psNewStruct); }
        }
      }
    }
  }
...
BOOL scrGetDroidCount(void)
{
  ...
  if (player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrGetUnitCount:player number is too high");
    return FALSE;
  }

  if (!stackPushResult(VAL_INT, getNumDroids(player))) { return FALSE; }
...
BOOL scrFireWeaponAtObj(void)
{
  ...
  if (!stackPopParams(2, ST_WEAPON, &wIndex, ST_BASEOBJECT, &psTarget)) { return FALSE; }

  if (psTarget == NULL)
  {
    ASSERT_TEXT(FALSE,"scrFireWeaponAtObj: Null target pointer");
    return FALSE;
  }
...
BOOL scrFireWeaponAtLoc(void)
{
  ...
  if (!stackPopParams(3, ST_WEAPON, &wIndex, VAL_INT, &x, VAL_INT, &y)) { return FALSE; }
...
BOOL scrSetDroidKills(void)
{
  ...
  if (!stackPopParams(2, ST_DROID, &psDroid, VAL_INT, &kills)) { return TRUE; }

  if ((psDroid == NULL) || (psDroid->type != OBJ_DROID))
  {
    ASSERT_TEXT(FALSE, "scrSetUnitKills: NULL/invalid unit pointer");
    return FALSE;
  }
...
BOOL scrResetPlayerVisibility(void)
{
  ...
  if (player < 0 || player > MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrResetPlayerVisibility: invalid player");
    return FALSE;
  }
  ...
  for (i = 0; i < MAX_PLAYERS; i++)
  {
    ...
    for (psObj = (BASE_OBJECT*)apsDroidLists[i]; psObj; psObj = psObj->psNext) { psObj->visible[player] = 0; }

    for (psObj = (BASE_OBJECT*)apsStructLists[i]; psObj; psObj = psObj->psNext) { psObj->visible[player] = 0; }
  }

  for (psObj = (BASE_OBJECT*)apsFeatureLists[0]; psObj; psObj = psObj->psNext) { psObj->visible[player] = 0; }
...
BOOL scrSetVTOLReturnPos(void)
{
  ...
  if (!stackPopParams(3, VAL_INT, &player, VAL_INT, &tx, VAL_INT, &ty)) { return FALSE; }

  if (player < 0 || player >= MAX_PLAYERS)
  {
    ASSERT_TEXT(FALSE, "scrSetVTOLReturnPos: invalid player");
    return FALSE;
  }
...
BOOL scrResetLimboMission(void)
{
  ...
  {
    ASSERT_TEXT(FALSE, "scrResetLimboMission: current mission type invalid");
    return FALSE;
  }
...
BOOL scrIsVtol(void)
{
  ...
  if (!stackPopParams(1, ST_DROID, &psDroid)) { return TRUE; }

  if (psDroid == NULL) { ASSERT_TEXT(FALSE,"scrIsVtol: null droid passed in."); }
...
BOOL scrTutorialTemplates(void)
{
  ...
  for (psCurr = apsDroidTemplates[selectedPlayer], psPrev = NULL; psCurr != NULL; psCurr = psCurr->psNext)
  {
    ...
    {
      if (psPrev) { psPrev->psNext = psCurr->psNext; }
      else { apsDroidTemplates[selectedPlayer] = psCurr->psNext; }
  ...
  // Delete the template.
  if (psCurr) { HEAP_FREE(psTemplateHeap, psCurr); }
  else
  {
    DBERROR(("tutorial template setup failed"));
    return FALSE;
