...
... this replaces the thousands of occurance that is spread through out the code
 ...
 ... There were a couple of places where it skipping around a routine if the stat was a expansion module
	(loadSaveStructureV7 & 9) this code seemed suspect, && to clarify it we replaced it with the routine below
...
void structureType(STRUCTURE_STATS *pStructure, char *pType)
{
	...
	ASSERT_TEXT(FALSE, "Unknown Structure Type");
}
...
//#endif
		...
		else
		{
			...
			ASSERT_TEXT(psStructure->pSensor != NULL, 
			...
			{
				DBERROR(("loadStructureStats: a Turret Sensor && weapon \
...
//	FREE(pData);
	...
	{
		DBERROR( ("intAddObjectStats: destroy structure stat !found\n") );
	}
	...
	for (player = 0; player < MAX_PLAYERS; player++)
	{
			...
			psCurr->psNext)
		{
			...
			if (displayError)
			{
				...
				{
					ASSERT_TEXT(FALSE, "There appears to be too many %s on this map!", 
...
BOOL structSetManufacture(STRUCTURE *psStruct, DROID_TEMPLATE *psTempl, UBYTE quantity)
{
	...
	ASSERT_TEXT(PTRVALID(psStruct, sizeof(STRUCTURE)) && psStruct->type == OBJ_STRUCTURE &&
	...
	ASSERT_TEXT(psTempl == NULL || PTRVALID(psTempl, sizeof(DROID_TEMPLATE)),
...
//					nayborType = WALL_CORNER;
				...
				if (nayborType != scanType)
				{
					...
					if (scanType == WALL_CORNER)
					{
						...
						{
							/* Still being built - so save && load build points */
...
//#endif
	...
	if (IsStatExpansionModule(pStructureType)==FALSE)
	{
        ...
        {
            ASSERT_TEXT(FALSE, "buildStructure:Invalid structure type");
            return NULL;
        }
        ...
        if (player == selectedPlayer)
        {
            ...
            if (pStructureType->psWeapStat && pStructureType->psWeapStat->
        ...
        {
            ASSERT_TEXT(FALSE, "buildStructure: x coord too near edge");
	    	return NULL;
        }
	    if(((y >> TILE_SHIFT) < TOO_NEAR_EDGE) || ((y >> TILE_SHIFT) > (SDWORD)(
            mapHeight - TOO_NEAR_EDGE)))
        {
            ASSERT_TEXT(FALSE, "buildStructure: y coord too near edge");
		    return NULL;
        }
...
//psor				mapTile(mapX+width, mapY+breadth)->psObject = (BASE_OBJECT*) psBuilding;
				...
				{
					ASSERT_TEXT(!(TILE_HAS_STRUCTURE(mapTile(mapX+width,mapY+breadth))),
...
//        psBuilding->asWeaps[0].nStat = 0;
        ...
        if (pStructureType->psWeapStat)
		{
            ...
            if (bMultiPlayer && pStructureType->psWeapStat->
...
//		memset(psBuilding->visible, 0, sizeof(psBuilding->visible));
		/* !sure about above line - it concerns me, so... */
...
//		}
 //		}
        ...
        /*if we're coming from a SAVEGAME && we're on an Expand_Limbo mission, 
        ...
        changed to !include them. This is the only HACK I can think of to 
...
//psor		psBuilding = (STRUCTURE *)mapTile(x >> TILE_SHIFT, y >> TILE_SHIFT)->psObject;
		...
		if (pStructureType->type == REF_FACTORY_MODULE)
		{
			...
			if (((FACTORY*)psBuilding->pFunctionality)->capacity < SIZE_SUPER_HEAVY)
			{
				...
				ASSERT_TEXT(((FACTORY*)psBuilding->pFunctionality)->productionOutput +
...
void createAssemblyPoint(STRUCTURE* psStruct)
{
	...
	UDWORD	x, y;
		...
		case REF_VTOL_FACTORY:
			...
			break;
		default:
			ASSERT_TEXT(FALSE, "setFunctionality: Invalid factory type");
		}
...
void assignFactoryCommandDroid(STRUCTURE *psStruct, DROID *psCommander)
{
	...
	ASSERT_TEXT(StructIsFactory(psStruct),"assignFactoryCommandUnit: structure !a factory");
	...
	case REF_CYBORG_FACTORY:
		...
		break;
	default:
		ASSERT_TEXT(FALSE,"assignfactorycommandUnit: unknown factory type");
		typeFlag = FACTORY_FLAG;
		break;
	}
...
/* Place a newly manufactured droid next to a factory  && then send if off 
...
//}
	 ...
	 * determine the subject stats (for research || manufacture)
	 * || base object (for repair) || update power levels for resourceExtractor
	 */
...
//				}
			...
			if ( psChosenObj == NULL )
			{
				ASSERT_TEXT( PTRVALID( psRepairFac->psGroup, sizeof(DROID_GROUP) ),
...
/*				actionDroidObjLoc( psDroid, DACTION_MOVETOREPAIRPOINT,
	...
	/* check subject stats (for research || manufacture) */
	if (pSubject != NULL)
	{
...
//				POWER REQUIRMENTS REMOVED - AB  22/09/98 - and back again 07/01/99
			 ...
			 //		bFinishRepair = TRUE;
					...
					secondarySetState(psDroid, DSO_RETURN_TO_LOC, (SECONDARY_STATE)0);
...
//						moveShuffleDroid(psDroid, -TILE_UNITS, 0);
		...
		else if (structureMode == REF_REARM_PAD)
		{
			...
			ASSERT_TEXT( PTRVALID(psDroid, sizeof(DROID)),
					"aiUpdateStructure: invalid droid pointer"  );
			ASSERT_TEXT(vtolDroid(psDroid),"aiUpdateStructure: invalid droid type");
				 ...
				 psDroid->sMove.Status == MOVEINACTIVE )
			{
				...
				if (vtolHappy(psDroid))
				{
					...
					/*  Totally ditch all this now - John
						...
						then catch this && tell it what to do if anything.
...
iVector			dv;
	...
	/* Only add smoke if they're visible && they can 'burn' */
...
// tjc 4-dec-98  ... 
                ...
                //HARD_CODE don't ever want more than one Las Sat structure
                if (psBuilding->psWeapStat && psBuilding->psWeapStat->
                    weaponSubClass == WSC_LAS_SAT && getLasSatExists(selectedPlayer))
                {
                    continue;
                }
		...
		psStats->ref < (REF_STRUCTURE_START + REF_RANGE))
	{
        ...
        if (wallDrag.status != DRAG_INACTIVE)
        {
            if (psBuilding->type == REF_WALL || psBuilding->type == REF_DEFENSE)
            {
				...
				dx = (UWORD)(abs((int)(mouseTileX - wallDrag.x1)));
				dy = (UWORD)(abs((int)(mouseTileY - wallDrag.y1)));
...
//		site.yBR = (UWORD)(y + psBuilding->baseBreadth - 1);
		...
		switch(psBuilding->type)
		{
            ...
            case REF_MISSILE_SILO:
				/*need to check each tile the structure will sit on is !water*/
				for (i = site.xTL; i <= site.xBR && valid; i++) 
				{
					for (j = site.yTL; j <= site.yBR && valid; j++) 
					{
                        ...
                        psDroid->droidType == DROID_CYBORG_CONSTRUCT)
                    {
                        ...
                        for (order = 0; order < psDroid->listSize; order++)
                        {
                            if (psDroid->asOrderList[order].order == DORDER_BUILD)
                            {
                                ...
                                if (psBuilding->type == REF_WALL || psBuilding->type == 
                                    REF_WALLCORNER && ((STRUCTURE_STATS *)psDroid->
                                    asOrderList[order].psOrderTarget)->type == REF_WALL || 
                                    ((STRUCTURE_STATS *)psDroid->asOrderList[order].
		...
		if(x+side<mapWidth && y<mapHeight)
		{
			if(!TILE_OCCUPIED(mapTile(x+side,y)) )
			{
				...
				*pDroidY = y << TILE_SHIFT;
				...
				ASSERT_TEXT(worldOnMap(*pDroidX,*pDroidY),"checkWidth : Insane droid position");
		...
		if(y+side<mapHeight && x<mapWidth)
		{
			if(!TILE_OCCUPIED(mapTile(x,y+side)) )
			{
				...
				*pDroidY = (y + side) << TILE_SHIFT;
				...
				ASSERT_TEXT(worldOnMap(*pDroidX,*pDroidY),"checkHeight : Insane droid position");
...
BOOL removeStruct(STRUCTURE *psDel, BOOL bDestroy)
{
	...
	ASSERT_TEXT( PTRVALID(psDel, sizeof(STRUCTURE)),
    ...
    if (bDestroy)
    {
	    ...
	    HOW MUCH IS THERE && !RES EXTRACTORS */
...
//---------------------------------------  Do we add immediate explosions?
		/* Set off some explosions, but !for walls */
		/* First Explosions */
...
//--------------------------------------- Do we add a fire?
		...
		{
			/* Oil resources burn && puff out smoke && for longer*/
...
/* For now all this does is work out what height the terrain needs to be set to
   ...
   The x && y passed in are the CENTRE of the structure*/
...
BOOL checkSpecificStructExists(UDWORD structInc, UDWORD player)
{
	...
	ASSERT_TEXT(structInc < numStructureStats, 
...
void findAssemblyPointPosition(UDWORD *pX, UDWORD *pY, UDWORD player)
{
	...
	/*if (TILE_OCCUPIED(mapTile(*pX,*pY)))
	{
		...
		{
			ASSERT_TEXT(FALSE, "findAssemblyPointPosition: Unable to find a free location");
		}
	}
	else
	{
    ...
    ASSERT_TEXT(FALSE, "findAssemblyPointPosition: unable to find a valid location!");
}
...
{
	ASSERT_TEXT(PTRVALID(psAssemblyPoint, sizeof(FLAG_POSITION)),
...
#endif
	    ASSERT_TEXT(FALSE, "Building more than %d %s for player %d", MAX_FACTORY, pType, player);
		inc = 1;
	}
...
STRUCTURE_STATS * structGetDemolishStat( void )
{
	...
	{
		DBERROR( ("structGetDemolishStat: stat !initialised1\n") );
	}
...
void checkForResExtractors(STRUCTURE *psBuilding)
{
	...
	if (psBuilding->pStructureType->type != REF_POWER_GEN)
	{
		ASSERT_TEXT(FALSE, "checkForResExtractors: invalid structure type");
		return;
	}
	psPowerGen = (POWER_GEN *)psBuilding->pFunctionality; 
...
void checkForPowerGen(STRUCTURE *psBuilding)
{
	...
	{
		ASSERT_TEXT(FALSE, "checkForPowerGen: invalid structure type");
		return;
	}
...
void informPowerGen(STRUCTURE *psStruct)
{
	...
	{
		ASSERT_TEXT(FALSE, "informPowerGen: invalid structure type");
		return;
	}
...
void releaseResExtractor(STRUCTURE *psRelease)
{
	...
	{
		ASSERT_TEXT(FALSE, "releaseResExtractor:Invalid structure type");
		return;
	}
...
void releasePowerGen(STRUCTURE *psRelease)
{
	...
	{
		ASSERT_TEXT(FALSE, "releasePowerGen:Invalid structure type");
		return;
	}
...
BOOL electronicDamage(BASE_OBJECT *psTarget, UDWORD damage, UBYTE attackPlayer)
{
    ...
    ASSERT_TEXT(attackPlayer < MAX_PLAYERS,
    ...
    if (psTarget->type == OBJ_STRUCTURE)
    {
	    ...
	    ASSERT_TEXT(psStructure->pStructureType->resistance != 0, 
    ...
    else if (psTarget->type == OBJ_DROID)
    {
        ...
        if (bMultiPlayer)
        {
            ...
            {
                ASSERT_TEXT(FALSE, "electronicDamage: Cannot attack a Transporter in multiPlayer");
                return TRUE;
            }
        }
...
UDWORD	structureBaseBody(STRUCTURE *psStructure)
{
	...
	case REF_FACTORY:
	case REF_VTOL_FACTORY:
		ASSERT_TEXT(PTRVALID(psStructure->pFunctionality, sizeof(FUNCTIONALITY)),
...
/*find the 'best' prop/body/weapon component the losing player has && 
...
/*find the 'best' repair component the losing player has && 
'give' it to the reward player*/
...
void cancelProduction(STRUCTURE *psBuilding)
{
	...
	ASSERT_TEXT(StructIsFactory(psBuilding), 
		"cancelProduction: structure !a factory");
...
void holdProduction(STRUCTURE *psBuilding)
{
	...
	ASSERT_TEXT(StructIsFactory(psBuilding), 
		"holdProduction: structure !a factory");
...
void releaseProduction(STRUCTURE *psBuilding)
{
	...
	ASSERT_TEXT(StructIsFactory(psBuilding), 
		"releaseProduction: structure !a factory");
...
DROID_TEMPLATE * factoryProdUpdate(STRUCTURE *psStructure, DROID_TEMPLATE *psTemplate)
{
	...
	ASSERT_TEXT(psStructure->player == productionPlayer, 
...
UWORD countAssignableFactories(UBYTE player,UWORD factoryType)
{
	...
	ASSERT_TEXT(player == selectedPlayer, 
...
{
	ASSERT_TEXT(player < MAX_PLAYERS,
		"checkFactoryExists: invalid player");
	ASSERT_TEXT(factoryType < NUM_FACTORY_TYPES,
        ...
        if (inc != selectedPlayer)
        {
			    ...
			    psStruct->psNext)
		    {
			    if(StructIsFactory(psStruct))
			    {
				    ...
				    {
					    ASSERT_TEXT(psFactory->psAssemblyPoint != NULL,
			    ...
			    else if (psStruct->pStructureType->type == REF_REPAIR_FACILITY)
			    {
				    ...
				    if (psRepair->psDeliveryPoint == NULL)//need to add one
				    {
					    ...
					    {
						    ASSERT_TEXT(psRepair->psDeliveryPoint != NULL,"checkDeliveryPoints: no delivery point for repair facility");
					    }
					    else
					    {
						    // add an assembly point
						    ...
						    {
							    ASSERT_TEXT(FALSE,"checkDeliveryPoints: unable to create new delivery point for repair facility");
							    return;
						    }
...
void factoryLoopAdjust(STRUCTURE *psStruct, BOOL add)
{
	...
	ASSERT_TEXT(StructIsFactory(psStruct), 
		"factoryLoopAdjust: structure is !a factory");
	ASSERT_TEXT(psStruct->player == selectedPlayer, 
...
BOOL checkStructureStats(void)
{
	...
	for (structInc=0; structInc < numStructureStats; structInc++)
	{
        if (asStructureStats[structInc].numFuncs != 0)
        {
            ...
            {
                ASSERT_TEXT(PTRVALID(asStructureStats[structInc].asFuncList[inc], 
        ...
        else
        {
            ...
            {
                ASSERT_TEXT(FALSE, "checkStructureStats:Invalid functions attached to structure %s", 
                    asStructureStats[structInc].pName);
                return FALSE;
            }
        }
