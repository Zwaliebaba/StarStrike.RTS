...
GLuint gfx_load_texture (char *filename, int x, int y, int size)
{
	...
	if (bmp == NULL)
	{
		DebugTrace("Error reading bitmap file: %s.\n", filename);
		return 1;
	}
...
GLuint gfx_load_font (char *filename, char *maskname)
{
	...
	if (!fontmask)
	{
		DebugTrace("Error reading font bitmap file: %s.\n", maskname);
		exit(0);
	}
...
int gfx_graphics_startup (void)
{
    ...
    if( SDL_SetVideoMode( screen_w, screen_h, color_depth, SDL_OPENGL | ((windowed) ? SDL_RESIZABLE : SDL_FULLSCREEN) ) == 0 ) {
	...
	if (!scanner_image)
	{
    DebugTrace("Error reading scanner bitmap file: {}.\n", scanner_filename);
		return 1;
	}
