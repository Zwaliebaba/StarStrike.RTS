/*	
	...
	Attempt to rationalise the unit selection procedure &&	
...
/*	
	...
	It is also possible to request whether the units be onscreen || !.
*/
...
char	selInfo[255];
	...
	case	DS_BY_TYPE:
		...
		case DST_ALL_SAME:
			...
			break;
		default:
			ASSERT_TEXT(FALSE,"Invalid selection type in uniDroidSelection");
		}
		break;
	default:
		ASSERT_TEXT(FALSE,"Invalid selection attempt in uniDroidSelection");
		break;
	}
...
BOOL	bLaterInList, bMatch;
		...
		psCurr && !psResult; psCurr = psCurr->psNext)
	{
        ...
        if (bMatch)
		{
			...
			/* Only select is this isn't the old one && it's further on in list */
...
BOOL		bLaterInList;
	...
	ASSERT_TEXT(structType>=REF_HQ && structType<=NUM_DIFF_BUILDINGS,
