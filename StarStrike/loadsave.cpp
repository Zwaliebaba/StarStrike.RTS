...
void load_current_cmdr_data(void)
{
  ...
  Dirlist* cur_elem = commanders;
  for (i = 0; i < current_commander; i++) cur_elem = cur_elem->next;
...
void initialize_load_commander_screen(void)
{
  ...
  for (Dirlist* cur_elem = commanders; cur_elem != nullptr; cur_elem = cur_elem->next) num_commanders++;

  if (commanders != nullptr) load_current_cmdr_data();

  current_subscreen = SUBSCR_LOAD_CMDR_SELECT;
}

void complete_load_commander_screen(void) { dirlist_free(commanders); }
...
void select_previous_commander(void)
{
  ...
  if (current_commander < current_commander_base) current_commander_base--;
}
...
void select_next_commander(void)
{
  ...
  if ((current_commander - current_commander_base) == COMMANDER_LIST_SIZE) current_commander_base++;
}
...
void handle_load_commander_screen_keys(int key, char ascii)
{
  ...
  switch (current_subscreen)
  {
    case SUBSCR_LOAD_CMDR_SELECT:
      if (key == KBD_UP) select_previous_commander();

      if (key == KBD_DOWN) select_next_commander();

      if (key == KBD_ENTER) { if (commanders && !current_cmdr_file_is_corrupt) current_subscreen = SUBSCR_LOAD_CMDR_CONFIRM; }

      if (key == KBD_ESCAPE) switch_to_screen(SCR_CMDR_STATUS);
    ...
    case SUBSCR_LOAD_CMDR_CONFIRM:
      ...
      if (key == KBD_N) current_subscreen = SUBSCR_LOAD_CMDR_SELECT;
      break;
  }
}
...
void draw_load_commander_screen_select(void)
{
  ...
  if (current_cmdr_file_is_corrupt) gfx_display_centre_text(90, "File is corrupt", 140, GFX_COL_GOLD);
  else display_abbreviated_status(&current_cmdr_data);

  y = 160 + (current_commander - current_commander_base) * 16;
  gfx_draw_rectangle(2, y + 1, 510, y + 15, GFX_COL_DARK_RED);

  Dirlist *cur_elem = commanders;
  for (i = 0; i < current_commander_base; i++) cur_elem = cur_elem->next;
...
static void add_name_char(int letter)
{
  ...
  if (len == MAX_CMDR_NAME_LENGTH) return;
...
void check_existing_file(void)
{
  ...
  if (dirlist) cmdr_file_exists = 1;
  else cmdr_file_exists = 0;
...
void save_commander(void)
{
  ...
  if (save_commander_file(str)) info_message("Error Saving Commander!");
  else info_message("Commander Saved.");
...
void handle_save_commander_screen_keys(int key, char ascii)
{
  switch (current_subscreen)
  {
    ...
    case SUBSCR_SAVE_CMDR_CONFIRM:
      ...
      if (key == KBD_N) current_subscreen = SUBSCR_SAVE_CMDR_INPUT;
      break;
  }
}