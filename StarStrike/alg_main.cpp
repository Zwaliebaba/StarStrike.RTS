...
void finish_game(void) { finish = 1; }
...
int ready_to_draw(void)
{
  ...
  timeslice = static_cast<double>(ms_count) / static_cast<double>(speed_cap);
  timedelta = static_cast<double>(elapsed) / static_cast<double>(speed_cap);
  return 1;
}
...
void draw_screen(void)
{
  ...
  switch (current_screen)
  {
    ...
    case SCR_RIGHT_VIEW:
      ...
      if (draw_lasers) draw_laser_lines();
      draw_laser_sights();
      break;
    ...
    case SCR_ESCAPE_POD:
      update_starfield();
      gfx_draw_view();
      if (escape_pod_timer < 90) gfx_display_centre_text(358, "Escape pod launched - Ship auto-destuct initiated.", 120, GFX_COL_WHITE);
      break;
  }
...
void handle_function_keys(int key)
{
  ...
  {
    case KBD_F1:
      if (docked) launch_player();
      else switch_to_screen(SCR_FRONT_VIEW);
      break;

    case KBD_F2:
      if (!docked) switch_to_screen(SCR_REAR_VIEW);
      break;

    case KBD_F3:
      if (!docked) switch_to_screen(SCR_LEFT_VIEW);
      break;

    case KBD_F4:
      if (!docked) switch_to_screen(SCR_RIGHT_VIEW);
      else switch_to_screen(SCR_EQUIP_SHIP);
      break;
    ...
    case KBD_F8:
      if (!witchspace) switch_to_screen(SCR_MARKET_PRICES);
      break;
...
void auto_dock(void)
{
  ...
  if (ship.velocity > 22) flight_speed = 22;
  else flight_speed = ship.velocity;

  if (ship.acceleration > 0)
  {
    flight_speed++;
    if (flight_speed > 22) flight_speed = 22;
  }

  if (ship.acceleration < 0)
  {
    flight_speed--;
    if (flight_speed < 1) flight_speed = 1;
  }

  if (ship.rotx == 0) flight_climb = 0;

  if (ship.rotx < 0)
  {
    increase_flight_climb();

    if (ship.rotx < -1) increase_flight_climb();
  }
  ...
  if (ship.rotx > 0)
  {
    ...
    if (ship.rotx > 1) decrease_flight_climb();
  }

  if (ship.rotz == 127) flight_roll = -14;
  else
  {
    if (ship.rotz == 0) flight_roll = 0;

    if (ship.rotz > 0)
    {
      increase_flight_roll();

      if (ship.rotz > 1) increase_flight_roll();
    }
    ...
    if (ship.rotz < 0)
    {
      ...
      if (ship.rotz < -1) decrease_flight_roll();
    }
  }
}
...
void update_escape_sequence(void)
{
  ...
  else if ((ship_count[SHIP_CORIOLIS] == 0) && (ship_count[SHIP_DODEC] == 0))
  {
    auto_dock();
...
{
  if (kbd[KBD_FIRE]) { if (draw_lasers == 0) draw_lasers = fire_laser(); }

  if (kbd[KBD_DOCK] && cmdr.docking_computer)
  {
    if (instant_dock) engage_docking_computer();
    else engage_auto_pilot();
  }

  if (kbd[KBD_ECM] && cmdr.ecm) activate_ecm(1);

  if (kbd[KBD_HYPERSPACE])
  {
    if (kbd[KBD_CTRL]) start_galactic_hyperspace();
    else start_hyperspace();
  }

  if (kbd[KBD_JUMP] && !witchspace) jump_warp();

  if (kbd[KBD_FIRE_MISSILE]) fire_missile();

  if (kbd[KBD_TARGET_MISSILE]) arm_missile();

  if (kbd[KBD_UNARM_MISSILE]) unarm_missile();

  if (kbd[KBD_INC_SPEED]) { if (flight_speed < myship.max_speed) flight_speed++; }

  if (kbd[KBD_DEC_SPEED]) { if (flight_speed > 1) flight_speed--; }
  ...
  if (kbd[KBD_ESCAPE] && cmdr.escape_pod && !witchspace) switch_to_screen(SCR_ESCAPE_POD);

  if (kbd[KBD_PAUSE]) game_paused = 1;
}
...
void handle_keyboard_state(void)
{
  ...
  switch (current_screen)
  {
    ...
    case SCR_SHORT_RANGE:
      ...
      if (kbd[KBD_RIGHT]) move_cross(1, 0);

      if (kbd[KBD_LEFT]) move_cross(-1, 0);

      if (kbd[KBD_UP]) move_cross(0, -1);

      if (kbd[KBD_DOWN]) move_cross(0, 1);
...
void handle_key_event(int key, char ascii)
{
  if (game_paused)
  {
    if (key == KBD_RESUME) game_paused = 0;
    return;
  }

  if ((current_screen != SCR_INTRO_ONE) && (current_screen != SCR_INTRO_TWO) && (current_screen != SCR_GAME_OVER) && (current_screen != SCR_MISSION) && (current_screen != SCR_ESCAPE_POD) && (current_screen != SCR_BREAK_PATTERN)) handle_function_keys(key);
  ...
  switch (current_screen)
  {
    ...
    case SCR_LEFT_VIEW:
    case SCR_RIGHT_VIEW:
      if (key == KBD_D) { if (auto_pilot) disengage_auto_pilot(); }
      break;
    ...
    case SCR_SHORT_RANGE:
      ...
      if (key == KBD_ORIGIN) move_cursor_to_origin();
      if (key == KBD_D) show_distance_to_planet();
      if (key == KBD_FIND)
      {
        find_input = 1;
        *find_name = '\0';
    ...
    case SCR_EQUIP_SHIP:
      if (key == KBD_UP) select_previous_equip();

      if (key == KBD_DOWN) select_next_equip();

      if (key == KBD_ENTER) buy_equip();

      break;

    case SCR_OPTIONS:
      if (key == KBD_UP) select_previous_option();

      if (key == KBD_DOWN) select_next_option();

      if (key == KBD_ENTER) do_option();

      break;

    case SCR_SETTINGS:
      if (key == KBD_RIGHT) select_right_setting();

      if (key == KBD_LEFT) select_left_setting();

      if (key == KBD_UP) select_up_setting();

      if (key == KBD_DOWN) select_down_setting();

      if (key == KBD_ENTER) toggle_setting();
    ...
    case SCR_MARKET_PRICES:
      if (docked)
      {
        if (key == KBD_RIGHT) buy_stock();

        if (key == KBD_LEFT) sell_stock();

        if (key == KBD_UP) select_previous_stock();

        if (key == KBD_DOWN) select_next_stock();
      }
      break;
    ...
    case SCR_QUIT:
      if (key == KBD_Y) finish_game();

      if (key == KBD_N)
      {
        if (docked) switch_to_screen(SCR_CMDR_STATUS);
        else switch_to_screen(SCR_FRONT_VIEW);
      }
...
void update_game_over_screen(void)
{
  update_universe();
  game_over_timer++;
  if (game_over_timer == 100) reset_game();
}
...
void initialize_break_pattern(void) { break_pattern_timer = 0; }
...
void update_simulation(void)
{
  ...
  if (draw_lasers > 0) draw_lasers--;

  mcount--;
  if (mcount < 0) mcount = 255;

  if (((mcount & 127) == 0) && (auto_pilot)) info_message("Docking Computers On");

  if (((mcount & 3) == 0) && (hyper_ready)) countdown_hyperspace();
  ...
  switch (current_screen)
  {
    ...
    default:
      if (!docked)
      {
        ...
        if (game_over == 1) switch_to_screen(SCR_GAME_OVER);
      }

      if (cross_timer > 0)
      {
        cross_timer--;
        if (cross_timer == 0) show_distance_to_planet();
      }
  }
