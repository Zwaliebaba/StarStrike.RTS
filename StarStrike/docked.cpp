...
const char *economy_type[] = {"Rich Industrial", "Average Industrial", "Poor Industrial", "Mainly Industrial", "Mainly Agricultural", "Rich Agricultural", "Average Agricultural", "Poor Agricultural"};

const char *government_type[] = {"Anarchy", "Feudal", "Multi-Government", "Dictatorship", "Communist", "Confederacy", "Democracy", "Corporate State"};
...
void show_distance(char *str, struct galaxy_seed from_planet, struct galaxy_seed to_planet)
{
  ...
  if (light_years > 0) sprintf(str, "Distance: %2d.%d Light Years ", light_years / 10, light_years % 10);
  else sprintf(str, "%s", "");
}
...
void display_galactic_chart(void)
{
  ...
  draw_fuel_limit_circle(docked_planet.d * GFX_SCALE, (docked_planet.b / (2 / GFX_SCALE)) + (18 * GFX_SCALE) + 1);
  ...
  for (i = 0; i < 256; i++)
  {
    ...
    if ((glx.e | 0x50) < 0x90) gfx_draw_colour_line(px, py, px + 1, py, GFX_COL_WHITE);
    else gfx_plot_pixel(px, py, GFX_COL_WHITE);
...
struct rank rating[NO_OF_RANKS] = {{0x0000, "Harmless"}, {0x0008, "Mostly Harmless"}, {0x0010, "Poor"}, {0x0020, "Average"}, {0x0040, "Above Average"}, {0x0080, "Competent"}, {0x0200, "Dangerous"}, {0x0A00, "Deadly"}, {0x1900, "---- E L I T E ---"}};
...
static char *condition_txt[] = {"Docked", "Green", "Yellow", "Red"};
...
void display_commander_status(void)
{
  ...
  else
  {
    ...
    if ((condition == 2) && (g_energy < 128)) condition = 3;
  }
  ...
  if (cmdr.legal_status == 0) strcpy(str, "Clean");
  else strcpy(str, cmdr.legal_status > 50 ? "Fugitive" : "Offender");

  gfx_display_colour_text(16, 138, "Legal Status:", GFX_COL_GREEN_1);
  gfx_display_text(128, 138, str);

  for (i = 0; i < NO_OF_RANKS; i++) { if (cmdr.score >= rating[i].score) strcpy(str, rating[i].title); }
...
void display_abbreviated_status(commander *_cmdr)
{
  char planet_name[16];
  char str[100];

  gfx_display_colour_text(16, 58, "Commander Name:", GFX_COL_GREEN_1);
  sprintf(str, "%s", _cmdr->name);
  gfx_display_text(170, 58, str);

  gfx_display_colour_text(16, 74, "Present System:", GFX_COL_GREEN_1);
  name_planet(planet_name, _find_planet(_cmdr->ship_x, _cmdr->ship_y, _cmdr->galaxy));
  capitalise_name(planet_name);
  sprintf(str, "%s", planet_name);
  gfx_display_text(150, 74, str);

  sprintf(str, "%d.%d Cr", _cmdr->credits / 10, _cmdr->credits % 10);
  gfx_display_colour_text(16, 90, "Cash:", GFX_COL_GREEN_1);
  gfx_display_text(70, 90, str);

  if (_cmdr->legal_status == 0) strcpy(str, "Clean");
  else strcpy(str, _cmdr->legal_status > 50 ? "Fugitive" : "Offender");

  gfx_display_colour_text(16, 106, "Legal Status:", GFX_COL_GREEN_1);
  gfx_display_text(128, 106, str);

  for (int i = 0; i < NO_OF_RANKS; i++) { if (_cmdr->score >= rating[i].score) strcpy(str, rating[i].title); }
...
void display_stock_price(int i)
{
  ...
  if (stock_market[i].current_quantity > 0) { sprintf(str, "%d%s", stock_market[i].current_quantity, unit_name[stock_market[i].units]); }
  else strcpy(str, "-");

  gfx_display_text(338, y, str);

  if (cmdr.current_cargo[i] > 0) { sprintf(str, "%d%s", cmdr.current_cargo[i], unit_name[stock_market[i].units]); }
  else strcpy(str, "-");
...
{
  if ((!docked) || (hilite_item == 0)) return;
...
{
  if ((!docked) || (hilite_item == 16)) return;
...
void buy_stock(void)
{
  ...
  if ((item->current_quantity == 0) || (cmdr.credits < item->current_price)) return;

  cargo_held = total_cargo();

  if ((item->units == TONNES) && (cargo_held == cmdr.cargo_capacity)) return;
...
void sell_stock(void)
{
  ...
  if ((!docked) || (cmdr.current_cargo[hilite_item] == 0)) return;
...
void select_next_equip(void)
{
  ...
  if (hilite_item == (NO_OF_EQUIP_ITEMS - 1)) return;
  ...
  if (next != hilite_item) hilite_item = next;
}
...
void select_previous_equip(void)
{
  ...
  if (prev != hilite_item) hilite_item = prev;
}
...
void list_equip_prices(void)
{
  ...
  {
    equip_stock[i].canbuy = ((equip_present(equip_stock[i].type) == 0) && (equip_stock[i].price <= cmdr.credits));
...
void buy_equip(void)
{
  ...
  if (equip_stock[hilite_item].name[0] == '+')
  {
    ...
    hilite_item++;
    for (i = 0; i < 5; i++) equip_stock[hilite_item + i].show = 1;
    return;
  }

  if (equip_stock[hilite_item].canbuy == 0) return;
...
void initialize_market_screen(void) { hilite_item = 0; }