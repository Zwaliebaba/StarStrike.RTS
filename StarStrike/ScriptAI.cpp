...
BOOL scrGroupAddGroup(void)
{
	...
	ASSERT_TEXT(PTRVALID(psTo, sizeof(DROID_GROUP)),
		"scrGroupAddGroup: Invalid group pointer");
	ASSERT_TEXT(PTRVALID(psFrom, sizeof(DROID_GROUP)),
		"scrGroupAddGroup: Invalid group pointer");
...
BOOL scrIterateGroup(void)
{
	...
	{
		ASSERT_TEXT(FALSE, "scrIterateGroup: invalid group, InitGroupIterate !called?");
		return FALSE;
	}
...
BOOL scrOrderGroupObj(void)
{
	...
	ASSERT_TEXT(PTRVALID(psObj, sizeof(BASE_OBJECT)),
...
BOOL scrOrderDroidStatsLoc(void)
{
	...
	ASSERT_TEXT(PTRVALID(psStats, sizeof(BASE_STATS)),
...
BOOL scrSetDroidSecondary(void)
{
	...
	secondarySetState(psDroid, (SECONDARY_ORDER)sec, (SECONDARY_STATE)state);
...
BOOL scrSetGroupSecondary(void)
{
	...
	grpSetSecondary(psGroup, (SECONDARY_ORDER)sec, (SECONDARY_STATE)state);
...
BOOL scrSetDroidTarPref(void)
{
	...
	ASSERT_TEXT( (SCR_DT_COMMAND		== pref) ||
			 (SCR_DT_SENSOR			== pref) ||
							 ...
							 SDWORD x1, SDWORD y1, SDWORD x2, SDWORD y2)
{
	...
	case SCR_TAR_DROID:
		...
		break;
	default:
		ASSERT_TEXT(FALSE, "scrTargetInArea: invalid target type");
		return NULL;
	}
...
BOOL skGetTemplate(void)
{
	...
	if (!stackPushResult((INTERP_TYPE)ST_TEMPLATE, (UDWORD)(psT->psTempl) ))
	{
		return FALSE;
	}
	return TRUE;
