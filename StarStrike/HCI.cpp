...
/* functions to select && get stats from the current object list */
...
/* The current map width && height in the new map edit boxes */
 ...
 * intAddBuild, intAddManufacture && intAddResearch
 */
 ...
 * This works for droid || structure buttons
 */
...
DBPRINTF(("HCI Quit %d\n",retID));
	...
	default:
		...
		case INT_NORMAL:
			break;
		default:
			ASSERT_TEXT(FALSE, "intRunWidgets: unknown interface mode");
			break;
		}
		break;
	}	
...
/* Get  && validate the new map size from the options screen */
...
//#endif
		...
		case IDOPT_LOADGAME:
			...
			break;
		default:
			ASSERT_TEXT(FALSE, "intProcessOptions: Unknown return code");
			break;
		}
	}
}
...
static void intProcessEdit(UDWORD id)
{
	...
	case IDED_LABEL:
		break;
	default:
		ASSERT_TEXT(FALSE, "intProcessEdit: Unknown return code");
		break;
	}
}
#endif
...
//DBPRINTF(("intAddStats(%p,%d,%p,%p)\n",ppsStatsList, numStatsListEntries, psStats, psObj);
	...
	/* Reset the tabs && lock the button */
	widgSetTabs(psWScreen, IDOBJ_TABFORM, objMajor, objMinor);
	if(id != 0)
	{
...
//				psObj->selected = TRUE;
	...
	else
	{
		...
		{
			/* !a button on the build form, must be on the stats form */
			intProcessStats(id);
		}
...
static void intProcessStats(UDWORD id)
{
	...
	if (id >= IDSTAT_START &&
		id <= IDSTAT_END)
	{
		ASSERT_TEXT(id - IDSTAT_START < numStatsListEntries,
...
//printf("WKEY_SECONDARY : %d\n",id);
		...
		else
		{
			...
			if(objMode == IOBJ_MANUFACTURE)
			{
				...
				ASSERT_TEXT(PTRVALID(psObjSelected, sizeof(STRUCTURE)),
					"intProcessStats: Invalid structure pointer");
				ASSERT_TEXT(PTRVALID(psStats, sizeof(DROID_TEMPLATE)),
...
//		widgEndScreen(psWScreen);
		...
		/*if(DroidIsBuilding((DROID *)psObjSelected)) {
			...
			ASSERT_TEXT(Structure!=NULL,"Bad structure pointer");
...
//	int	i;
	...
	/*draw the background for the design screen && the Intelligence screen*/
...
void intBuildFinished(DROID *psDroid)
{
	...
	if ((intMode == INT_OBJECT || intMode == INT_STAT) &&
		...
		objMode == IOBJ_BUILD)
	{
		/* Find which button the droid is on && clear it's stats */
		droidID = 0;
...
void intManufactureFinished(STRUCTURE *psBuilding)
{
	...
	if ((intMode == INT_OBJECT || intMode == INT_STAT) &&
		(objMode == IOBJ_MANUFACTURE))
	{
		/* Find which button the structure is on && clear it's stats */
		structureID = 0;
    	numObjects = 0;
 ...
 * given the total number of buttons && the number of
 * buttons per page.
...
//	widgEndScreen(psWScreen);
	...
	/* Add the load && save game buttons */
	sButInit.formID = IDOPT_FORM;
...
//	W_LABINIT		sLabInitCmdFacts;
	...
	ASSERT_TEXT(psSelected == NULL || PTRVALID(psSelected, sizeof(BASE_OBJECT)),
...
//#endif
	...
	/* Reset the current object && store the current list */
	psObjSelected = NULL;
	psObjList = psObjects;
...
// Action progress bar.
	...
	for(i=0; i<(UDWORD)numObjects; i++)
	{
		...
		if(psObj->died == 0) {	// Don't add the button if the objects dead.
			...
			/* Got an object - set the text && tip for the button */
			switch (psObj->type)
			{
			case OBJ_DROID:
	...
	// Get the construction power of a structure..
				...
				switch(Structure->pStructureType->type) {
					...
					default:
						ASSERT_TEXT(FALSE, "intAddObject: invalid structure type");
				}
...
/* Get the object refered to by a button ID on the object screen.
 * This works for object || stats buttons
 */
static BASE_OBJECT *intGetObject(UDWORD id)
{
//	UDWORD			objID;
	...
	/* Find the object that the ID refers to */
	ASSERT_TEXT( ( (SDWORD)id - IDOBJ_OBJSTART >= 0 ) &&
...
found:	// Jump to here if an object is found
	ASSERT_TEXT(psObj != NULL, "intGetObject: couldn't match id to button");
	*/
...
donelab:
		...
		/* If this matches psSelected note the form && button */
...
//	widgStartScreen(psWScreen);
	...
	/* Set the correct page && button if necessary */
	if (statID)
	{
...
//	UDWORD	i;
	...
	ASSERT_TEXT(PTRVALID(psObj, sizeof(DROID)) && psObj->type == OBJ_DROID,
...
static BOOL setConstructionStats(BASE_OBJECT *psObj, BASE_STATS *psStats)
{
	...
	ASSERT_TEXT(psStats == NULL || PTRVALID(psStats, sizeof(STRUCTURE_STATS)),
...
static BOOL selectResearch(BASE_OBJECT *psObj)
{
	...
	ASSERT_TEXT(PTRVALID(psObj, sizeof(STRUCTURE)) && psObj->type == OBJ_STRUCTURE,
		"selectResearch: invalid Structure pointer");

	psResFacility = (STRUCTURE *)psObj;

	/* A Structure is a research facility if its type = REF_RESEARCH && is 
	   completely built*/
...
static BOOL setResearchStats(BASE_OBJECT *psObj, BASE_STATS *psStats)
{
	...
	/* psStats might be NULL if the operation is canceled in the middle */
	ASSERT_TEXT(psStats == NULL || PTRVALID(psStats, sizeof(RESEARCH)),
...
static BOOL selectManufacture(BASE_OBJECT *psObj)
{
	...
	/* A Structure is a Factory if its type = REF_FACTORY || REF_CYBORG_FACTORY ||
	REF_VTOL_FACTORY && it is completely built*/
...
static BOOL setManufactureStats(BASE_OBJECT *psObj, BASE_STATS *psStats)
{
	...
	/* psStats might be NULL if the operation is canceled in the middle */
	ASSERT_TEXT(psStats == NULL || PTRVALID(psStats, sizeof(DROID_TEMPLATE)),
...
//			DBMB(("Non stop production"));
		...
		/* check power if factory !on infinte production*/
...
static BOOL intAddBuildScreens(DROID *pSelected)
{
	...
	if(pSelected == NULL) {
		...
		for(psCurrDroid = apsDroidLists[selectedPlayer]; psCurrDroid;
			...
			if (droidType(psCurrDroid) == DROID_CONSTRUCT ||
...
static void intObjectRMBPressed(UDWORD id)
{
	...
	ASSERT_TEXT((SDWORD)id - IDOBJ_OBJSTART < numObjects,
...
static void intObjStatRMBPressed(UDWORD id)
{
	...
	ASSERT_TEXT((SDWORD)id - IDOBJ_STATSTART < numObjects,
...
of the required type. If there is more than one, they are all deselected && 
...
cyborg factories 1-5 && then Vtol factories 1-5*/
void orderFactories(void)
{
	...
	ASSERT_TEXT(numObjects <= NUM_FACTORY_TYPES * MAX_FACTORY, 
	...
	while(entry < numObjects)
	{
			...
			psStruct = psNext)
		{
			...
			ASSERT_TEXT(StructIsFactory(psStruct), 
				"orderFactories: structure is !a factory");
