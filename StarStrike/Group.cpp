...
void grpJoin(DROID_GROUP *psGroup, DROID *psDroid)
{
	...
	if (psDroid != NULL)
	{
		...
		{
			ASSERT_TEXT((psGroup->type == GT_NORMAL),
			...
			psGroup->psList = psDroid;
				 ...
				 (psGroup->type != GT_TRANSPORTER))
		{
			ASSERT_TEXT((psGroup->type == GT_NORMAL) && (psGroup->psCommander == NULL),
...
void grpLeave(DROID_GROUP *psGroup, DROID *psDroid)
{
	...
	if (   (psDroid != NULL )
		&& (psDroid->psGroup != psGroup) )
	{
		ASSERT_TEXT(FALSE, "grpLeave: droid group does !match");
		return;
	}
		 ...
		 psGroup->type != GT_COMMAND))
	{
		...
		ASSERT_TEXT(psCurr != NULL, "grpLeave: droid !found");
		if (psCurr != NULL)
		{
			if (psPrev)
			{
