...
void clustInitialise(void)
{
	...
	ASSERT_TEXT(CLUSTER_MAX <= UBYTE_MAX,
	...
	for(cluster=1; cluster<CLUSTER_MAX; cluster++)
	{
		...
		for(player=0; player<MAX_PLAYERS; player++)
		{
			...
			for(psDroid=apsDroidLists[player]; psDroid; psDroid=psDroid->psNext)
			{
				...
				{
					ASSERT_TEXT( (found == MAX_PLAYERS) || (droidUsage != 0),
			...
			for(psStruct=apsStructLists[player]; psStruct; psStruct=psStruct->psNext)
			{
				...
				{
					ASSERT_TEXT( (found == MAX_PLAYERS) || (structUsage != 0),
						"clustValidateUsage: cluster has mixed players");
			...
			if (found == player)
			{
				ASSERT_TEXT( (droidUsage == 0) || (structUsage == 0),
					"clustValidateUsage: cluster contains both droids && structs");

				ASSERT_TEXT( aClusterUsage[cluster] == droidUsage + structUsage,
...
void clustRemoveObject(BASE_OBJECT *psObj)
{
	...
	ASSERT_TEXT(psObj->cluster < CLUSTER_MAX,
		"clustRemoveObject: invalid cluster number");

	// update the usage counter
	if (psObj->cluster != 0)
	{
		ASSERT_TEXT(aClusterUsage[psObj->cluster] > 0,
...
//				aClusterVisibility[newCluster] = aClusterVisibility[oldCluster];
	...
	case OBJ_STRUCTURE:
		...
		break;
	default:
		ASSERT_TEXT(FALSE,"clustUpdateObject: invalid object type");
		break;
	}
...
{
	ASSERT_TEXT((clusterID >= 0) && (clusterID < CLUSTER_MAX),
...
void clustGetCenter(BASE_OBJECT *psObj, SDWORD *px, SDWORD *py)
{
	...
	case OBJ_STRUCTURE:
		...
		break;
	default:
		ASSERT_TEXT(FALSE,"clustGetCenter: invalid object type");
		psList = NULL;
		break;
	}
...
//				clustGetClusterID(psObj), psObj->player, player));
			...
			case OBJ_FEATURE:
				...
				break;
			default:
				ASSERT_TEXT(FALSE, "clustObjectSeen: invalid object type");
				break;
			}
...
//			psObj->player, psObj->cluster));
		...
		case OBJ_STRUCTURE:
			...
			break;
		default:
			ASSERT_TEXT(FALSE, "clustObjectAttacked: invalid object type");
			break;
		}
