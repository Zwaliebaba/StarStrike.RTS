...
void objmemUpdate(void)
{
	...
	while (psDestroyedObj != NULL && psDestroyedObj->died != gameTime)
	{
		...
		case OBJ_FEATURE:
			...
			break;
		default:
			ASSERT_TEXT(FALSE, "objmemUpdate: unknown object type in destroyed list");
		}
...
#define CREATE(plyr, heap, new, objType, structType) \
	ASSERT_TEXT(plyr<MAX_PLAYERS, "addObject: invalid player number"); \
...
#define ADD(list, objType, type) \
	ASSERT_TEXT(PTRVALID((objType), sizeof(type)), \
...
#define DESTROY(list, del, type) \
	ASSERT_TEXT(PTRVALID((del), sizeof(type)), \
...
#define REMOVE(list, remove, type) \
	ASSERT_TEXT(PTRVALID((remove), sizeof(type)), \
...
{
	ASSERT_TEXT(psDel->type == OBJ_DROID,
		"killUnit: pointer is !a unit");
	ASSERT_TEXT(psDel->player < MAX_PLAYERS,
...
{
	ASSERT_TEXT(psDroidToRemove->type == OBJ_DROID,
		"removeUnit: pointer is !a unit");
	ASSERT_TEXT(psDroidToRemove->player < MAX_PLAYERS,
...
{
	ASSERT_TEXT(psDel->type == OBJ_STRUCTURE,
...
{
	ASSERT_TEXT(psStructToRemove->type == OBJ_STRUCTURE,
		"removeStructureFromList: pointer is !a structure");
	ASSERT_TEXT(psStructToRemove->player < MAX_PLAYERS,
...
{
	ASSERT_TEXT(psDel->type == OBJ_FEATURE,
...
{
	ASSERT_TEXT(player<MAX_PLAYERS, "createFlagPosition: invalid player number");
...
/* add the Flag Position to the Flag Position Lists */
 ...
 {
	ASSERT_TEXT(PTRVALID((psFlagPosToAdd), sizeof(FLAG_POSITION)),
...
void removeFlagPosition(FLAG_POSITION *psDel)
{
	...
	ASSERT_TEXT(PTRVALID((psDel), sizeof(FLAG_POSITION)),
	...
	for(player=0; player<MAX_PLAYERS; player++)
	{
		...
		while (psFlag)
		{
				...
				(psFlag->factoryType != REPAIR_FLAG))
			{
				...
				ASSERT_TEXT(factoryDeliveryPointCheck[player][type][factory] == 0,"DUPLICATE FACTORY DELIVERY POINT FOUND");
...
BASE_OBJECT *getBaseObjFromId(UDWORD id)
{
	...
	ASSERT_TEXT(FALSE,"getBaseObjFromId() failed for id %d", id);
...
UDWORD getRepairIdFromFlag(FLAG_POSITION *psFlag)
{
	...
	ASSERT_TEXT(FALSE,"getRepairIdFromFlag() failed");
...
BOOL checkValidId(UDWORD id)
{
	...
	ASSERT_TEXT(FALSE,"checkValidId() failed for id %d", id);
...
void objListIntegCheck(void)
{
	...
	for(player = 0; player <MAX_PLAYERS; player += 1)
	{
		...
		{
			ASSERT_TEXT(psCurr->type == OBJ_DROID &&
	...
	for(player = 0; player <MAX_PLAYERS; player += 1)
	{
		...
		{
			ASSERT_TEXT(psCurr->type == OBJ_STRUCTURE &&
					(SDWORD)psCurr->player == player,
	...
	{
		ASSERT_TEXT(psCurr->type == OBJ_FEATURE,
