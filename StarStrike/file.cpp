...
{
  std::wstring configFile = FileSys::GetHomeDirectory() + L"newkind.cfg";
...
void read_scanner_config_file(const std::wstring& filename)
{
  char str[256];

  std::wstring configFile = FileSys::GetHomeDirectory() + filename;
...
void read_config_file(void)
{
  ...
  hstring scannerFile = to_hstring(str);
  read_scanner_config_file(scannerFile.c_str());

  fclose(fp);
}

int checksum(const unsigned char *block)
{
  int acc = 0x49;
  int carry = 0;
...
int _save_commander_file(const char *path)
{
  ...
  for (i = 0; i < NO_OF_STOCK_ITEMS; i++) block[23 + i] = cmdr.current_cargo[i];
  ...
  for (i = 0; i < NO_OF_STOCK_ITEMS; i++) block[53 + i] = stock_market[i].current_quantity;
  ...
  for (i = 76; i < 256; i++) block[i] = 0;

  if (fwrite(block, 256, 1, fp) != 1) return 1;
...
int _load_commander_file(const char *path, struct commander *saved_cmdr)
{
  unsigned char block[256];
  int i;
  ...
  if (fread(block, 256, 1, fp) != 1) return 1;

  int chk = checksum(block);

  if ((block[74] != (chk ^ 0xA9)) || (block[75] != chk)) return 1;
  ...
  for (i = 0; i < NO_OF_STOCK_ITEMS; i++) saved_cmdr->current_cargo[i] = block[23 + i];
  ...
  for (i = 0; i < NO_OF_STOCK_ITEMS; i++) saved_cmdr->station_stock[i] = block[53 + i];
...
void set_commander_name(char *fname, char *cname)
{
  ...
  for (int i = 0; i < MAX_CMDR_NAME_LENGTH; i++)
  {
    if (!isalnum(*fname)) break;
...
{
  char* result = static_cast<char*>(malloc(strlen(patha) + strlen(PATH_SEPARATOR) + strlen(pathb) + 1));
...
{
  char* path = concat_paths(save_game_dir, filename);

  int rv = _load_commander_file(path, saved_cmdr);

  if (!rv) set_commander_name(filename, saved_cmdr->name);
...
int save_commander_file(char *filename)
{
  ...
  char* path = concat_paths(save_game_dir, filename);
  int rv = _save_commander_file(path);
  free(path);
  return rv;
}