 ...
 * Order.c
 *
 * Functions for setting the orders of a droid || group of droids
 *
 */
...
//	UDWORD		sensorDist;
	...
	ASSERT_TEXT(psDroid->droidType == DROID_REPAIR || psDroid->droidType == 
...
BASE_OBJECT * checkForDamagedStruct(DROID *psDroid, STRUCTURE *psTarget)
{
    ...
    ASSERT_TEXT(psDroid->droidType == DROID_CONSTRUCT || psDroid->droidType == 
...
//
            ...
            /*don't try the unload if moving droids to safety && still got some 
            droids left  - wait until full && then launch again*/
            if (psDroid->player == selectedPlayer && getDroidsToSafetyFlag() && 
                missionDroidsRemaining(selectedPlayer))
            {
...
//			(secondaryGetState(psDroid, DSO_ATTACK_LEVEL, &state) && (state != DSS_ALEV_ALWAYS)))
    ...
    case DORDER_DISEMBARK:
        ...
        if (bMultiPlayer)
        {
            ...
            {
                /*once the Transporter has reached its destination (&& landed), 
	...
	case DORDER_RTR_SPECIFIED:
		...
		else if (psDroid->action == DACTION_NONE)
		{
			...
			ASSERT_TEXT( PTRVALID(psRepairFac, sizeof(REPAIR_FACILITY)),
...
/*		if (psDroid->psTarget == NULL)
		...
		break;
	default:
		ASSERT_TEXT(FALSE, "orderUpdateUnit: unknown order");
	}
...
void orderCmdGroupBase(DROID_GROUP *psGroup, DROID_ORDER_DATA *psData)
{
	...
	ASSERT_TEXT(PTRVALID(psGroup, sizeof(DROID_GROUP)),
...
static void orderPlayFireSupportAudio( BASE_OBJECT *psObj )
{
	...
	switch ( psObj->type )
	{
		case OBJ_DROID:
			psDroid = (DROID *) psObj;
			ASSERT_TEXT( PTRVALID(psObj, sizeof(DROID)),
		...
		case OBJ_STRUCTURE:
			ASSERT_TEXT( PTRVALID(psObj, sizeof(STRUCTURE)),
...
//			psDroid->order = DORDER_ATTACK;
	...
	case DORDER_BUILD:
		...
		ASSERT_TEXT(PTRVALID(psOrder->psStats, sizeof(STRUCTURE_STATS)),
...
//								&actionX,&actionY))
		{
			...
			ASSERT_TEXT(psDroid->psTarStats != NULL,
	...
	case DORDER_RTR_SPECIFIED:
		...
		if (psOrder->psObj == NULL)
		{
			...
			for(psStruct=apsStructLists[psDroid->player]; psStruct; psStruct = psStruct->psNext)
			{
					...
					((psStruct->pStructureType->type == REF_HQ) && (psRepairFac == NULL)))
				{
					...
					/* choose current structure if first repair facility found ||
		...
		if ( psRepairFac != NULL )
		{
			if (psRepairFac->pStructureType->type == REF_REPAIR_FACILITY)
			{
				...
				psDroid->psTarget = (BASE_OBJECT *) psRepairFac;
                /*if in multiPlayer, && the Transporter has been sent to be 
...
//					(sLandingZone[0].x1 + sLandingZone[0].x2)/2, (sLandingZone[0].y1 + sLandingZone[0].y2)/2);
	...
	case DORDER_RECYCLE:
		...
		for(psStruct=apsStructLists[psDroid->player]; psStruct; psStruct = psStruct->psNext)
		{
                ...
                psStruct->pStructureType->type == REF_REPAIR_FACILITY)
			{
				...
				/* choose current structure if first facility found ||
				 * nearer than previously chosen facility
				 */
	...
	case DORDER_REARM:
		...
		break;
	default:
		ASSERT_TEXT(FALSE, "orderUnitBase: unknown order");
		break;
	}
						...
						BASE_STATS *psStats, UDWORD x, UDWORD y)
{
	...
	ASSERT_TEXT(order == DORDER_BUILD,
						...
						BASE_STATS *psStats, UDWORD x1, UDWORD y1, UDWORD x2, UDWORD y2)
{
	...
	ASSERT_TEXT(order == DORDER_LINEBUILD,
		"orderUnitStatsTwoLoc: Invalid order for location");
	ASSERT_TEXT(x1 == x2 || y1 == y2,
...
/* Add an order with a location && a stat */
void orderDroidStatsTwoLocAdd(DROID *psDroid, DROID_ORDER order,
						BASE_STATS *psStats, UDWORD x1, UDWORD y1, UDWORD x2, UDWORD y2)
{
	DROID_ORDER_DATA	sOrder;
...
BOOL orderDroidList(DROID *psDroid)
{
	...
	if (psDroid->listSize > 0)
	{
        ...
        case DORDER_LINEBUILD:
            ...
            break;
        default:
            ASSERT_TEXT(FALSE, "orderDroidList: Invalid order");
            return FALSE;
        }
...
//	if (vtolDroid(psDroid) OR psDroid->droidType == DROID_TRANSPORTER)
    ...
    if (psDroid->droidType == DROID_TRANSPORTER)
	{
        ...
        to fly to location && all units disembark*/
	...
	{
		order = DORDER_PATROL;
		secondarySetState(psDroid, DSO_PATROL, (SECONDARY_STATE)0);
	}
...
/* Give selected droids an order from a location target || 
...
/*		else
			  ...
			  psObj->type == OBJ_STRUCTURE )
	{
		...
		{
			/* check repair facility && in need of repair */
			if ( psStruct->pStructureType->type == REF_REPAIR_FACILITY &&
				 psStruct->status == SS_BUILT)
...
//				secondarySetState(psDroid, DSO_HALTTYPE, DSS_HALT_HOLD);
        ...
        psObj->type == OBJ_FEATURE )
	{
		...
		ASSERT_TEXT( PTRVALID(psObj, sizeof(FEATURE)),
...
BOOL secondaryGetState(DROID *psDroid, SECONDARY_ORDER sec, SDWORD *pState)
{
	UDWORD	state;
...
void secondaryCheckDamageLevel(DROID *psDroid)
{
	...
	if( secondaryGetState(psDroid, DSO_REPAIR_LEVEL, &State) )
	{
		...
		if( State && (SDWORD)(PERCENT(psDroid->body,psDroid->originalBody)) <= State)
		{
			...
			/* set return to repair if !on hold */
			if ( psDroid->order != DORDER_RTR &&
				 psDroid->order != DORDER_RTB &&
				 !vtolRearming(psDroid))
			{
...
void secondarySetAverageGroupState(UDWORD player, UDWORD group)
{
	...
	for(i=0; i<MAX_ORDERS; i++)
	{
		...
		secondarySetGroupState(player, group, (SECONDARY_ORDER)aOrders[i].order, (SECONDARY_STATE)state);
	}
}
...
//					 &&(psDroid->sMove.MovementList[nCount].YCoordinate!= -1);nCount++);// number of new moves.
		...
		{
			DBPRINTF(("Waypoint List Exceeded, Point !Added."));
...
BOOL setFactoryState(STRUCTURE *psStruct, SECONDARY_ORDER sec, SECONDARY_STATE State)
{
    ...
    {
        ASSERT_TEXT(FALSE, "setFactoryState: structure is !a factory");
        return FALSE;
    }
...
BOOL getFactoryState(STRUCTURE *psStruct, SECONDARY_ORDER sec, SECONDARY_STATE *pState)
{
    ...
    if (!StructIsFactory(psStruct))
    {
        ASSERT_TEXT(FALSE, "getFactoryState: structure is !a factory");
        return FALSE;
    }
