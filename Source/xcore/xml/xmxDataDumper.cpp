...
bool BXMXDataDumper::dumpNode(uint nodeIndex)
{
    ...
    bool unicode;
    if (!BXMXVariantHelpers::unpackVariantToString(pName->getPtr(), pName->getBufSize(), unicode,
                                                   BConstDataBuffer(mpXMXData->getVariantData().getPtr(),
                                                                    mpXMXData->getVariantData().getSize()), node.mName,
                                                   false))
        return false;

    if (!BXMXVariantHelpers::unpackVariantToString(pText->getPtr(), pText->getBufSize(), unicode,
                                                   BConstDataBuffer(mpXMXData->getVariantData().getPtr(),
                                                                    mpXMXData->getVariantData().getSize()), node.mText,
                                                   false))
        return false;
    ...
    for (uint attrIndex = 0; attrIndex < numAttributes; attrIndex++)
    {
        ...
        if (!BXMXVariantHelpers::unpackVariantToString(pAttrName->getPtr(), pAttrName->getBufSize(), unicode,
                                                       BConstDataBuffer(mpXMXData->getVariantData().getPtr(),
                                                                        mpXMXData->getVariantData().getSize()),
                                                       attribute.mName, false))
            return false;

        if (!BXMXVariantHelpers::unpackVariantToString(pAttrText->getPtr(), pAttrName->getBufSize(), unicode,
                                                       BConstDataBuffer(mpXMXData->getVariantData().getPtr(),
                                                                        mpXMXData->getVariantData().getSize()),
                                                       attribute.mText, false))
            return false;
